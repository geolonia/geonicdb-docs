...
GeonicDB は、単一の Context Broker で NGSIv2 と NGSI-LD の両方の API をサポートしています。両 API は統一された内部ストレージ形式を共有していますが、**エンティティはプロトコルごとに分離されています** -- NGSIv2 経由で作成されたエンティティは NGSIv2 経由でのみアクセス可能であり、NGSI-LD についても同様です。
...
GeonicDB のデュアル API アーキテクチャは、FIWARE NGSIv2 と ETSI NGSI-LD の両方の仕様をサポートしています。各エンティティは、それを作成したプロトコルでタグ付けされ、2 つの API 間での厳格な分離が保証されます。
...
- **API 選択の自由** - 各ユースケースに最適な API を選択できます。プロトコル間の連携が必要な場合はフェデレーションを使用します

---## 統合内部フォーマット

GeonicDB は、両方の API からのデータを統合された内部フォーマットに変換します。
...
GeonicDB は、NGSIv2 型、内部型、NGSI-LD 型を以下のルールに従って変換します。
...
**例 (NGSIv2 レスポンスと `options=dateCreated,dateModified`):**
...
**注意:** `pick` パラメータを使用する場合、レスポンスには明示的に要求された属性と `@context`、`id`、`type` (常に存在) が含まれます。ただし、`createdAt` と `modifiedAt` は、`pick` を使用した場合でも返されません。これらのシステム属性には `sysAttrs` オプションが必要です。
...
以下の機能は両方の API で共有されています。### 1. クエリ言語
...
| `..` | 範囲 (含む) | `mq=temperature.accuracy==0.9..1.0` |
...
|--------|--------|---------|-------------|
| **総件数** | `Fiware-Total-Count` | `NGSILD-Results-Count` | クエリ結果の総数 |
...
> **注:** プロトコルが分離されているため、NGSI-LD エンティティ (Relationship 属性を持つものを含む) は NGSIv2 API 経由ではアクセスできません。
...
> **注:** プロトコルが分離されているため、NGSI-LD エンティティ (LanguageProperty 属性を持つものを含む) は NGSIv2 API 経由ではアクセスできません。### 3. スコープ (Scope 階層)
...
- **servicePath と scope は独立した概念です (#964):** 自動的に同期されることはありません
  - NGSIv2 `Fiware-ServicePath` → DB には `servicePath` として格納されます (インフラストラクチャレベルの分離)
  - NGSI-LD `scope` → DB には `scope` として格納されます (ユーザー定義の論理階層)
  - NGSI-LD は ETSI GS CIM 009 仕様に従い `Fiware-ServicePath` ヘッダーを無視します### 4. 属性の射影 (pick / omit パラメータ)
...
- `omit` を使用する場合: 指定した属性以外のすべての属性が含まれます。`id` と `type` は除外できません (ETSI GS CIM 009 V1.9.1 仕様に準拠)
...
- GeonicDB は Smart Data Models の `@context` の自動補完をサポートしていますが、`@context` は NGSIv2 API からは返されません
...
- **NGSIv2**（標準）では、同じ ID でも異なる型を持つエンティティが共存できますが、GeonicDB はこの動作をサポート**しません**
...
- **バッチ作成**（`POST /ngsi-ld/v1/entityOperations/create`）：重複した ID に対してエンティティごとのエラー詳細を含む `207` が返されます
- 同じ ID を持つエンティティ間の型による曖昧さ解消のための NGSIv2 `?type=` パラメータは適用されなくなりました

この統一により、NGSIv2 の型ベースの曖昧さ解消が NGSI-LD の一意 ID モデルと競合する相互運用性の問題のクラスが排除されます。
...
- 使用する API に関わらず、一貫性と将来の移行のために URN 形式を使用することを推奨します

**ベストプラクティス：**

- NGSIv2 API を使用する場合でも、すべてのエンティティに URN 形式を使用してください
- NGSIv2 から NGSI-LD に移行する場合、エンティティは NGSI-LD API 経由で再作成する必要があります（プロトコルの分離により API 間のアクセスができません）

---

## フェデレーション

GeonicDB のフェデレーション機能は、リモートコンテキストプロバイダのプロトコルを自動検出します。
...
2. **自動検出** - URL パスからの自動検出：
   - `/v2/` を含む → NGSIv2
   - `/ngsi-ld/` を含む → NGSI-LD
   - それ以外 → NGSIv2（デフォルト）### NGSIv2 からのフェデレーション
...
3. NGSI-LD プロトコルを使用してクエリを転送します: `GET /ngsi-ld/v1/entities/urn:ngsi-ld:Vehicle:V999`4. NGSI-LD → 内部形式 → NGSIv2 の順でレスポンスを変換し、クライアントに返します
...
3. NGSIv2 プロトコルを使用してクエリを転送します: `GET /v2/entities/urn:ngsi-ld:Sensor:S888`4. NGSIv2 → 内部形式 → NGSI-LD の順でレスポンスを変換し、クライアントに返します

---

## ユースケースとベストプラクティス### どの API を使うべきか？
...
2. **クロスプロトコルのニーズには Federation を使用** - NGSIv2 クライアントが NGSI-LD エンティティにアクセスする必要がある場合(またはその逆)は、Federation を介してコンテキストソースを登録してください
3. **マイグレーションには再作成が必要** - NGSIv2 から NGSI-LD へエンティティを移行するには、NGSIv2 API からエクスポートし、NGSI-LD API 経由で再作成する必要があります。自動的なクロスプロトコルマイグレーションはありません### ベストプラクティス
...
| **エンティティの分離** | `protocol: 'ngsiv2'` | `protocol: 'ngsild'` | 各 API は独自のエンティティのみを認識 |
| **エンティティ ID** | 任意の文字列 | URI (URN を推奨) | URN を推奨。**ID はテナント + servicePath ごとに一意** (型による識別は削除) |
| **属性の型** | シンプル (Number、Text など) | セマンティック (Property、Relationship など) | 型マッピングルールで対応関係を定義 (上記の表を参照) |
| **システム属性** | `dateCreated`、`dateModified` | `createdAt`、`modifiedAt` | 内部では統一され、API ごとに変換 |
| **地理クエリ** | ✅ | ✅ | 共有機能 |
...
| **ユースケース** | IoT、レガシーシステム | セマンティックウェブ、オープンデータ | ユースケースごとに 1 つの API を選択 |

GeonicDB は厳格なプロトコル分離により、NGSIv2 と NGSI-LD の両方の API を提供します。ユースケースに最適な API を選択し、クロスプロトコルアクセスが必要な場合はフェデレーションを活用してください。
