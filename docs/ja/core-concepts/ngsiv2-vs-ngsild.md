...
GeonicDB は単一の Context Broker で NGSIv2 と NGSI-LD の両方の API をサポートしています。両方の API は統一された内部ストレージフォーマットを共有していますが、**エンティティはプロトコルによって分離されています** -- NGSIv2 経由で作成されたエンティティは NGSIv2 経由でのみアクセス可能であり、NGSI-LD についても同様です。
...
GeonicDB のデュアル API アーキテクチャは、FIWARE NGSIv2 と ETSI NGSI-LD の両方の仕様をサポートしています。各エンティティは、それを作成したプロトコルでタグ付けされ、2 つの API 間の厳格な分離が保証されます。
...
* クエリはプロトコルによってフィルタリングされます:NGSIv2 API は `protocol: 'ngsiv2'` のエンティティのみを返し、NGSI-LD API は `protocol: 'ngsild'` のエンティティのみを返します
...
* **既存システムとの統合** - NGSIv2 と NGSI-LD のワークロードを干渉することなく並行して実行できます
...
エンティティのメタデータ(作成タイムスタンプと変更タイムスタンプ)は、API によって異なる名前を使用します。
...
**例(NGSIv2 レスポンス、`options=dateCreated,dateModified` の場合):**
...
> **メディアタイプ vs. 表現フォーマット。** これらは独立した軸です。`Accept` ヘッダーは *メディアタイプ* を選択し、`options` / `format` は以下の *表現フォーマット* を選択します。ETSI GS CIM 009 clause 6.3.4 によると、**`Accept` ヘッダーが存在しない場合(または `*/*`)は `application/json` に解決され**、`application/ld+json` ではありません — レスポンスボディに `@context` を埋め込みたい場合は `Accept: application/ld+json` を明示的に送信してください(#1734)。`application/json` / `application/ld+json`(および エンティティエンドポイントでの `application/geo+json`)のみがサポートされています。それ以外は `406 Not Acceptable` を返します。[API\_NGSILD.md — Content Negotiation](../api-reference/ngsild.md#content-negotiation-and-context) を参照してください。
...
> **メタデータ名の文字セット (#1946):** `mq` 条件内の属性名とメタデータ名の両方が `\w+` (Orion 互換文法) として解析されるため、メタデータ名は書き込み時に `^[A-Za-z0-9_]+$` に対して検証されます。`.`、`-`、`$`、または空白を含む名前は、クエリで取得できないデータとして保存されるのではなく、`400` で拒否されます。[エンティティフィールドの文字セット](#entity-field-character-set-id--type--attribute-name--geonicdb-独自拡張) を参照してください。
...
NGSI-LD 仕様 (ETSI GS CIM 009 clause 4.18 / 5.2.x) は、スコープセグメント文字を `[A-Za-z0-9_]` (ALPHA / DIGIT / アンダースコア) に制限しています。GeonicDB はこれを **POSIX ポータブルファイル名文字セット** にわずかに拡張し、スコープがファイルシステムスタイルのパスとして直感的になるようにしています:
...
スコープ文字列ごとの正規表現: `^(/[A-Za-z0-9._][A-Za-z0-9._-]*)+$`

**なぜ POSIX ポータブルか?** 純粋な NGSI-LD 仕様セット (`[A-Za-z0-9_]`) は実世界での使用には過度に制限的です。POSIX ポータブルは `.` と `-` を追加し、`/com.example.region/tokyo-shibuya` や `/v1.0/sensors` のような自然な名前を可能にします。同時に、`scopeQ` 予約文字 (`;` `+` `#`) と半角スペースを拒否します — これらは**静かな地雷**になります: 保存は成功しますが、エンティティは `scopeQ` 経由で照合不可能になります。
...
検証は `POST/PATCH/PUT /entities`、`POST /entityOperations/*`、temporal エンドポイント、ReactiveCore ルールの `createEntity` アクションに適用されます。既存の保存済みエンティティで非準拠スコープを持つもの (pre-#1189 データ) は読み取り可能なまま残ります; 新しい書き込みリクエストのみが拒否されます。

厳密な NGSI-LD 仕様準拠を目指すクライアントは、実装間での移植性のためにスコープを `[A-Za-z0-9_]` 内に保つべきです。

#### スコープ更新セマンティクス (GeonicDB 独自拡張)

NGSI-LD 仕様 (ETSI GS CIM 009) は、エンティティ上の `scope` フィールドと `scopeQ` クエリパラメータを定義していますが、PATCH/PUT/upsert を通じて既存スコープを解除する方法は**指定していません**。GeonicDB は以下のように API を拡張します:
...
* 解除拡張をサポートするエンドポイント: `PATCH /entities/{id}` (マージパッチ)、`PUT /entities/{id}` (置換)、`POST /entityOperations/upsert` (マージと置換の両モード)。attrs レベルのエンドポイント `PATCH /entities/{id}/attrs` は現在 `scope` フィールドを扱いません (送信された値は黙って無視されます)。スコープを更新する必要がある場合は、エンティティレベルの `PATCH /entities/{id}` を代わりに使用してください。
  
* 明示的解除状態 (`scope: null`) はフィールドを完全に削除するのではなく、ストレージに保存されます。これにより API は「スコープが意図的にクリアされた」と「スコープが設定されたことがない」を区別できます。
  
* `scopeQ` クエリは `scope: null` のエンティティにマッチしません (フィールドには照合するパスコンポーネントがありません)。これは、スコープを持ったことがないエンティティと一貫しています。
...
* `id` は `:` を受け入れます。なぜなら NGSI-LD URN 形式 `urn:ngsi-ld:Type:identifier` が正規の id 形式だからです。
  
* **NGSI-LD パス** は POSIX ポータブル短縮名に加えて、`type` として絶対 IRI を受け入れます (#1211)。例: `https://uri.fiware.org/ns/data-models#WeatherObserved`、`urn:ngsi-ld:Type:Sensor`。**NGSIv2 パス** は URI 形式を引き続き拒否します (プロトコル分離)。
...
* エラーレスポンスには、問題のある値と許可される文字セットが含まれます。例: `Entity type contains invalid characters (allowed: A-Z a-z 0-9 . _ - (must not start with -)); got "Sensor@Type"`。
...
検証は NGSIv2 の `POST/PATCH/PUT /v2/entities` と NGSI-LD の `POST/PATCH/PUT /ngsi-ld/v1/entities`、temporal エンドポイント、およびバッチ操作に適用されます。非準拠フィールドを持つ既存の保存済みエンティティ(#1209 以前のデータ)は読み取り可能なまま残ります。新しい書き込みリクエストのみが拒否されます。

厳密な NGSI-LD 仕様準拠を目指すクライアントは、実装間での可搬性のために `type` を `[A-Za-z0-9_]` の範囲内に保つことができます。
...
* `pick` と `omit` は同時に使用できません (`attrs` とも使用不可)
  
* `pick` を使用する場合:`@context` とリストされた Entity メンバーのみが残ります。`id` / `type` / `scope` をリストすることは有効です。これらを省略すると、有効な NGSI-LD Entity ではなくなるペイロードが生成されます (ETSI v1.8.1 新機能により意図的)。
  
* `omit` を使用する場合:リストされた Entity メンバーが削除されます。`id` および `type` を指定した場合も含みます (#2275)。
  
* Retrieve Entity は、プロジェクション後にメンバーが残らない場合 `404 ResourceNotFound` を返します。Query Entities は依然として `200` を返します。
...
* **NGSI-LD** はエンティティ ID を URI として扱い、これは本質的に一意です
  
* **NGSIv2** (標準) は同じ ID でも異なる type を持つエンティティの共存を許可しますが、GeonicDB はこの動作を**サポートしません**
...
* **バッチ upsert** (`POST /ngsi-ld/v1/entityOperations/upsert`): `entityId` のみでエンティティをマッチングします。属性は更新されます (type の扱いは upsert セマンティクスに従います)
  
* **バッチ作成** (`POST /ngsi-ld/v1/entityOperations/create`): 重複 ID に対してエンティティごとのエラー詳細を含む `207` を返します
  
* 同じ ID のエンティティ間で type を区別するための NGSIv2 `?type=` パラメータはもはや適用されません

この統一により、NGSIv2 の type ベースの区別が NGSI-LD の一意 ID モデルと衝突する相互運用性の問題のクラスが排除されます。
...
* どちらの API を使用する場合でも、一貫性と将来の移行のために URN 形式の使用が推奨されます
...
* NGSIv2 から NGSI-LD に移行する場合、エンティティは NGSI-LD API 経由で再作成する必要があります (プロトコル分離により API 間のアクセスは防止されます)
...
**NGSIv2 でクエリすると自動的に NGSI-LD プロバイダーに転送されます:**
...
4. NGSI-LD → 内部フォーマット → NGSIv2 へとレスポンスを変換し、クライアントに返します
...
**NGSI-LD でクエリすると自動的に NGSIv2 プロバイダーに転送されます:**
...
4. NGSIv2 → 内部フォーマット → NGSI-LD へとレスポンスを変換し、クライアントに返します
...
理由: NGSI-LD 仕様に準拠し、両方の API 間での互換性を維持します。
...
理由: 標準モデルタイプにより、エンティティがContext Broker間で相互運用可能になります。GeonicDB は、エンティティの `type` から `@context` を推測**しない**点に注意してください(#1733)。書き込みと読み取りの両方でモデルの `@context` を提供してください(JSON-LD `Link` ヘッダー、または `application/ld+json` 書き込みの場合は本文)。これがない場合、レスポンスは NGSI-LD コアの `@context` のみを使用し、マッピングされていない用語を完全修飾 URI としてレンダリングします(ETSI GS CIM 009 clause 5.5.5 / 5.5.7 に従う)。
...
GeonicDB は NGSIv2 と NGSI-LD API の両方を提供し、厳密なプロトコル分離を行っています。ユースケースに最適な API を選択し、プロトコル間のアクセスが必要な場合は Federation を活用してください。
