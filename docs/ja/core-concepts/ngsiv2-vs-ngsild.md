...
GeonicDB は単一の Context Broker で NGSIv2 と NGSI-LD の両方の API をサポートしています。両方の API は統一された内部ストレージ形式を共有していますが、**エンティティはプロトコルごとに分離されています** -- NGSIv2 経由で作成されたエンティティは NGSIv2 経由でのみアクセス可能で、NGSI-LD も同様です。
...
GeonicDB のデュアル API アーキテクチャは、FIWARE NGSIv2 と ETSI NGSI-LD の両方の仕様をサポートしています。各エンティティは作成したプロトコルでタグ付けされ、2 つの API 間の厳密な分離が保証されます。
...
* 各エンティティは作成時に設定される `protocol` フィールド (`'ngsiv2'` または `'ngsild'`) を持っています
...
* **既存システムとの統合** - NGSIv2 と NGSI-LD ワークロードを干渉することなく並行して実行できます
  
* **API 選択の自由** - 各ユースケースに最適な API を選択できます。プロトコル間のニーズには Federation を使用します
...
### Federation を介したクロスプロトコルアクセス

直接的なクロスプロトコルアクセスはサポートされていません。プロトコル間でエンティティにアクセスする必要がある場合は、**Federation**(Context Source Registration)を使用して、一方の GeonicDB インスタンスを他方のプロトコルのコンテキストプロバイダーとして登録してください。詳細は [Federation](#federation) セクションを参照してください。
...
**注意:** `pick` パラメータを使用する場合、レスポンスには明示的に要求された属性と、`@context`、`id`、`type`(これらは常に存在します)が含まれます。ただし、`createdAt` と `modifiedAt` は、`pick` を使用しても返されません — これらのシステム属性には `sysAttrs` オプションが必要です。
...
> **メディアタイプと表現フォーマット。** これらは独立した軸です。`Accept` ヘッダーは *メディアタイプ* を選択し、`options` / `format` は以下の *表現フォーマット* を選択します。ETSI GS CIM 009 clause 6.3.4 によると、**`Accept` ヘッダーが存在しない場合 (または `*/*` の場合) は `application/json` に解決され**、`application/ld+json` にはなりません — レスポンスボディに `@context` を埋め込みたい場合は明示的に `Accept: application/ld+json` を送信してください (#1734)。`application/json` / `application/ld+json` (および entity エンドポイントでの `application/geo+json`) のみがサポートされており、それ以外は `406 Not Acceptable` を返します。[API\_NGSILD.md — Content Negotiation](../api-reference/ngsild.md#content-negotiation-and-context) を参照してください。
...
> **メタデータ名の文字セット (#1946):** `mq` 条件における属性名とメタデータ名の両方は `\w+` (Orion 互換文法) として解析されるため、メタデータ名は書き込み時に `^[A-Za-z0-9_]+$` に対して検証されます。`.`、`-`、`$`、または空白を含む名前は、クエリで取得できないデータとして保存される代わりに、`400` で拒否されます。[エンティティフィールド文字セット](#entity-field-character-set-id--type--attribute-name--geonicdb-独自拡張) を参照してください。

#### スコープクエリ (scopeQ) の詳細

NGSI-LD の `scopeQ` パラメータは、エンティティスコープ階層に対するクエリをサポートしています。
...
| `/path/#`         | Self and all descendants          | `scopeQ=/Japan/#` (e.g., Tokyo, Tokyo/Shibuya)                  |
| `;`               | AND (conjunction)                 | `scopeQ=/Japan/Tokyo;/IoT`                                      |
| `,` or `\|` | OR (disjunction; both orOp forms) | `scopeQ=/Japan/Tokyo,/IoT` or `scopeQ=/Japan/Tokyo\|/IoT` |

無効な文字セット / 500 文字を超える用語 / 2,000 文字を超える scopeQ 文字列 / 50 を超える OR の選択肢または合計用語 → **400 BadRequestData** (#2583; 書き込み側の `SCOPE_PATH_PATTERN` と同じ文字セット)。
...
#### スコープ文字セット (GeonicDB 独自拡張)

NGSI-LD 仕様 (ETSI GS CIM 009 clause 4.18 / 5.2.x) は、スコープセグメント文字を `[A-Za-z0-9_]` (ALPHA / DIGIT / アンダースコア) に制限しています。GeonicDB はこれを **POSIX Portable Filename Character Set** にわずかに拡張し、スコープがファイルシステムスタイルのパスとして直感的になるようにしています:
...
**なぜ POSIX portable か?** 純粋な NGSI-LD 仕様セット (`[A-Za-z0-9_]`) は実運用では過度に制限的です。POSIX portable は `.` と `-` を追加し、`/com.example.region/tokyo-shibuya` や `/v1.0/sensors` のような自然な名前を可能にします。同時に、`scopeQ` の予約文字 (`;` `+` `#`) と半角スペースは拒否されます — これらはそうでなければ**サイレントな落とし穴**となります: ストレージは成功しますが、エンティティは `scopeQ` でマッチ不可能になります。
...
検証は `POST/PATCH/PUT /entities`、`POST /entityOperations/*`、temporal エンドポイント、および ReactiveCore ルールの `createEntity` アクションに適用されます。既存の保存済みエンティティで非準拠の scope を持つもの (#1189 以前のデータ) は読み取り可能なまま残ります; 新しい書き込みリクエストのみが拒否されます。

厳密な NGSI-LD 仕様準拠を目指すクライアントは、実装間の移植性のために scope を `[A-Za-z0-9_]` 内に保つべきです。

#### Scope 更新セマンティクス (GeonicDB 独自拡張)

NGSI-LD 仕様 (ETSI GS CIM 009) はエンティティの `scope` フィールドと `scopeQ` クエリパラメータを定義していますが、PATCH/PUT/upsert を通じて既存の scope を解除する方法は**指定していません**。GeonicDB は以下のように API を拡張しています:
...
* unset 拡張をサポートするエンドポイント: `PATCH /entities/{id}` (merge-patch)、`PUT /entities/{id}` (replace)、`POST /entityOperations/upsert` (merge と replace の両モード)。attrs レベルのエンドポイント `PATCH /entities/{id}/attrs` は現在 `scope` フィールドを尊重しません (送信された値は静かに無視されます)。scope を更新する必要がある場合は、代わりにエンティティレベルの `PATCH /entities/{id}` を使用してください。
  
* 明示的な unset 状態 (`scope: null`) はフィールドを完全に削除するのではなく、ストレージに保持されます。これにより、API は「scope が意図的にクリアされた」と「scope が設定されたことがない」を区別できます。
  
* `scopeQ` クエリは `scope: null` を持つエンティティにマッチしません (フィールドにマッチするパスコンポーネントがないため)。これは scope を持ったことがないエンティティと一致しています。
  
* 厳密な NGSI-LD 仕様準拠を目指すクライアントは、この拡張に依存することを避けるべきです。

#### エンティティフィールド文字セット (id / type / 属性名) — GeonicDB 独自拡張

NGSIv2 spec の Field syntax restrictions (制御文字 / 空白 / `&` `?` `/` `#` 以外を許容) と NGSI-LD spec はいずれも実装側が追加制約を加えることを許容している。GeonicDB は #1189 で scope に採用した **POSIX Portable Filename Character Set** を `type` にも展開しつつ、attribute name は実装上の制約から旧来の厳格な charset を維持する (#1209):
...
* `id` は `:` を受け入れます。これは NGSI-LD URN 形式 `urn:ngsi-ld:Type:identifier` が正規の id 形式であるためです。
  
* **NGSI-LD path** は POSIX portable 短縮名に加えて絶対 IRI を `type` として受け入れます (#1211)。例: `https://uri.fiware.org/ns/data-models#WeatherObserved`、`urn:ngsi-ld:Type:Sensor`。**NGSIv2 path** は引き続き URI 形式を拒否します (プロトコル分離)。
...
* エラーレスポンスには、問題のある値と許可されている文字セットが含まれます。例:`Entity type contains invalid characters (allowed: A-Z a-z 0-9 . _ - (must not start with -)); got "Sensor@Type"`
...
検証は NGSIv2 の `POST/PATCH/PUT /v2/entities` と NGSI-LD の `POST/PATCH/PUT /ngsi-ld/v1/entities`、temporal エンドポイント、およびバッチ操作に適用されます。#1209 以前のデータなど、不適合フィールドを持つ既存の保存済みエンティティは読み取り可能なまま残ります。拒否されるのは新しい書き込みリクエストのみです。

厳密な NGSI-LD 仕様準拠を目指すクライアントは、実装間の移植性のために `type` を `[A-Za-z0-9_]` の範囲内に保つことができます。
...
> **注:** プロトコル分離のため、NGSI-LD エンティティ(Relationship 属性を持つものを含む)は NGSIv2 API 経由ではアクセスできません。
...
> **注:** プロトコル分離のため、NGSI-LD エンティティ(LanguageProperty 属性を持つものを含む)は NGSIv2 API 経由ではアクセスできません。
...
* `servicePath` は `?attrs=servicePath` 経由で組み込み属性として利用できます
...
* **servicePath と scope は独立した概念です(#964):** これらは自動的には同期されません
  ...
  * NGSI-LD は ETSI GS CIM 009 仕様に従って `Fiware-ServicePath` ヘッダーを無視します
...
* `pick` と `omit` は同時に使用できません (`attrs` とも併用不可)
  
* `pick` を使用する場合:`@context` とリストされた Entity メンバーのみが残ります。`id` / `type` / `scope` をリストすることは有効です。これらを省略すると、有効な NGSI-LD Entity ではなくなるペイロードが生成されます (ETSI v1.8.1 New Features で意図的に定義されています)。
  
* `omit` を使用する場合:リストされた Entity メンバーが削除されます。これには `id` と `type` が含まれる場合もあります (#2275)。
  
* Retrieve Entity は、プロジェクション後にメンバーが残らない場合、`404 ResourceNotFound` を返します。Query Entities は引き続き `200` を返します。
...
これは両方の API で ID のセマンティクスを統一するための意図的な設計決定です:


* **NGSI-LD** はエンティティ ID を URI として扱い、本質的に一意です
  
* **NGSIv2** (標準) は同じ ID で異なる type を持つエンティティの共存を許可しますが、GeonicDB はこの動作を**サポートしません**

**影響:**

* **直接作成** (`POST /v2/entities`、`POST /ngsi-ld/v1/entities`): 既存のエンティティと同じ ID を持つエンティティを作成すると (異なる `type` でも)、`409 AlreadyExists` が返されます
  
* **バッチ更新** (`POST /v2/op/update` で `append`/`appendStrict` を使用): `entityId` のみでエンティティを照合します。属性は更新されますが、元の `type` は保持されます
  
* **バッチアップサート** (`POST /ngsi-ld/v1/entityOperations/upsert`): `entityId` のみでエンティティを照合します。属性は更新されます (type の処理はアップサートのセマンティクスに従います)
  
* **バッチ作成** (`POST /ngsi-ld/v1/entityOperations/create`): 重複した ID に対してエンティティごとのエラー詳細を含む `207` を返します
  
* 同じ ID を持つエンティティ間での type の曖昧さを解消するための NGSIv2 の `?type=` パラメータは適用されなくなりました

この統一により、NGSIv2 の type ベースの曖昧さ解消が NGSI-LD の一意な ID モデルと競合する相互運用性の問題のクラスが排除されます。
...
* NGSIv2 は任意の文字列を ID として使用することを許可します (例: `Room1`、`sensor-abc`)
  
* 一貫性と将来の移行のために、どの API を使用する場合でも URN 形式の使用が推奨されます
...
* NGSIv2 から NGSI-LD に移行する場合、エンティティは NGSI-LD API を介して再作成する必要があります (プロトコルの分離により API 間アクセスが禁止されています)
...
登録されたリモートプロバイダについて、GeonicDB は次の順序でプロトコルを検出します:
...
4. NGSI-LD → 内部フォーマット → NGSIv2 とレスポンスを変換し、クライアントに返します
...
4. NGSIv2 → 内部フォーマット → NGSI-LD とレスポンスを変換し、クライアントに返します
...
GeonicDB は両方の API を同時にサポートしますが、エンティティはプロトコル分離されています。各 API は独自のエンティティセットに対して独立して動作します。
...
3. **マイグレーションには再作成が必要** - NGSIv2 から NGSI-LD へエンティティをマイグレーションするには、NGSIv2 API からエクスポートし、NGSI-LD API 経由で再作成してください。自動的なクロスプロトコルマイグレーションはありません
...
理由: 標準的なモデルタイプにより、Context Broker間でエンティティの相互運用性が保たれます。GeonicDB はエンティティの `type` から `@context` を推測**しない**ことに注意してください (#1733) — 書き込みと読み込みの両方でモデルの `@context` を提供してください (JSON-LD `Link` ヘッダー、または `application/ld+json` 書き込みの場合は本文)。`@context` がない場合、レスポンスは NGSI-LD コアの `@context` のみを使用し、マッピングされていない用語は完全修飾 URI としてレンダリングされます (ETSI GS CIM 009 clause 5.5.5 / 5.5.7 に準拠)。
...
GeonicDB は NGSIv2 と NGSI-LD API の両方を提供しており、厳格なプロトコル分離が行われています。ユースケースに最適な API を選択し、プロトコル間アクセスが必要な場合は Federation を活用してください。
