...
GeonicDB は、単一の Context Broker で NGSIv2 と NGSI-LD の両方の API をサポートします。両 API は統一された内部ストレージフォーマットを共有しますが、**エンティティはプロトコルごとに分離されています** -- NGSIv2 で作成されたエンティティは NGSIv2 経由でのみアクセス可能で、NGSI-LD も同様です。
...
GeonicDB のデュアル API アーキテクチャは、FIWARE NGSIv2 と ETSI NGSI-LD の両方の仕様をサポートします。各エンティティは作成時に使用されたプロトコルでタグ付けされ、2 つの API 間の厳密な分離を保証します。
...
- **API 選択の自由** - 各ユースケースに最適な API を選択でき、クロスプロトコルのニーズにはフェデレーションを使用します

---

## 統合内部フォーマット

GeonicDB は両方の API からのデータを統合された内部フォーマットに変換します。
...
プロトコル分離の導入前に作成されたエンティティ(つまり、データベースに `protocol` フィールドがないもの)は `'ngsild'` として扱われます。これらは NGSI-LD API を介してのみアクセス可能です。
...
| `geo:point` | `GeoJSON` (Point) | `GeoProperty` | 緯度/経度座標 |
...
| 属性名 | 型 | 説明 |
|----------------|------|-------------|
| `dateCreated` | `DateTime` | エンティティ作成のタイムスタンプ (ISO 8601) |
| `dateModified` | `DateTime` | エンティティ最終更新のタイムスタンプ (ISO 8601) |
...
| 属性名 | 型 | 説明 |
|----------------|------|-------------|
| `createdAt` | ISO 8601 文字列 | エンティティ作成のタイムスタンプ |
| `modifiedAt` | ISO 8601 文字列 | エンティティ最終更新のタイムスタンプ |

**注意:** `pick` パラメータを使用する場合、レスポンスには明示的に要求された属性と、`@context`、`id`、`type` (常に存在) が含まれます。ただし、`createdAt` および `modifiedAt` は、`pick` を使用しても返されません。これらのシステム属性には `sysAttrs` オプションが必要です。
...
| 形式 | options パラメータ | 説明 |
|--------|-------------------|-------------|
| **normalized** (デフォルト) | (なし) | 型とメタデータを含む完全形式 |
...
| 形式 | Accept ヘッダー | 説明 |
|--------|---------------|-------------|
| **normalized** (デフォルト) | `application/ld+json` | 型とメタデータを含む完全形式 |
| **concise** | `application/ld+json` + `options=concise` | 簡潔形式 (省略記法) |
...
| `..` | 範囲(両端含む) | `mq=temperature.accuracy==0.9..1.0` |
...
### 5. Federation (コンテキストソース登録)

| 機能 | NGSIv2 | NGSI-LD | 説明 |
|---------|--------|---------|-------------|
| **Registration API** | `/v2/registrations` | `/ngsi-ld/v1/csourceRegistrations` | リモートプロバイダー登録 |
...
- NGSIv2 は階層的なエンティティ管理に `Fiware-ServicePath` ヘッダーを使用
- `servicePath` は `?attrs=servicePath` 経由で組み込み属性として利用可能
- **servicePath と scope は独立した概念です (#964):** 自動的には同期されません
  - NGSIv2 `Fiware-ServicePath` → DB では `servicePath` として保存 (インフラストラクチャレベルの分離)
  - NGSI-LD `scope` → DB では `scope` として保存 (ユーザー定義の論理階層)
  - NGSI-LD は ETSI GS CIM 009 仕様に従い `Fiware-ServicePath` ヘッダーを無視

### 4. 属性のプロジェクション (pick / omit パラメータ)
...
- `omit` を使用する場合: 指定された属性を除くすべての属性が含まれます。`id` と `type` は除外できません (ETSI GS CIM 009 V1.9.1 仕様に準拠)
...
これは、両方の API で ID セマンティクスを統一する意図的な設計決定です:

- **NGSI-LD** はエンティティ ID を URI として扱い、本質的に一意です
- **NGSIv2** (標準) では同じ ID で異なるタイプのエンティティが共存できますが、GeonicDB はこの動作を**サポートしません**

**影響:**
- **直接作成** (`POST /v2/entities`、`POST /ngsi-ld/v1/entities`): 既存のエンティティと同じ ID でエンティティを作成する場合 (異なる `type` であっても)、`409 AlreadyExists` が返されます
- **バッチ更新** (`POST /v2/op/update` with `append`/`appendStrict`): `entityId` のみでエンティティを照合します。属性は更新されますが、元の `type` は保持されます
- **バッチアップサート** (`POST /ngsi-ld/v1/entityOperations/upsert`): `entityId` のみでエンティティを照合します。属性は更新されます (タイプの処理はアップサートセマンティクスに従います)
- **バッチ作成** (`POST /ngsi-ld/v1/entityOperations/create`): 重複する ID に対してエンティティごとのエラー詳細を含む `207` を返します
- 同一 ID のエンティティ間のタイプによる曖昧性解消のための NGSIv2 `?type=` パラメータは適用されなくなりました

この統一により、NGSIv2 のタイプベースの曖昧性解消が NGSI-LD の一意 ID モデルと競合する相互運用性の問題が解消されます。
...
- 一貫性と将来の移行のために、どの API を使用する場合でも URN 形式の使用を推奨します
...
**NGSIv2 でクエリすると、NGSI-LD プロバイダーへ自動的に転送されます：**
...
3. NGSI-LD プロトコルを使用してクエリを転送します：`GET /ngsi-ld/v1/entities/urn:ngsi-ld:Vehicle:V999`4. NGSI-LD → 内部形式 → NGSIv2 とレスポンスを変換してクライアントに返します
...
**NGSI-LD でクエリすると、NGSIv2 プロバイダーへ自動的に転送されます：**
...
3. NGSIv2 プロトコルを使用してクエリを転送します：`GET /v2/entities/urn:ngsi-ld:Sensor:S888`4. NGSIv2 → 内部形式 → NGSI-LD とレスポンスを変換してクライアントに返します
...
3. **移行には再作成が必要** - エンティティを NGSIv2 から NGSI-LD に移行するには、NGSIv2 API からエクスポートし、NGSI-LD API 経由で再作成してください。自動的なプロトコル間移行はありません
...
理由: 開発環境と本番環境の分離、顧客ごとのデータ分離を可能にします。
...
| **エンティティ分離** | `protocol: 'ngsiv2'` | `protocol: 'ngsild'` | 各 API は自身のエンティティのみ参照 |
| **エンティティ ID** | 任意の文字列 | URI (URN 推奨) | URN 推奨。**ID はテナント + servicePath ごとに一意** (型による曖昧性解消は廃止) |
| **属性タイプ** | シンプル (Number, Text など) | セマンティック (Property, Relationship など) | 型マッピングルールで対応関係を定義 (上記表を参照) |
| **システム属性** | `dateCreated`, `dateModified` | `createdAt`, `modifiedAt` | 内部的に統合、API ごとに変換 |
| **ジオクエリ** | ✅ | ✅ | 共有機能 |
...
GeonicDB は厳格なプロトコル分離により NGSIv2 と NGSI-LD の両方の API を提供します。ユースケースに最適な API を選択し、プロトコル間アクセスが必要な場合はフェデレーションを活用してください。
