...
GeonicDB は複数の AI 指向インターフェースを提供しており、AI エージェント (Claude、GPT-4、Gemini など) が API を簡単に利用できます。
...
MCP `temporal` は、ETSI GS CIM 009 clause 4.5.9 の簡易時系列表現 (`{ type, values: [...] }`) を反映したマルチインスタンス短縮記法を受け入れます:
...
`batch_create` / `batch_upsert` / `batch_delete` は HTTP 時系列バッチエンドポイントとサービスを共有しており、その **DB タイムアウトアボート** (GeonicDB 拡張) を含みます:要素の処理中にデータベースクエリタイムアウト (`maxTimeMS` 超過、`ServiceUnavailable`) が発生すると、残りの**未試行**要素はアボートされ、`errors` 配列に次のように返されます
...
`Not attempted:` プレフィックスは、実際にタイムアウトした要素 (ドライバー由来のメッセージを持つ) と未試行の要素を区別するものです。`success` にリストされたエンティティはコミットされます。**`errors` にリストされた entityId のみを再試行してください**。`batch_create` / `batch_upsert` の場合、時系列書き込みは追加専用であるため、既に成功したエンティティを再送信すると時系列インスタンスが重複します。`batch_delete` の場合、既に削除されたエンティティを再送信すると、単に見つからないと報告されます (データは重複しません)。HTTP API は同じ分割を `207 Multi-Status` として返し、ProblemDetails 型のエラー (`type` は `https://uri.etsi.org/ngsi-ld/errors/ServiceUnavailable`、同じ `detail` テキスト) を含みます。

### 時系列 `modify_instance` と属性タイプ (#2311)

HTTP `PATCH /ngsi-ld/v1/temporal/entities/{entityId}/attrs/{attrName}/{instanceId}` (ETSI GS CIM 009 clause 5.6.14) は本文の `type` を受け入れ、属性インスタンスのタイプを置き換えます (`input.type ?? oldDoc.type`)。MCP `temporal` ツールはそのフィールドを `type` ではなく **`attrType`** として公開します:
...
`modify_instance` に `type: "FuelProbe"` を渡すと、属性タイプがエンティティタイプ名に書き換えられては**なりません**。既存の属性タイプを維持する(部分更新)には `attrType` を省略してください。

A2A はこの操作を実装していません。

MCP/A2A を共有表現レイヤー経由でルーティングすることの影響 (**以前の生出力に依存していたクライアントにとっては破壊的変更**):


* システム時系列属性 (`createdAt` / `modifiedAt` / `expiresAt`) は、`options=sysAttrs` がリクエストされた**場合のみ**返されます (clause 6.3.11)。以前は無条件で返されていました。
  
* 内部的な `attrNameForm` マーカーは出力されなくなりました — これは API 契約の一部ではありませんでした。
  
* `instanceId` は**引き続き**返されます:これは clause 4.5.7 で定義された表現メンバーであり、modify/delete インスタンス操作 (clause 5.6.14) で必要です。
...
HTTP、MCP `temporal.get` / `query` / `batch_query`、および A2A `temporal.get` / `query` / `list` はすべて、`timerel` / `timeAt` / `endTimeAt` / `timeproperty` / `lastN` / `attrs` / `format` / `options` / `aggrMethods` / `aggrPeriodDuration` を同じリゾルバー経由でルーティングするため、いずれも黙って無視されることはありません。A2A クライアントにとって注目すべき 2 つの影響があります:


* **存在しないエンティティに対する `temporal.get` は明確なエラーになりました** (`Entity not found: <id>`) — 空の `entities` 配列を伴う `200` の代わりに。「見つからない」と「読み取れない」は以前は区別できませんでした。
  
* **切り捨てが報告されます。** デフォルトの `lastN` 上限が属性の履歴を切り捨てた場合、HTTP API は `NGSILD-Warning: 199 …` を設定します。MCP と A2A にはヘッダーがないため、同じテキストがレスポンスオブジェクトの `warnings` 配列に返されます。その不在は何も切り捨てられなかったことを意味します。

`sysAttrs` は A2A でブール値として機能し続け、`options: "sysAttrs"` (ETSI GS CIM 009 clause 6.3.11) も同様に受け入れられます。
...
属性は**ユーザー定義サブ属性** (Property of Property / Relationship of Property、ETSI GS CIM 009 clause 4.5) と**予約メンバー** `observedAt`、`unitCode`、`datasetId`、および `valueType` を持つことができます。**MCP / A2A `entities` (およびバッチ) 書き込み**は、HTTP NGSI-LD API とまったく同じルールでこれらを永続化します:
...
* **例外 — MCP `temporal` 書き込み (#2722):** `attrMetadata` 経由の `unitCode` / その他の予約メタデータは、MCP 時系列 (単一属性パスまたは `values[]` インスタンス) では**配線されていません**。各**インスタンス**の `datasetId` / `observedAt` は機能します。`values[]` エンベロープ上の親キーは拒否されます (#2717)。上記の例が `temporal` ツールに適用されると仮定しないでください。
...
* **「HTTP と同じルール」の例外であった名前が 1 つありました:`__proto__`。** これは `^[A-Za-z0-9_]+$` にマッチします。#2059 以降、MCP と A2A はこれを自身のプロパティとして永続化し、#2100 以降、HTTP NGSI-LD 書き込みパスも `validateNgsiLdBody` を介して同じことを行います (そうしないと Zod のオブジェクト再構築がキーを黙ってドロップしてしまいます)。
  
* 1 レベルのネストが保持されます。サブ属性自身の `observedAt` とより深いネストは保持されません。
  
* サブ属性は、MCP と A2A の読み取りが返す簡略化された (keyValues) 表現には表示されません。それらを読み戻すには、正規化表現で HTTP NGSI-LD API を使用してください。
...
> **重要**: API キー (`gdb_` で始まる文字列) は作成時に一度だけ表示されます。安全に保管してください。`--save` フラグを使用すると、自動使用のためにキーが CLI 設定に保存されます。

API キーは XACML 認可モデルの下でデフォルトで全拒否です。以下の 2 つの方法のいずれかでアクセスを許可します:
...
> **注意**: `env` の値は実際の API キーとテナント名に置き換えてください。環境変数を使用することで、認証情報を `args` 配列から除外できます。
...
Streamable HTTP に対応した任意の MCP クライアントは直接接続できます:
...
* **認証が有効の場合**: 省略すると、ログインしているユーザーのテナントがデフォルトとして使用されます。`super_admin` はデータツールを使用できません (403 を返します)。代わりに `tenant_admin` または `user` ロールを使用してください。ただし、`tenant_admin`/`user` は自分自身のテナントにのみアクセスできます。これは認証されていない (匿名) 呼び出し元にも適用されます (#2186): `tenant` は省略するか、呼び出し元自身のテナント (`Fiware-Service` ヘッダーから取得) と一致させる必要があり、異なるテナントを指定すると拒否されます (`Access denied`)。`tenant` を省略する (または呼び出し元自身のテナントを指定する) ことが許可されるのは、そのテナントが実際に `Fiware-Service` から解決できる場合のみです。テナントコンテキストを持たない匿名呼び出し元、つまりヘッダーが存在しないテナントを指定している場合も、`Access denied` で拒否され、`default` テナントへのフォールバックは**行われません**。
...
**例外 — `config` ツールのルール操作**: ReactiveCore Rules は `servicePath` を第一級フィールドとして使用するため、`config` ツールはルールの `list` (オプションのフィルターとして) および `create` 操作でこれを受け入れます (他のルール操作では無視されます)。値はトリミングされ、空文字列は `/` にデフォルト設定され、結果は `/^\/[\w/]*$/` に一致する単一の正確なパス (例: `/sensors`) でなければなりません。階層的な `/#` やカンマ区切りの複数パス値は、ルールが正確な `servicePath` の等価性でマッチングするため拒否されます (#1607/#1608)。

### NGSI-LD クエリパラメータ

`entities` ツールは NGSI-LD クエリパラメータの全セットをサポートします:
...
`batch` ツールの `query` アクションは `orderBy`、`orderDirection`、`sysAttrs`、`typePattern`、`local` もサポートします。`temporal` ツールの `query` および `batch_query` アクションは `typePattern` (AND として `type` と組み合わせ) もサポートします。`batch` ツールの `purge` は `typePattern` を受け付けず、黙って無視されるのではなく拒否されます。これは HTTP `DELETE /ngsi-ld/v1/entities` スコープと一致します。

**A2A スキルは同じ `typePattern` を受け付けます (#2156)。** `entities` (`list`、`search_by_location`、`search_by_attribute`)、`batch` (`query`)、および `temporal` (`query` / `list`) は同じセマンティクス (逐語的な正規表現、`type` との AND) でこれを適用します。`batch` の `purge` はこれを拒否し、HTTP `POST /ngsi-ld/v1/entityOperations/purge` もリクエストボディに配置された場合に拒否します。そこで黙って無視すると、呼び出し元が要求したよりも**広い**セットを削除することになるためです。
...
デフォルトが異なる理由:`local` は HTTP バインディング用に定義されており、MCP / A2A はそのバインディングではないため、ローカルのみのデフォルトはコンプライアンスの逸脱ではありません。両方とも**同期リクエスト/レスポンス**のサーフェスであり、転送はプロバイダーのラウンドトリップを追加するため、AI クライアントは簡単にタイムアウトに達する可能性があります。転送を明示的にすることで、エージェントが要求していない分散オペレーションをトリガーすることを防ぎます。
...
転送できないアクションに `local` を渡すこと、または異なるメカニズム(#2008 / #2036)を通じて登録宣言された名前をマージする `types` / `attributes` リソースに渡すこと(MCP は配線していません)は、静かに無視されるのではなく**拒否**されるため、クライアントは転送していないのに転送したと信じることはありません。

承認は転送によって変更されません:呼び出し元のポリシー(#2003)から派生した行レベルの読み取り述語は、ローカルエンティティと同様にリモートエンティティにも適用されるため、特定のエンティティタイプ / スコープ / 所有者に制限されたプリンシパルは、ローカルで読み取れない Context Source エンティティを見ることはありません。

**ID による取得(`entities` / `get`)は今のところローカルのみです。** リモートのみのエンティティはローカルの承認コンテキストを持たないため、`entityType` 条件付きの `Deny` は HTTP の ID 別パスでそれらを見逃していました。#2092 は、`FederationService.getEntity` 内のリモートエンティティに同じ行レベルの読み取り述語(#2003)を適用することでこれを解決しました。MCP / A2A からの ID による転送は、承認の穴でブロックされなくなりましたが、未実装のままです。これを追加するには、HTTP コントローラーが使用するのと同じ `resolveReadableEntityFilter` の配線が必要です。Temporal 読み取りパスには、`docs/API_NGSILD.md` の既存の注記と一致して、フェデレーションの配線がまったくありません。
...
* **認証**: 認証が有効になっている場合(デフォルト)、Bearer トークンが必要です。明示的な `AUTH_ENABLED=false` では、認証なしで動作します。
...
* **分散オペレーションはオプトインです**: HTTP API とは異なり、MCP / A2A は `local="false"` が渡されない限り Context Sources に転送せず、上記の(#2072)クエリアクションのみに適用されます。
  
* **レート制限**: MCP エンドポイントは、REST API と同じレート制限、ストレージクォータ、およびリクエストボディサイズ制限の対象となります。

## JSON Schema とカスタムデータモデル

カスタムデータモデルは、作成時に自動的に JSON Schema (Draft 2020-12) が生成されます。この JSON Schema は、以下の目的で AI ツールに活用できます。

**`additionalProperties` フィールド**: エンティティが `propertyDetails` で定義されていない属性を持つことができるかどうかを制御します。デフォルトは `true` (NGSI-LD セマンティクスに従い、任意の追加属性を許可) です。`false` に設定すると厳格な検証が強制され、定義された属性のみが受け入れられます。AI エージェントは、エンティティを作成する際にこのフィールドをチェックして、追加属性が許可されているかどうかを判断する必要があります。
...
複合制約は正確なタプルに一致するため、重複する**値**を拒否しますが、重複する**範囲**を拒否することはありません。上記の例では、同じ部屋で同じ時刻に開始する 2 番目の予約は拒否されますが、後で開始してそれと重複するもの (10:00-11:00 vs 10:30-11:30) は拒否されません。制約が範囲全体をカバーするようにするには、予約を共有の固定長グリッド (例: 時刻または 30 分区切りで開始する 30 分スロット) に整列させ、予約の `[start, end)` が重複するグリッドスロットごとに 1 つのエンティティを保存します (開始スロットだけでなく)。その後、それらのスロットを共有 id 属性でグループ化します。これは、すべての予約が同じグリッドとスロットルールに対して生成された場合にのみ衝突を保証します。グリッドに整列していない間隔、または一貫性のないスロットセット (例: 開始スロットのみ) で生成された場合、実際の重複を見逃したり、実際には重複しないスロットで衝突したりする可能性があります。任意の、グリッド化されていない間隔の重複には、常に別のアプリケーションレベルのチェックが必要です。提供しない保証にちなんで制約に名前を付けること (開始時刻のみのタプルに対する `no-double-booking` など) は、エージェントや開発者が独自の重複チェックをスキップする原因となります。
...
自動生成された語彙 IRI は **このContext Broker自身のベース URL** (#1984) 上に存在し、`GET /vocab/{tenantId}/{term}` を介して参照解決可能です。ホスト名ごとのデータベースへのルーティング要求 (#1291) では、ベース URL は **そのデプロイメントの登録されたホスト名** (#3190) になります。これは、そのホストだけがコンテキストを保持するデータベースを提供するためです。それ以外の場合は、`API_BASE_URL` 環境変数から取得されます。この変数は SAM テンプレートパラメータ **`ApiBaseUrl`** (`infrastructure/template.yaml`) からデプロイ時に注入されます。設定されていない場合はリクエストの `Host` ヘッダーにフォールバックします。IRI をホスト名間で安定させるために `ApiBaseUrl` を設定してください。詳細は [API.md → Broker base URL resolution](../api-reference/endpoints.md#broker-base-url-resolution) を参照してください。

プロパティ URI はエンティティタイプに依存しません。同じプロパティ名(例: `email`)は、同じテナント内の異なるエンティティタイプ間で同じ URI を共有します。

### @context 解決 (#1733)

NGSI-LD レスポンスをレンダリングするために使用される `@context` は、リクエストが提供したもの **のみ** です。提供されない場合は、NGSI-LD コア `@context` のみが使用され、それでコンパクト化できない用語は完全修飾 URI としてレンダリングされます(ETSI GS CIM 009 clause 5.5.5 / 5.5.7)。
...
MCP `config` ツールを介して作成または更新されたカスタムデータモデルは、HTTP 経由で作成されたものと **同じ自動生成 `@context`** を取得します(#1986):`contextUrl` が省略された場合、JSON-LD コンテキストが `propertyDetails` から生成され、`/ngsi-ld/v1/jsonldContexts/` の下に登録され、その URL が `contextUrl` として保存されます。Context Broker自身の公開 URL は、**HTTP コントローラーと同じ優先順位** (#2075 / #3190) で解決されます:登録されたデプロイメントホスト名(ホスト名ベースのルーティングがホスト名ごとのデータベースに到達した場合)→ `API_BASE_URL` → MCP リクエスト自身の `Host` ヘッダー → ローカルサーバーアドレス。MCP ツールハンドラー自体は HTTP イベントを受信しませんが、サーバーはリクエストごとに再構築されます(`POST /mcp` 自体が HTTP 呼び出しです)。そのため、そのリクエストのイベントがイベントなしで解決されるのではなく、`config` ツールにスレッド化されます。`API_BASE_URL` を未設定のままにしているデプロイメント(例: SSM パラメータが欠落している環境)は、常に `http://localhost:3000` にフォールバックする代わりに、リクエスト自身の `Host` から構築された `contextUrl` を取得するようになりました。
...
1. 開発者が SDK をインストールする: `npm install @geolonia/geonicdb-sdk`
   
2. 開発者が SDK をインポートする: `import GeonicDB from '@geolonia/geonicdb-sdk'`
   
3. AI がパッケージから TypeScript 宣言を読み込む
   
4. AI が文書化された API を使用して正しいコードを生成する

個別のドキュメント URL や特別な設定は不要です。TypeScript プロジェクトでは、完全な型チェックと IDE のオートコンプリートをすぐに利用できます。詳細は SDK ドキュメントを参照してください。
...
* **DPoP**: `Authorization: DPoP <token>` + `DPoP` プルーフヘッダー（有効化されている場合）
...
* **A2A** はエージェント間通信用 — AI エージェントが GeonicDB をピアエージェントとして協調動作します
