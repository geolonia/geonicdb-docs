...
GeonicDB は複数の AI 指向インターフェースを提供し、AI エージェント (Claude、GPT-4、Gemini など) が簡単に API を利用できるようにします。
...
各ツールは `action` と `resource` パラメータを介してその操作を選択します。
...
### 時間的表現パラメータ (#2032 / #2033)

`temporal` ツールは HTTP Temporal API と同じ表現パラメータを受け入れ、そのレスポンスは HTTP と**同じ表現レイヤー**を通過します (#2033):
...
MCP `temporal` は、ETSI GS CIM 009 clause 4.5.9 の簡略化された時間的表現 (`{ type, values: [...] }`) を反映するマルチインスタンス短縮形式を受け入れます:
...
* 親オブジェクトキー:**`type` (オプション) と `values` (必須) のみ**。その他の親キー (`observedAt`、`datasetId`、`unitCode`、…) は、実行可能なメッセージとともに **400** を返します (#2717)。
  
* `observedAt` / `datasetId` は親ではなく、**各インスタンス**に配置してください。
  
* `unitCode` / その他の予約されたメタデータは、MCP temporal 書き込みパス (単一属性またはインスタンス) では**まだ配線されていません**。#2722 で追跡されています。

### MCP `temporal` バッチアクションと DB タイムアウト中止 (#2542)

`batch_create` / `batch_upsert` / `batch_delete` は、HTTP temporal バッチエンドポイントとそのサービスを共有し、その **DB タイムアウト中止** (GeonicDB 拡張) を含みます:要素の処理中にデータベースクエリタイムアウト (`maxTimeMS` 超過、`ServiceUnavailable`) が発生すると、残りの**未試行**要素は中止され、`errors` 配列に次のように返されます
...
`Not attempted:` プレフィックスは、未試行の要素と実際にタイムアウトした要素 (ドライバー由来のメッセージを持つ) を区別するものです。`success` にリストされているエンティティはコミットされています。**`errors` にリストされている entityIds のみを再試行してください**。`batch_create` / `batch_upsert` の場合、temporal 書き込みは追加専用であるため、すでに成功したエンティティを再送信すると、それらの temporal インスタンスが重複します。`batch_delete` の場合、すでに削除されたエンティティを再送信すると、単に見つからないと報告されます (データは重複しません)。HTTP API は、ProblemDetails 型のエラー (`type` `https://uri.etsi.org/ngsi-ld/errors/ServiceUnavailable`、同じ `detail` テキスト) を持つ `207 Multi-Status` として同じ分割を返します。

### Temporal `modify_instance` と属性タイプ (#2311)

HTTP `PATCH /ngsi-ld/v1/temporal/entities/{entityId}/attrs/{attrName}/{instanceId}` (ETSI GS CIM 009 clause 5.6.14) はボディ `type` を受け入れ、属性インスタンスタイプ (`input.type ?? oldDoc.type`) を置き換えます。MCP `temporal` ツールはそのフィールドを `type` ではなく **`attrType`** として公開しています:
...
`type: "FuelProbe"` を `modify_instance` に渡した場合、属性タイプをエンティティタイプ名に書き換えては**いけません**。既存の属性タイプを保持する(部分更新)には、`attrType` を省略してください。

A2A はこの操作を実装していません。

MCP/A2A を共有表現レイヤー経由でルーティングすることの影響(**以前の生出力に依存していたクライアントにとって破壊的変更**):


* システム temporal 属性 (`createdAt` / `modifiedAt` / `expiresAt`) は `options=sysAttrs` がリクエストされた場合に**のみ**返されるようになりました (clause 6.3.11)。以前は無条件に返されていました。
  
* 内部的な `attrNameForm` マーカーは出力されなくなりました — これは API 契約の一部ではありませんでした。
  
* `instanceId` は**引き続き**返されます: これは clause 4.5.7 で定義されている表現メンバーであり、modify/delete instance 操作 (clause 5.6.14) で必要とされます。
  
* 属性名とタイプ名は clause 5.5.7 の圧縮を経ます。MCP/A2A はリクエスト `@context` を提供しないため、絶対 IRI として保存された名前は完全修飾 URI としてレンダリングされます (clause 5.5.7 のフォールバック)。これは HTTP がコアのみのリクエスト `@context` に対して行うのとまったく同じです。

A2A `temporal` スキルは `get` / `query` / `list` アクションで同じ形状を返します。

**Temporal リクエストパラメータはすべてのエントリーポイントで同一に解釈されます (#2099 / #2156)。** HTTP、MCP `temporal.get` / `query` / `batch_query`、および A2A `temporal.get` / `query` / `list` はすべて `timerel` / `timeAt` / `endTimeAt` / `timeproperty` / `lastN` / `attrs` / `format` / `options` / `aggrMethods` / `aggrPeriodDuration` を同じリゾルバー経由でルーティングするため、どれも暗黙的に無視されることはありません。A2A クライアントに対して特筆すべき 2 つの影響:


* **存在しないエンティティに対する `temporal.get` は明示的なエラーになりました** (`Entity not found: <id>`) 空の `entities` 配列を持つ `200` ではなく — 「見つからない」と「読み取り不可」は以前は区別できませんでした。
  
* **切り捨てが報告されます。** デフォルトの `lastN` キャップが属性の履歴を切り捨てた場合、HTTP API は `NGSILD-Warning: 199 …` を設定します; MCP と A2A にはヘッダーがないため、同じテキストがレスポンスオブジェクトの `warnings` 配列で返されます。その不在は何も切り捨てられなかったことを意味します。

`sysAttrs` は A2A 上で引き続き boolean として機能し、`options: "sysAttrs"` (ETSI GS CIM 009 clause 6.3.11) も同様に受け入れられます。
...
属性は**ユーザー定義サブ属性** (Property of Property / Relationship of Property、ETSI GS CIM 009 clause 4.5) と**予約済みメンバー** `observedAt`、`unitCode`、`datasetId`、`valueType` を持つことができます。**MCP / A2A `entities` (およびバッチ) 書き込み**は、HTTP NGSI-LD API とまったく同じルールでこれらを永続化します:
...
* **例外 — MCP `temporal` 書き込み (#2722):** `attrMetadata` 経由の `unitCode` / その他の予約済みメタデータは MCP temporal (単一属性パスまたは `values[]` インスタンス) では**配線されていません**。各**インスタンス**の `datasetId` / `observedAt` は機能します; `values[]` エンベロープ上の親キーは拒否されます (#2717)。上記の例が `temporal` ツールに適用されると仮定しないでください。
  
* `unitCode` と `valueType` は `Property` にのみ受け入れられます (entities / HTTP)。他の属性タイプ (`Relationship`、`ListRelationship`、`LanguageProperty`、`VocabProperty`、`JsonProperty`、`GeoProperty`、`ListProperty`、`TemporalProperty`) はそれらを持たず (clause 4.5.2 / 4.5.3.3 / 4.5.18.3 / 4.5.20.3 / 4.5.22.3 / 4.5.24.3)、ドロップされます。
  
* サブ属性名は属性名と同じルールに従います: 短い名前 (`^[A-Za-z0-9_]+$`) または絶対 IRI。それ以外 (例えば `unit.code`) は拒否されます。NGSI-LD コアボキャブラリー内の絶対 IRI は正規形式で保存されるため、`https://uri.etsi.org/ngsi-ld/default-context/accuracy` は `accuracy` として保存され、読み取られます (clause 5.5.7)。
  
* **かつて「HTTP と同じルール」の例外だった名前が 1 つあります: `__proto__`。** これは `^[A-Za-z0-9_]+$` にマッチします。#2059 以降、MCP と A2A はこれを独自プロパティとして永続化し、#2100 以降は HTTP NGSI-LD 書き込みパスも `validateNgsiLdBody` 経由で同様に処理します (そうしないと Zod のオブジェクト再構築がキーを暗黙的にドロップしてしまいます)。
  
* 1 レベルのネストは保持されます。サブ属性自身の `observedAt` とより深いネストは保持されません。
  
* サブ属性は MCP と A2A の読み取りが返す簡略化された (keyValues) 表現には現れません。それらを読み取るには、正規化された表現を持つ HTTP NGSI-LD API を使用してください。
...
**`auth` に関する注意**: ai-plugin.json の `auth` ブロックは、機械可読な形式で単一のスキームのみを表現できます — ここでは Bearer (`authorization_type: "bearer"`) です。X-Api-Key と OAuth 2.0 client credentials の代替手段は、人間が読める `instructions` テキスト内でのみ言及されています。サポートされているすべてのスキームの完全な機械可読定義については、`/openapi.json` の `securitySchemes` (`BearerAuth`、`ApiKeyAuth`、`DPoPAuth`、`basicAuth`) を使用してください。これが認証定義の信頼できる情報源です。
...
> **注意**: `--transport http-only` は必須です。GeonicDB は Streamable HTTP (POST) のみをサポートしており、SSE は利用できません。`--allow-http` は `http://` URL に必要です（本番環境の `https://` では不要）。
...
API キーには有効期限がなく、Claude Desktop のような長期間の統合に推奨されます。
...
* `--policy <policyId>` を使用して作成時にポリシーをバインドする（`geonic me policies create` で作成された個人ポリシー）
...
> **注**: `env` の値は実際の API キーとテナント名に置き換えてください。環境変数を使用することで、認証情報を `args` 配列から分離できます。
...
Claude Code は Streamable HTTP をネイティブにサポートしているため、`mcp-remote` プロキシは不要です。
...
Streamable HTTP に対応している MCP クライアントであれば、直接接続できます。
...
各ツールには、操作の対象となるテナントを指定するための `tenant` パラメータがあります。


* **認証が無効の場合**: 省略すると、`default` テナントが使用されます。
  
* **認証が有効の場合**: 省略すると、ログインしているユーザーのテナントがデフォルトとして使用されます。`super_admin` はデータツールを使用できません (403 を返します)。代わりに `tenant_admin` または `user` ロールを使用してください。ただし、`tenant_admin`/`user` は自分のテナントにのみアクセスできます。これは認証されていない (匿名の) 呼び出し元にも適用されます (#2186): `tenant` は省略するか、呼び出し元自身のテナント (`Fiware-Service` ヘッダーから取得) と一致する必要があります — 異なるテナントを指定すると拒否されます (`Access denied`)。`tenant` を省略する (または呼び出し元自身のテナントを指定する) ことは、そのテナントが実際に `Fiware-Service` から解決できる場合にのみ許可されます。テナントコンテキストを持たない匿名の呼び出し元 — つまり、ヘッダーで指定されたテナントが存在しない場合 — も `Access denied` で拒否され、`default` テナントへのフォールバックは行われません。

### ServicePathの指定

**データツールは `servicePath` をサポートしていません (#1608)**。NGSI-LD には `Fiware-ServicePath` の概念がなく、MCP データツール (`entities`、`batch`、`temporal`、および `types`/`attributes` リソースを含む) は NGSI-LD API で動作するため、すべてのデータはルートパス `/` 配下に格納されます (HTTP NGSI-LD API と一致)。これらのツールの `servicePath` パラメータは非推奨です: ルート以外の値を指定するとエラーで拒否されます。
...
**例外 — `config` ツールのルール操作**: ReactiveCore Rules は `servicePath` をファーストクラスのフィールドとして使用するため、`config` ツールはルールの `list` (オプションのフィルターとして) および `create` 操作でこれを受け付けます (他のルール操作では無視されます)。値はトリミングされ、空文字列は `/` にデフォルト設定され、結果は `/^\/[\w/]*$/` に一致する単一の正確なパスでなければなりません (例: `/sensors`) — 階層的な `/#` およびカンマ区切りの複数パス値は拒否されます。これは、ルールが正確な `servicePath` の等価性でマッチングを行うためです (#1607/#1608)。
...
`batch` ツールの `query` アクションも `orderBy`、`orderDirection`、`sysAttrs`、`typePattern`、`local` をサポートしています。`temporal` ツールの `query` および `batch_query` アクションも `typePattern` をサポートしています(`type` と AND で結合)。`batch` ツールの `purge` は `typePattern` を受け付けません — 暗黙的に無視されるのではなく拒否され、HTTP `DELETE /ngsi-ld/v1/entities` のスコープと一致します。

**A2A スキルは同じ `typePattern` を受け付けます (#2156)。** `entities` (`list`、`search_by_location`、`search_by_attribute`)、`batch` (`query`)、および `temporal` (`query` / `list`) は同じセマンティクス(逐語的な正規表現、`type` との AND)でこれを適用します。`batch` の `purge` はこれを拒否し、HTTP `POST /ngsi-ld/v1/entityOperations/purge` でリクエストボディに配置された場合も同様です — そこで暗黙的に無視すると、呼び出し元が要求した**より広い**セットが削除されることになります。
...
**MCP ツールと A2A スキルはそのデフォルトを反転します:オプトインしない限り、ローカルデータのみをクエリします。**
...
デフォルトが異なる理由:`local` は HTTP バインディング用に定義されており、MCP / A2A はそのバインディングではないため、ローカルのみのデフォルトはコンプライアンス違反ではありません。両方とも**同期リクエスト/レスポンス**サーフェスであり、転送はプロバイダーのラウンドトリップを追加するため、AI クライアントが簡単にタイムアウトに達する可能性があります。転送を明示的にすることで、エージェントが要求していない分散オペレーションをトリガーすることを防ぎます。
...
転送できないアクションに `local` を渡す、または `types` / `attributes` リソースに渡すこと(これらは登録宣言された名前を MCP が接続しない別のメカニズム(#2008 / #2036)を通じてマージします)は、暗黙的に無視されるのではなく**拒否される**ため、クライアントは実際には転送していないのに転送したと信じることはありません。

認可は転送によって変更されません:呼び出し元のポリシー(#2003)から導出された行レベル読み取り述語は、ローカルエンティティだけでなくリモートエンティティにも適用されるため、特定のエンティティタイプ / スコープ / 所有者に制限されたプリンシパルは、ローカルで読み取れない Context Source エンティティを決して見ることがありません。

\*\*ID による取得(`entities` / `get`)は現時点ではローカルのみです。\*\*リモートのみのエンティティはローカル認可コンテキストを持たないため、`entityType` を条件とした `Deny` は HTTP の ID による取得パスでそれらを見逃していました。#2092 は `FederationService.getEntity` 内でリモートエンティティに同じ行レベル読み取り述語(#2003)を適用することでこれを解決しました。したがって、MCP / A2A からの ID による転送は認可の穴によってブロックされなくなりましたが、実装されていません。これを追加するには、HTTP コントローラーが使用する同じ `resolveReadableEntityFilter` の配線が必要になります。時系列読み取りパスには、既存の `docs/API_NGSILD.md` の注記に一致して、フェデレーション配線がまったくありません。
...
**`additionalProperties` フィールド**:エンティティが `propertyDetails` で定義されていない属性を持つことができるかを制御します。デフォルトは `true` です(任意の追加属性を許可し、NGSI-LD セマンティクスに従います)。`false` に設定すると厳密な検証を強制します — 定義された属性のみが受け入れられます。AI エージェントはエンティティを作成する際にこのフィールドを確認し、追加の属性が許可されているかどうかを判断する必要があります。

**`uniqueConstraints` フィールド**:複合ユニーク属性の組み合わせ(例:`[{"name": "one-booking-per-slot", "fields": ["room", "date", "startTime"]}]`)を宣言し、データベースのユニークインデックスを介してサーバー側で強制されます。エンティティの作成/更新が制約された組み合わせを重複させる場合、API は違反した制約名とともに `409 AlreadyExists` を返します。AI エージェントはエンティティを作成する前にデータモデルの `uniqueConstraints` を確認し、「violates unique constraint」を含む 409 をデータの競合(異なる値を選択)として扱い、エンティティ ID の衝突として扱わないようにする必要があります。

複合制約は正確なタプルに一致するため、重複する**値**を拒否しますが、重複する**範囲**を拒否することはありません。上記の例では、同じ部屋で同じ時刻に開始する 2 番目の予約は拒否されますが、後から開始してそれと重複する予約は拒否されません(10:00-11:00 対 10:30-11:30)。制約が範囲全体をカバーするようにするには、予約を共有の固定長グリッド(例:正時/30 分に開始する 30 分スロット)に整列させ、予約の `[start, end)` が重複する各グリッドスロットごとに 1 つのエンティティを格納します — 開始スロットだけではなく — 次に、それらのスロットを共有 id 属性でグループ化します。これは、すべての予約が同じグリッドとスロットルールに対して生成された場合にのみ衝突を保証します:グリッドに整列していない間隔、または一貫性のないスロットセット(例:開始スロットのみ)は、実際の重複を見逃したり、実際には重複しないスロットで衝突したりする可能性があります。任意の、グリッド化されていない間隔の重複は、常に別のアプリケーションレベルのチェックが必要です。提供しない保証にちなんで制約に名前を付けること(開始時刻のみのタプルに `no-double-booking` など)は、エージェントと開発者が独自の重複チェックをスキップするように導きます。
...
自動生成される語彙 IRI は**このContext Broker自身のベース URL** (#1984) 上に存在し、`GET /vocab/{tenantId}/{term}` 経由で参照解決可能です。ホスト名ごとのデータベースへのルーティングリクエスト (#1291) では、ベース URL は**そのデプロイメントの登録されたホスト名** (#3190) になります。なぜなら、そのホストのみがコンテキストを保持するデータベースを提供するためです。それ以外の場合は `API_BASE_URL` 環境変数から取得されます。この変数は SAM テンプレートパラメータ **`ApiBaseUrl`** (`infrastructure/template.yaml`) からデプロイ時に注入されます。設定されていない場合はリクエストの `Host` ヘッダーにフォールバックします。IRI をホスト名間で安定させるために `ApiBaseUrl` を設定してください。詳細は [API.md → Broker base URL resolution](../api-reference/endpoints.md#broker-base-url-resolution) を参照してください。

プロパティ URI はエンティティタイプに依存しません — 同じプロパティ名(例: `email`)は、同じテナント内の異なるエンティティタイプ間で同じ URI を共有します。

### @context の解決 (#1733)

NGSI-LD レスポンスをレンダリングするために使用される `@context` は、リクエストが提供したもの**のみ**です。何も提供されない場合、NGSI-LD コア `@context` のみが使用され、それが圧縮できない用語は完全修飾 URI としてレンダリングされます(ETSI GS CIM 009 clause 5.5.5 / 5.5.7)。
...
したがって、`attrs=https://uri.etsi.org/ngsi-ld/default-context/temperature` は `temperature` 属性を返しますが、`q` 内の同じ IRI はすべてのエントリ(HTTP、MCP、A2A を問わず)で沈黙した空の結果ではなく明示的なエラーになります。

MCP `config` ツールを通じて作成または更新されたカスタムデータモデルは、HTTP 経由で作成されたものと**同じ自動生成された `@context`** を取得します(#1986): `contextUrl` が省略された場合、JSON-LD コンテキストが `propertyDetails` から生成され、`/ngsi-ld/v1/jsonldContexts/` の下に登録され、その URL が `contextUrl` として保存されます。Context Broker自身のパブリック URL は **HTTP コントローラーと同じ優先順位**で解決されます(#2075 / #3190): 登録されたデプロイメントホスト名(ホスト名ベースのルーティングがホスト名ごとのデータベースに到達した場合)→ `API_BASE_URL` → MCP リクエスト自身の `Host` ヘッダー → ローカルサーバーアドレス。MCP ツールハンドラー自体は HTTP イベントを受信しませんが、サーバーはリクエストごとに再構築されます(`POST /mcp` 自体が HTTP 呼び出しです)。そのため、そのリクエストのイベントはイベントなしで解決されるのではなく、`config` ツールにスレッド化されます。`API_BASE_URL` を未設定のままにしているデプロイメント(例: SSM パラメータが欠落している環境)は、常に `http://localhost:3000` にフォールバックするのではなく、リクエスト自身の `Host` から構築された `contextUrl` を取得するようになりました。

カスタムデータモデルの `contextUrl` は自動的には注入されません。モデルの語彙を必要とする AI エージェントは、データモデルから `contextUrl` を読み取り、JSON-LD `Link` ヘッダー経由で読み取り時にそれを渡す必要があります — コンテキストなしで返される完全修飾 URI は、それ自体が曖昧さのないセマンティック識別子です。

## AI コーディングアシスタントと JavaScript SDK

GeonicDB JavaScript SDK (`@geolonia/geonicdb-sdk`) は AI 支援開発を念頭に設計されています。npm パッケージには完全な TypeScript 型宣言が含まれているため、AI コーディングアシスタント (Claude Code、Cursor、GitHub Copilot など) は追加の設定なしで公開 API 全体を自動的に発見できます。
...
両者は同じ基礎サービス層を共有し、同じ 5 つのスキル/ツールカテゴリをサポートしています。
