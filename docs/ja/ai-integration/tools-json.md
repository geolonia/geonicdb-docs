...
GeonicDB は、AI エージェント (Claude、GPT-4、Gemini など) が API を簡単に利用できるように、複数の AI 向けインターフェースを提供します。
...
> **注意**: `env` の値を実際の API キーとテナント名に置き換えてください。環境変数を使用することで、`args` 配列に認証情報を記述せずに済みます。
...
`entities`、`types`、`attributes`、`batch`、`temporal` の各ツールには `servicePath` パラメータがあり、階層的なスコープ内でエンティティを管理できます。

#### 基本形式

- **形式**: `/` で始まるパス (例: `/hello`、`/city/sensors`)
- **デフォルト**: 省略した場合、ルートパス `/` が使用されます
- **用途**: 同一テナント内でエンティティをグループ化または分離するために使用されます
...
複数のパスをカンマで区切って指定することで、同時に検索できます (最大 10 パス)。
...
| `orderBy` | 属性またはシステムフィールドで並べ替え。降順の場合は `!` を前置 | `"createdAt"`、`"!modifiedAt"` |
| `orderDirection` | 並べ替え方向 (`!` プレフィックスの代替) | `"asc"`、`"desc"` |
...
**`additionalProperties` フィールド**: エンティティが `propertyDetails` で定義されていない属性を持つことができるかどうかを制御します。デフォルトは `true` です(NGSI-LD のセマンティクスに従い、追加の属性を許可します)。`false` に設定すると、厳密な検証が強制されます — 定義された属性のみが受け入れられます。AI エージェントは、エンティティを作成する際にこのフィールドをチェックして、追加の属性が許可されているかどうかを判断する必要があります。
...
GeonicDB JavaScript SDK (`@geolonia/geonicdb-sdk`) は、AI 支援開発向けに設計されています。npm パッケージには完全な TypeScript 型宣言が含まれているため、AI コーディングアシスタント(Claude Code、Cursor、GitHub Copilot など)は、追加の設定なしに完全なパブリック API を自動的に発見できます。
...
別途ドキュメント URL や特別な設定は必要ありません。TypeScript プロジェクトでは、完全な型チェックと IDE のオートコンプリートがすぐに使えます。詳細については、SDK ドキュメントを参照してください。
...
どちらも同じ基盤サービス層を共有し、同じ 5 つのスキル/ツールカテゴリをサポートします。
