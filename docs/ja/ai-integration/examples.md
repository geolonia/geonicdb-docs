...
MCP `temporal` は ETSI GS CIM 009 clause 4.5.9 の簡略化された temporal 表現(`{ type, values: [...] }`)を反映したマルチインスタンス短縮表記を受け入れます:
...
* 親オブジェクトのキー:**`type`(オプション)と `values`(必須)のみ**。その他の親キー(`observedAt`、`datasetId`、`unitCode`、…)は、実行可能なメッセージとともに **400** を返します(#2717)。
  
* `observedAt` / `datasetId` は親ではなく、**各インスタンス**に配置してください。
  
* `unitCode` / その他の予約されたメタデータは、MCP temporal 書き込みパス(単一属性またはインスタンス)で**まだ接続されていません**。#2722 で追跡されています。
...
`Not attempted:` プレフィックスは、未試行要素と実際にタイムアウトした要素(ドライバー由来のメッセージを持つ)を区別するものです。`success` にリストされたエンティティはコミットされます。**`errors` にリストされた entityIds のみを再試行してください**。`batch_create` / `batch_upsert` の場合、temporal 書き込みは追加のみであるため、既に成功したエンティティを再送信すると、それらの temporal インスタンスが重複します。`batch_delete` の場合、既に削除されたエンティティを再送信すると、単に見つからないと報告されます(データは重複しません)。HTTP API は、ProblemDetails 型のエラー(`type` `https://uri.etsi.org/ngsi-ld/errors/ServiceUnavailable`、同じ `detail` テキスト)を含む `207 Multi-Status` として同じ分割を返します。

### Temporal `modify_instance` と属性タイプ (#2311)

HTTP `PATCH /ngsi-ld/v1/temporal/entities/{entityId}/attrs/{attrName}/{instanceId}` (ETSI GS CIM 009 clause 5.6.14) はボディ `type` を受け入れ、属性インスタンスのタイプを置き換えます (`input.type ?? oldDoc.type`)。MCP の `temporal` ツールはこのフィールドを `type` ではなく **`attrType`** として公開しています:
...
`type: "FuelProbe"` を `modify_instance` に渡しても、属性タイプをエンティティタイプ名に書き換えては**いけません**。既存の属性タイプを保持するには `attrType` を省略してください (部分更新)。

A2A はこの操作を実装していません。

MCP/A2A を共有表現レイヤーを通じてルーティングすることの結果 (**以前の生出力に依存していたクライアントにとっては破壊的変更**):


* システムの時系列属性 (`createdAt` / `modifiedAt` / `expiresAt`) は `options=sysAttrs` がリクエストされた場合に**のみ**返されます (clause 6.3.11)。以前は無条件に返されていました。
  
* 内部の `attrNameForm` マーカーは出力されなくなりました — これは API コントラクトの一部ではありませんでした。
  
* `instanceId` は依然として返されます: これは clause 4.5.7 で定義された表現メンバーであり、インスタンスの変更/削除操作 (clause 5.6.14) で必要です。
  
* 属性名とタイプ名は clause 5.5.7 の圧縮を経由します。MCP/A2A はリクエスト `@context` を提供しないため、絶対 IRI として保存された名前は完全修飾 URI としてレンダリングされます (clause 5.5.7 のフォールバック)。これは HTTP がコアのみのリクエスト `@context` に対して行うのとまったく同じです。

A2A の `temporal` スキルは、その `get` / `query` / `list` アクションで同じ形状を返します。

**時系列リクエストパラメータはすべてのエントリーポイントで同じように解釈されます (#2099 / #2156)。**
HTTP、MCP `temporal.get` / `query` / `batch_query`、および A2A `temporal.get` / `query` / `list` はすべて、`timerel` / `timeAt` / `endTimeAt` / `timeproperty` / `lastN` / `attrs` / `format` / `options` / `aggrMethods` / `aggrPeriodDuration` を同じリゾルバーを通じてルーティングするため、いずれも黙って無視されることはありません。A2A クライアントにとって注目すべき 2 つの結果:


* **存在しないエンティティに対する `temporal.get` は、明示的なエラーになりました** (`Entity not found: <id>`)。以前は空の `entities` 配列を持つ `200` でした — 「見つからない」と「読み取り不可」が区別できませんでした。
  
* \*\*切り捨てが報告されます。\*\*デフォルトの `lastN` キャップが属性の履歴を切り捨てる場合、HTTP API は `NGSILD-Warning: 199 …` を設定します; MCP と A2A にはヘッダーがないため、同じテキストがレスポンスオブジェクトの `warnings` 配列に返されます。その不在は何も切り捨てられなかったことを意味します。

`sysAttrs` は A2A でブール値として機能し続け、`options: "sysAttrs"` (ETSI GS CIM 009 clause 6.3.11) も受け入れられます。
...
属性は**ユーザー定義のサブ属性** (Property of Property / Relationship of Property、ETSI GS CIM 009 clause 4.5) と**予約済みメンバー** `observedAt`、`unitCode`、`datasetId`、`valueType` を持つことができます。**MCP / A2A の `entities` (およびバッチ) 書き込み**は、HTTP NGSI-LD API とまったく同じルールでこれらを永続化します:
...
* **例外 — MCP `temporal` 書き込み (#2722):** `attrMetadata` を介した `unitCode` / その他の予約済みメタデータは、MCP temporal (単一属性パスまたは `values[]` インスタンス) では**配線されていません**。各**インスタンス**の `datasetId` / `observedAt` は機能します; `values[]` エンベロープの親キーは拒否されます (#2717)。上記の例が `temporal` ツールに適用されると仮定しないでください。
...
* サブ属性名は属性名と同じルールに従います: 短い名前 (`^[A-Za-z0-9_]+$`) または絶対 IRI です。それ以外 (例えば `unit.code`) は拒否されます。NGSI-LD コアボキャブラリの絶対 IRI は正規形式で保存されるため、`https://uri.etsi.org/ngsi-ld/default-context/accuracy` は `accuracy` として保存され、読み戻されます (clause 5.5.7)。
  
* \*\*かつて「HTTP と同じルール」の例外だった名前が 1 つあります: `__proto__`。\*\*これは `^[A-Za-z0-9_]+$` にマッチします。#2059 以降、MCP と A2A はこれを自己プロパティとして永続化し、#2100 以降は HTTP NGSI-LD 書き込みパスも `validateNgsiLdBody` を介して同じことを行います (Zod のオブジェクト再構築では、そうしないとキーが黙って削除されます)。
  
* 1 レベルのネストが保持されます。サブ属性自身の `observedAt` とより深いネストは保持されません。
  
* サブ属性は、MCP と A2A の読み取りが返す簡略化された (keyValues) 表現には表示されません。それらを読み戻すには、正規化された表現を使用する HTTP NGSI-LD API を使用してください。
...
**`auth` に関する注意**: ai-plugin.json の `auth` ブロックは、単一のスキームのみを機械可読形式で表現できます — ここでは Bearer (`authorization_type: "bearer"`) です。X-Api-Key および OAuth 2.0 クライアント資格情報の代替手段は、人間が読めるテキストである `instructions` にのみ記載されています。サポートされているすべてのスキームの完全な機械可読定義については、`/openapi.json` の `securitySchemes` (`BearerAuth`、`ApiKeyAuth`、`DPoPAuth`、`basicAuth`) を使用してください。これが認証定義の信頼できる情報源です。
...
* **オペレーションモード**: ステートレス (Lambda 互換)
  
* **認証**: 認証が有効な場合(デフォルト)、アクセス制御とテナント分離は JWT Bearer トークン経由で適用されます
...
> **注意**: GeonicDB は Streamable HTTP (POST) のみをサポートしており、SSE は利用できないため、`--transport http-only` が必要です。`http://` URL には `--allow-http` が必要です（本番環境の `https://` には不要）。
...
> **重要**: API キー（`gdb_` プレフィックスの文字列）は作成時に一度だけ表示されます。安全に保管してください。`--save` フラグはキーを CLI 設定に保存し、自動使用を可能にします。

API キーは XACML 認可モデルの下でデフォルトですべて拒否されます。次の 2 つの方法のいずれかでアクセスを許可します：


* `--policy <policyId>` で作成時にポリシーをバインド（`geonic me policies create` で作成された個人ポリシー）
...
* **認証が有効の場合**: 省略すると、ログインユーザーのテナントがデフォルトとして使用されます。`super_admin` はデータツールを使用できません(403 を返します)。代わりに `tenant_admin` または `user` ロールを使用してください。ただし、`tenant_admin`/`user` は自分のテナントにのみアクセスできます。これは未認証(匿名)の呼び出し元にも適用されます(#2186): `tenant` は省略するか、呼び出し元自身のテナント(`Fiware-Service` ヘッダーから取得)と一致する必要があり、異なるテナントを指定すると拒否されます(`Access denied`)。`tenant` を省略する(または呼び出し元自身のテナントを指定する)ことは、そのテナントが `Fiware-Service` から実際に解決できる場合にのみ許可されます。つまり、テナントコンテキストを持たない匿名の呼び出し元(ヘッダーが存在しないテナントを指定している場合)も `Access denied` で拒否され、`default` テナントへのフォールバックは**行われません**。
...
**例外 — `config` ツールのルール操作**: ReactiveCore Rules は `servicePath` をファーストクラスフィールドとして使用するため、`config` ツールはルールの `list`(オプションのフィルタとして)および `create` 操作で受け付けます(その他のルール操作では無視されます)。値はトリミングされ、空文字列は `/` にデフォルト設定され、結果は `/^\/[\w/]*$/` に一致する単一の正確なパス(例: `/sensors`)である必要があります。階層的な `/#` やカンマ区切りの複数パス値は拒否されます。これは、ルールが正確な `servicePath` の等価性でマッチするためです(#1607/#1608)。
...
`batch` ツールの `query` アクションも `orderBy`、`orderDirection`、`sysAttrs`、`typePattern`、`local` をサポートしています。`temporal` ツールの `query` および `batch_query` アクションも `typePattern` をサポートしています(`type` と AND として組み合わせ)。`batch` ツールの `purge` は `typePattern` を受け付けません — HTTP `DELETE /ngsi-ld/v1/entities` のスコープと一致して、暗黙的に無視されるのではなく拒否されます。

**A2A スキルは同じ `typePattern` を受け付けます (#2156)。** `entities` (`list`、`search_by_location`、`search_by_attribute`)、`batch` (`query`)、および `temporal` (`query` / `list`) は同じセマンティクス(逐語的な正規表現、`type` との AND)で適用します。`batch` の `purge` はこれを拒否し、HTTP `POST /ngsi-ld/v1/entityOperations/purge` もリクエストボディに配置された場合は拒否します — そこで暗黙的に無視すると、呼び出し元が要求したよりも**広い**セットを削除することになります。
...
デフォルトが異なる理由:`local` は HTTP バインディング用に定義されており、MCP / A2A はそのバインディングではないため、ローカルのみのデフォルトはコンプライアンスの逸脱ではありません。どちらも**同期リクエスト/レスポンス**サーフェスであり、転送はプロバイダーのラウンドトリップを追加するため、AI クライアントは簡単にタイムアウトに達する可能性があります。転送を明示的にすることで、エージェントが要求していない分散オペレーションをトリガーすることを防ぎます。
...
転送できないアクションに `local` を渡す場合、または異なるメカニズム(#2008 / #2036)を通じて登録宣言された名前をマージする `types` / `attributes` リソースに渡す場合(MCP はワイヤリングしていない)、黙って無視されるのではなく**拒否**されます。これにより、クライアントが実際には転送していないのに転送したと信じることがありません。

認可は転送によって変更されません:呼び出し元のポリシー(#2003)から派生した行レベルの読み取り述語は、ローカルエンティティと同様にリモートエンティティにも適用されるため、特定のエンティティタイプ / スコープ / オーナーに制限されたプリンシパルは、ローカルで読み取れない Context Source エンティティを決して見ることはありません。

\*\*ID による取得(`entities` / `get`)は当面ローカルのみです。\*\*リモートのみのエンティティはローカルの認可コンテキストを持たないため、`entityType` 条件付きの `Deny` は HTTP の ID 指定パスでそれらを見逃していました。#2092 は `FederationService.getEntity` 内のリモートエンティティに同じ行レベルの読み取り述語(#2003)を適用することでこれを解決しました。したがって、MCP / A2A からの ID 指定転送は認可の穴によってブロックされることはなくなりましたが、実装されていません。これを追加するには、HTTP コントローラーが使用する同じ `resolveReadableEntityFilter` ワイヤリングが必要です。Temporal 読み取りパスには連携ワイヤリングが全くなく、`docs/API_NGSILD.md` の既存の注記と一致しています。
...
* **認証**:認証が有効な場合(デフォルト)、Bearer トークンが必要です。明示的に `AUTH_ENABLED=false` を設定した場合、認証なしで動作します。
  
* **OAuth スコープ**:OAuth トークンを使用する場合、各 MCP ツール操作に対応する OAuth スコープが必要です(例:`read:entities` はエンティティの読み取り、`write:entities` は書き込み)。スコープ制限は JWT RBAC トークンには適用されません。
  
* **分散オペレーションはオプトイン**:HTTP API とは異なり、MCP / A2A は `local="false"` が渡されない限り Context Sources に転送せず、上記でリストされたクエリアクションのみが対象です(#2072)。
...
**`additionalProperties` フィールド**: `propertyDetails` で定義されていない属性をエンティティが持てるかどうかを制御します。デフォルトは `true` (NGSI-LD のセマンティクスに従い、任意の追加属性を許可) です。`false` に設定すると厳格な検証が強制され、定義された属性のみが受け入れられます。AI エージェントは、エンティティを作成する際にこのフィールドをチェックして、追加の属性が許可されているかどうかを判断する必要があります。

**`uniqueConstraints` フィールド**: データベースの一意インデックスを介してサーバー側で強制される、複合一意属性の組み合わせ (例: `[{"name": "one-booking-per-slot", "fields": ["room", "date", "startTime"]}]`) を宣言します。エンティティの作成/更新が制約された組み合わせと重複する場合、API は違反した制約名を含む `409 AlreadyExists` を返します。AI エージェントは、エンティティを作成する前にデータモデルの `uniqueConstraints` をチェックし、「violates unique constraint」を含む 409 をデータの競合 (異なる値を選択) として扱い、エンティティ ID の衝突としては扱わないようにする必要があります。

複合制約は正確なタプルに一致するため、重複する**値**を拒否しますが、重複する**範囲**を拒否することはありません。上記の例では、同じ部屋で同じ時刻に開始する 2 番目の予約は拒否されますが、後で開始して重複するもの (10:00-11:00 vs 10:30-11:30) は拒否されません。制約が範囲全体をカバーするようにするには、予約を共有の固定長グリッド (例: 時刻または半時刻から開始する 30 分スロット) に整列させ、予約の `[start, end)` が重複するグリッドスロットごとに 1 つのエンティティを保存します — 開始スロットだけではなく — そして、それらのスロットを共有 id 属性でグループ化します。これは、すべての予約が同じグリッドとスロットルールに対して生成された場合にのみ衝突を保証します: グリッドに整列していない間隔、または一貫性のないスロットセット (例: 開始スロットのみ) で生成されたものは、実際の重複を見逃したり、実際には重複していないスロットで衝突したりする可能性があります。任意の、グリッド化されていない間隔の重複には、常に別のアプリケーションレベルのチェックが必要です。提供していない保証にちなんで制約に名前を付けること (例: 開始時刻のみのタプルに `no-double-booking` を付ける) は、エージェントや開発者が独自の重複チェックをスキップすることにつながります。
...
自動生成された語彙 IRI は**このContext Broker自身のベース URL** (#1984) 上に存在し、`GET /vocab/{tenantId}/{term}` を通じて参照解決可能です。ホスト名ごとのデータベースへのルーティングされたリクエスト (#1291) では、ベース URL は**そのデプロイメントの登録されたホスト名** (#3190) となります。これは、そのホストのみがコンテキストを保持するデータベースを提供するためです。それ以外の場合は、`API_BASE_URL` 環境変数から取得されます。これは SAM テンプレートパラメータ **`ApiBaseUrl`** (`infrastructure/template.yaml`) からデプロイ時に注入され、未設定の場合はリクエストの `Host` ヘッダーにフォールバックします。IRI がホスト名間で安定するように `ApiBaseUrl` を設定してください。詳細は [API.md → Broker base URL resolution](../api-reference/endpoints.md#broker-base-url-resolution) を参照してください。

プロパティ URI はエンティティタイプに依存しません — 同じプロパティ名(例: `email`)は、同じテナント内の異なるエンティティタイプ間で同じ URI を共有します。

### @context 解決 (#1733)

NGSI-LD レスポンスをレンダリングするために使用される `@context` は、**リクエストが提供したもののみ**です。何も提供されない場合は、NGSI-LD コアの `@context` のみが使用され、それがコンパクト化できない用語は完全修飾 URI としてレンダリングされます (ETSI GS CIM 009 clause 5.5.5 / 5.5.7)。

### クエリ内の属性名 (#1998)

MCP と A2A は NGSI-LD **コア語彙**を使用します — どちらもリクエストの `@context` を持ちません。属性名引数は、Context Brokerが保存するのと同じ正規形式に正規化されるため、コア語彙内の絶対 IRI は保存されたキーに解決されます:
...
したがって、`attrs=https://uri.etsi.org/ngsi-ld/default-context/temperature` は `temperature` 属性を返しますが、`q` 内の同じ IRI は、サイレントな空の結果ではなく、すべてのエントリ (HTTP、MCP、A2A を問わず) で明示的なエラーとなります。

MCP `config` ツールを通じて作成または更新されたカスタムデータモデルは、HTTP 経由で作成されたものと**同じ自動生成 `@context`** を取得します (#1986): `contextUrl` が省略された場合、JSON-LD コンテキストが `propertyDetails` から生成され、`/ngsi-ld/v1/jsonldContexts/` に登録され、その URL が `contextUrl` として保存されます。Context Broker自身の公開 URL は、**HTTP コントローラーと同じ優先順位** (#2075 / #3190) で解決されます: 登録されたデプロイメントホスト名 (ホスト名ベースのルーティングがホスト名ごとのデータベースに到達した場合) → `API_BASE_URL` → MCP リクエスト自身の `Host` ヘッダー → ローカルサーバーアドレス。MCP ツールハンドラー自体は HTTP イベントを受信しませんが、サーバーはリクエストごとに再構築されます (`POST /mcp` 自体が HTTP 呼び出しです)。そのため、そのリクエストのイベントがイベントなしで解決されるのではなく、`config` ツールにスレッド化されます。`API_BASE_URL` を未設定のままにしているデプロイメント (例: SSM パラメータが欠落している環境) は、常に `http://localhost:3000` にフォールバックするのではなく、リクエスト自身の `Host` から構築された `contextUrl` を取得するようになりました。
...
GeonicDB JavaScript SDK (`@geolonia/geonicdb-sdk`) は AI 支援開発向けに設計されています。npm パッケージには完全な TypeScript 型宣言が含まれているため、AI コーディングアシスタント (Claude Code、Cursor、GitHub Copilot など) は追加の設定なしで完全なパブリック API を自動的に発見できます。
...
4. AI が文書化された API を使用して正しいコードを生成する

別途のドキュメント URL や特別な設定は必要ありません。TypeScript プロジェクトでは、完全な型チェックと IDE オートコンプリートがそのまま利用できます。詳細については SDK ドキュメントを参照してください。
...
* **MCP** はツール呼び出しのためのものです — AI エージェントが GeonicDB をツールとして使用します
  
* **A2A** はエージェント間通信のためのものです — AI エージェントが GeonicDB をピアエージェントとして協調します

どちらも同じ基盤となるサービスレイヤーを共有し、同じ 5 つのスキル/ツールカテゴリをサポートしています。
