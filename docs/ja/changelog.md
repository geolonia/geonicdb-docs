...
- **セキュリティ**: `MONGODB_ENFORCE_SECRETS=true` で `MONGODB_URI` 環境変数経路を fail-closed に禁止 (issue #1086) (#1111)
  ...
  - テスト: `tests/unit/infrastructure/mongodb.test.ts` に `getMongoUriAsync` の 5 ケース (enforce + ARN 未設定・enforce + fetch 失敗・enforce + secret 取得・unset env フォールバック × 2) を追加### 2026-04-30
...
- **Breaking**: `onTokenRefresh(callback)` を廃止し `on('tokenRefresh', cb)` イベントに統合 (#831)
