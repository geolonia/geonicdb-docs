...
| `--no-color` | カラー出力を無効化 |
| `-v, --verbose` | HTTP リクエスト/レスポンスの詳細を stderr に表示 (機密情報はマスクされます) |
...
#### `geonic profile show [name]`プロファイルの設定を表示します。デフォルトではアクティブなプロファイルが対象です。機密性の高い値はマスクされます。
...
| `GDB_OAUTH_CLIENT_ID` | OAuth クライアント資格情報のクライアント ID |
| `GDB_OAUTH_CLIENT_SECRET` | OAuth クライアント資格情報のクライアントシークレット |
| `GDB_API_KEY` | API キー(`--api-key` と同等) |
| `GEONIC_CONFIG_DIR` | 設定ディレクトリパスの上書き |
| `NO_UPDATE_NOTIFIER` | アップデート通知の無効化 |

### オプションの解決順序

値は次の順序で解決されます(優先度が高い順):
...
ターミナルで実行する場合、CLI は email と password を対話的に入力するようプロンプトを表示します。非対話環境では、`GDB_EMAIL` と `GDB_PASSWORD` 環境変数を設定してください。
...
| `--tenant-id <id>` | スコープ付き認証用のテナント ID |

### トークン自動更新

リクエストが 401 Unauthorized を返し、`refreshToken` が利用可能な場合、CLI は `POST /auth/refresh` を介して自動的にトークンを更新し、リクエストを再試行します。

`clientId` と `clientSecret` が設定に保存されている場合 (例: `geonic me oauth-clients create --save` を使用)、CLI はトークンの有効期限が切れた際に Client Credentials フローを使用して自動的に再認証します。
...
`[json]` 引数を受け取るコマンドは、複数の入力方法をサポートしています。CLI は自動的に入力元を検出します:
...
JSON5 をサポート: クォートなしキー、シングルクォート、末尾のカンマ、コメントが使用可能です。
...
CLI がターミナルに接続されており、JSON 引数が提供されていない場合、対話的な `json>` プロンプトが開きます。入力は括弧がバランスすると自動送信されます。
...
| `table` | ASCII テーブル (配列は列、オブジェクトはキー・バリューのペア) |
...
CLI は 24 時間ごとに新しいバージョンをチェックし、アップデートが利用可能な場合に通知ボックスを表示します。このチェックは CI 環境と非 TTY ターミナルではスキップされます。`NO_UPDATE_NOTIFIER=1` を設定することで無効化できます。
...
| `--georel <rel>` | 地理関係 (例: `near;maxDistance==1000`) |
...
| `--count-only` | エンティティをリストせずに合計カウントのみを表示 |
...
エンティティに対するバッチ操作 (`/ngsi-ld/v1/entityOperations`)。エイリアス: `batch`。
...
コンテキスト サブスクリプション (`/ngsi-ld/v1/subscriptions`) を管理します。エイリアス: `sub`。

| コマンド | 説明 |
|---------|-------------|
| `geonic sub list` | サブスクリプション一覧を表示 |
...
コンテキスト ソース登録 (`/ngsi-ld/v1/csourceRegistrations`) を管理します。エイリアス: `reg`。

| コマンド | 説明 |
|---------|-------------|
| `geonic reg list` | 登録一覧を表示 |
...
---### `temporal`時系列エンティティデータ(`/ngsi-ld/v1/temporal`)を管理します。
...
#### `geonic temporal entityOperations query [json]`集約サポート付きの POST による時系列エンティティのクエリを実行します。
...
### `snapshots`エンティティのスナップショットを管理します。
...
| `geonic catalog datasets list` | データセットの一覧を表示 |
...
| `tenantId` | string | はい (super_admin) | 対象テナント ID。 super_admin には必須。 tenant_admin にはオプション (デフォルトは自身のテナント) |
| `allowedOrigins` | string[] | はい | CORS オリジン (最小 1、最大 20)。すべてを許可する場合は `["*"]` を使用 |
| `policyId` | string | いいえ | 既存の XACML ポリシーにバインド (ターゲットはバイパス)。省略した場合、認可はテナントポリシー、次にロールのデフォルトにフォールバック (`api_key` のデフォルトは拒否) |
| `rateLimit` | object | いいえ | `{ perMinute: number }` (1–1000、デフォルト: 60) |

> **注意**: 平文の API キーは作成レスポンスの `key` フィールドに一度だけ返されます。安全に保管してください。
...
#### `me oauth-clients`自分自身の OAuth クライアントを管理します (`/me/oauth-clients`)。`admin oauth-clients` とは異なり、これには管理者権限は必要ありません — 認証された任意のユーザーが自分自身のクライアントを管理できます。
...
|--------|-------------|
| `--name <name>` | クライアント名 |
| `--policy <policyId>` | 既存の XACML ポリシーにバインド (ターゲットはバイパスされます)。省略した場合、認可はテナントポリシー、次にロールのデフォルト (`user` のデフォルトは GET のみ) にフォールバックします |
| `--save` | 自動再認証のために認証情報を設定に保存 |
...
`--save` を使用すると、CLI は Client Credentials グラントを即座に実行し、`clientId`、`clientSecret`、および結果の `token` を設定に保存します。以降のトークン期限切れは自動的に処理されます。

#### `me api-keys`自分自身の API キーを管理します (`/me/api-keys`)。`admin api-keys` とは異なり、これには管理者権限は必要ありません — 認証された任意のユーザーが自分自身のキーを管理できます。ユーザーあたり 5 つのキーに制限されています。
...
> **注意**: プレーンテキストの API キーは、作成レスポンスの `key` フィールドに 1 回のみ返されます。安全に保管してください。

### `help`WP-CLI スタイルのヘルプで段階的な詳細を表示します。
...
永続化のために `~/.zshrc` に追加してください。

補完はサブコマンド名、オプションフラグ、および `--format` 値の候補 (`json`、`table`、`geojson`) をサポートします。
