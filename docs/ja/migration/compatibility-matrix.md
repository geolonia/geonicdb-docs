...
このドキュメントは、GeonicDB と FIWARE Orion Context Broker の機能を比較します。
...
| `GET /ngsi-ld/v1/entities` | ✅ | ✅ | エンティティの一覧表示 |
...
| `GET /ngsi-ld/v1/subscriptions` | ✅ | ✅ | サブスクリプションの一覧表示 |
...
| `GET /ngsi-ld/v1/csourceRegistrations` | ✅ | ✅ | 登録の一覧表示 |
...
| `GET /ngsi-ld/v1/csourceSubscriptions` | ✅ | ❌ | CSR サブスクリプションの一覧表示 (*) |
...
| `GET /ngsi-ld/v1/attributes` | ✅ | ✅ | 属性の一覧表示 |
...
| **準拠情報** | ✅ | ✅ | `/ngsi-ld/v1/info/conformance` |
| **ソース識別** | ✅ | ✅ | `/ngsi-ld/v1/info/sourceIdentity` |
| **ベクタータイル** | ✅ | ❌ | `/ngsi-ld/v1/tiles` GeoJSON ベクタータイル |

> **csourceSubscriptions に関する注記**
> Context Source Registration (CSR) サブスクリプション機能は、ETSI GS CIM 009 仕様で定義されています。GeonicDB は仕様準拠の実装を提供していますが、Orion-LD は現在実装していません (実装予定; [Orion-LD Issue #280](https://github.com/FIWARE/context.Orion-LD/issues/280) を参照)。
...
| 機能 | GeonicDB | FIWARE Orion-LD | 備考 |
|---------|:------------------:|:---------------:|-------|
| normalized | ✅ | ✅ | 完全フォーマット (デフォルト) |
| concise | ✅ | ✅ | 簡潔フォーマット (type 省略) |
...
| `keyValues` | ✅ | ✅ | 簡略化フォーマット |
| `values` | ✅ | ✅ | 値のみ |
| `unique` | ✅ | ✅ | `values` との組み合わせで重複排除 |
| `sysAttrs` | ✅ | ✅ | システム属性 (dateCreated、dateModified) を含む |
| `normalized` (デフォルト) | ✅ | ✅ | 完全フォーマット |
| **属性選択** | | | |
| `attrs` パラメータ | ✅ | ✅ | 含める属性 |
...
| 分散型タイプ/属性ディスカバリー | ✅ | ❌ | /types と /attributes に CSR を含める (ETSI GS CIM 009 - 5.9.3.3) |
| **モード** | | | |
...
| auxiliary | ✅ | ✅ | ローカル優先、欠落データをリモートで補完 (NGSI-LD 標準、NGSIv2 拡張) |
...
| **OIDC 外部 IdP 統合** | ✅ | ❌ | OpenID Connect による外部認証プロバイダー |
...
| 機能 | GeonicDB | FIWARE Orion | 備考 |
|---------|:------------------:|:------------:|-------|
| **CADDE 統合** | ✅ | ❌ | クロスドメインデータ交換プラットフォーム |
| `x-cadde-*` ヘッダーサポート | ✅ | ❌ | リソース URL とプロバイダー情報 |
| プロヴェナンス情報ヘッダー | ✅ | ❌ | `x-cadde-provenance-*` |
...
| **CKAN ハーベスターサポート** | ✅ | ❌ | 自動データハーベスティングサポート |
...
| **トレーシング** | | | |
| AWS X-Ray | ✅ | ❌ | |
| OpenTelemetry | ✅ | ⚠️ 限定的 | OTLP over HTTP/gRPC |
| **メトリクス** | | | |
| CloudWatch Metrics | ✅ | ❌ | |
...
| **CKAN 互換 API** | CKAN オープンデータポータルのハーベスターと互換 |
| **CADDE 統合** | CADDE (データ連携基盤) コネクタとの統合機能 |
| **WebSocket イベントストリーミング** | AWS API Gateway WebSocket API を介したリアルタイムエンティティ変更配信。エンティティタイプと ID パターンでフィルタリング可能 |
...
| **OIDC 外部 IdP 統合** | OpenID Connect による外部認証プロバイダー統合 |
| **XACML ポリシーセット** | ポリシーセットによる階層的アクセス制御管理 |
| **時系列バッチ操作** | `temporal/entityOperations/create`、`upsert`、`delete` (ETSI GS CIM 009 仕様を超えた独自拡張) |
| **Time Series Collection** | MongoDB Time Series Collection による最適化された時系列データ保存、`$dateTrunc` 集計、TTL データ保持ポリシー |
...
注: Keyrock IdM / Wilma PEP Proxy については、GeonicDB も API 互換性による統合が可能です (上記「認証と認可」セクションを参照)。
