...
GeonicDB についてよく寄せられる質問と回答をまとめたものです。
...
**A:** GeonicDB 自体には明示的なデータ量制限はありません。MongoDB のスケーリング能力に依存します。

#### ハードリミット (システム制約)

| 制約 | 値 | 説明 |
|------|-----|------|
| リクエストあたりの最大アイテム数 | 1,000 | ページネーション用の `limit` 上限 (FIWARE Orion 互換) |
| 管理 API の最大アイテム数 | 100 | 管理 API のページネーション上限 |
...
| 1,000,000 エンティティ以上 | MongoDB Atlas M50+ でシャーディングを検討 |
...
| クエリパターン | 理由 | 対策 |
|--------------|------|------|
| 属性値の部分一致検索 | インデックスが効果的でない | 可能な限り完全一致を使用 |
| `q` フィルタの複雑な組み合わせ | フルスキャンになる可能性 | フィルタ条件を絞り込む |
| 広範囲の Geo 検索 | 候補が多すぎる | 検索エリアを制限 |
| `limit` なしで全レコード取得 | メモリ消費が大きい | 常にページネーションを使用 |

### Q: 時系列 (Temporal) データで注意すべきことは?

**A:** 時系列データはエンティティ数 x 属性数 x 時間間隔でデータ量が急速に増加します。
...
> **注意**: MongoDB 8.0 以上が必要です (Time Series Collection サポートのため)。Amazon DocumentDB は Time Series Collection をサポートしていないため対応していません。

### Q: コストの見積もりは?

**A:** サーバーレスアーキテクチャのため、使用した分だけ課金されます。

| コンポーネント | 小規模 (月間 100,000 リクエスト) | 中規模 (月間 1,000,000 リクエスト) |
|--------------|---------------------------|----------------------------|
| Lambda | 約 $5 | 約 $20 |
...
| 観点 | NGSIv2 | NGSI-LD |
|------|--------|---------|
| 学習曲線 | 低い | やや高い (JSON-LD の理解が必要) |
| FIWARE エコシステム | 多くのツールが利用可能 | 対応ツールが増加中 |
| 時系列データ | 非サポート (別途実装が必要) | Temporal API で標準サポート |
| データ相互運用性 | 限定的 | JSON-LD により高い |
...
| Spatial ID | デジタル庁 3D 空間 ID 対応 | NGSIv2、NGSI-LD |
...
**A:** 地図アプリケーション用にエンティティの位置情報を GeoJSON タイル形式で出力する機能です。
...
**A:** デジタル庁・IPA が策定した「3D 空間識別子」仕様に対応した機能です。緯度経度に加えて高度 (階層) を含む 3D 空間を一意に識別できます。
...
| API Key (X-Api-Key) | IoT デバイスやサードパーティ統合用の軽量認証 |
...
| ロール | 権限 |
|--------|------|
| `super_admin` | プラットフォーム管理のみ (`/admin/*`、`/auth/*`)。データ API にはアクセス不可 (403 を返す) |
| `tenant_admin` | 割り当てられたテナントの管理、ユーザー管理 |
| `user` | エンティティの読み書き (ポリシーで制限可能) |
| `api_key` | X-Api-Key ヘッダー経由のスコープベースアクセス (origin/entity-type 制限) |
