...
**A:** GeonicDB 自体には明確なデータ量の上限はありません。MongoDB のスケーリング機能に依存します。
...
**A:** 時系列データの量は、エンティティ数 x 属性数 x 時間間隔で急速に増加します。
...
> **注意**: MongoDB 8.0 以上が必要です(Time Series Collection サポートのため)。Amazon DocumentDB は Time Series Collection をサポートしていないため、サポートされていません。

### Q: 推定コストはどのくらいですか?

**A:** サーバーレスアーキテクチャであるため、使用した分だけ課金されます。
...
**A:** 日本のデジタル庁/IPA が策定した「3次元空間識別子」仕様をサポートする機能です。緯度・経度に加えて高度(階層)を含む 3D 空間の一意識別を可能にします。
