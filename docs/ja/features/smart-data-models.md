...
GeonicDB は [Smart Data Models](https://smartdatamodels.org/) イニシアチブのデータモデルに対応しています。Smart Data Models は、FIWARE エコシステムやスマートシティ分野で広く使用されている標準化されたデータモデルのカタログです。

## 概要

スマートデータモデル対応には、以下の 2 つの機能が含まれます:
...
GeonicDB は、以下のドメインから主要な Smart Data Models に対応しています:
...
## MCP ツール: `data_models`Smart Data Models カタログを閲覧するための MCP ツールが利用可能です。### アクション

#### `list_domains` - ドメインのリスト取得
...
#### `list_models` - モデルのリスト取得

利用可能なデータモデルのリストを取得します。ドメインまたは検索キーワードでフィルタリングできます。
...
**注意**: `propertyDetails` フィールドは、主要なモデル (WeatherObserved、AirQualityObserved、OffStreetParking、OnStreetParking、TrafficFlowObserved、Vehicle、Device、Building、WasteContainer、EnergyMonitor) で利用可能です。各プロパティには以下の情報が含まれています:
...
- `@context`: JSON-LD 語彙 URI (オプション、HTTP(S) URL のみ)。カスタムデータモデルの場合は、Linked Data の相互運用性を向上させるため、よく知られた語彙 (例: `https://schema.org/email`) を指定してください。`@context` を持たないプロパティには、自動生成された URL (`https://geonicdb.geolonia.com/vocab/{tenantId}/{propertyName}`) が付与されます。

## @context の自動補完

NGSI-LD API を介してエンティティを取得する際、GeonicDB は既知の Smart Data Model タイプに対して適切な @context を自動的に追加します。
...
3. **デフォルトの NGSI-LD コア @context** - フォールバック### 例: スマートデータモデルエンティティの作成と取得
...
- **明示的な @context が優先されます**: Link ヘッダーで @context が指定された場合、SDM の自動補完よりも優先されます
- **未知のタイプはデフォルトの @context を使用します**: カスタムエンティティタイプの場合、NGSI-LD コアの @context のみが返されます## さまざまなドメインの例
...
Smart Data Models @context を使用することで、次のことが可能になります:
...
MCP ツールを通じて、AI アシスタント (Claude など) は次のことができます:
