...
HTTP メソッド、ヘッダー、ペイロードのカスタマイズを可能にします。
...
認証が有効な環境では、サブスクリプションに対して所有権ベースのアクセス制御が適用されます。
...
* **取得 (GET) と一覧表示 (LIST)** は、同じテナント内の認証されたユーザーであれば制限はありません。
...
`entities[]` に複数の要素が含まれる場合、サブスクリプション作成を成功させるには**すべての要素が Permit でなければなりません**(全 Permit セマンティクス)。単一の不一致なタイプまたは id パターンがあると、リクエスト全体が `403 Forbidden` で拒否されます。

> リテラル `body.type === "Subscription"` は `entityType` に伝播**されません**。ポリシーは `entities[].type` を対象とする必要があり、ラッパーオブジェクトのタイプではありません。
...
* 条件式をテストする: 手動でエンティティを更新し、条件が満たされることを確認
  
* 通知 URL をテストする: `curl` で直接到達可能か確認
...
* `httpCustom` マクロ構文を検証する(`${attrName}` は属性名と一致する必要がある)
...
* MQTT ブローカーの URL、ポート、および認証情報を確認する
...
* 通知の送信先が継続的にエラーを返している(5xx、タイムアウト)
  
* GeonicDB がサブスクリプションを自動的に無効にした

**解決策:**

* 通知の送信先エンドポイントのログを確認する
