...
ローカル開発環境で ReactiveCore Rules を試すには、以下の手順に従ってください。
...
エンティティの変更が自動的に監視され、ルールが実行準備完了になります。
...
* **レプリカセットモード**: Change Streams には レプリカセットモードが必要なため、MongoDB はレプリカセットモードで起動します (Change Streams はスタンドアロン MongoDB モードでは動作しません)。
...
3. **閾値監視**: 温度が 30 度を超えた場合に警告属性を自動的に追加
...
1. **エンティティ変更検出**
   ...
   * レガシーの `ChangeStreamProcessorFunction` は依然として `publishEntityChangeEvent()` を呼び出して、変更ストリーム由来のイベントを同じ EventBridge ファンアウトに再パブリッシュするため、`RuleProcessorFunction`、`SubscriptionMatcherFunction`、`WsBroadcastFunction` はすべてこれらの再生イベントを受信して処理します(ルールとサブスクリプションは再評価されます)。これはもはや `RuleEngineService` を直接呼び出さないため、ルールパスは単一の EventBridge コンシューマーに統合されます (#1119)
...
* エンティティ作成時のみアクションを実行する (例: `GeoJSON` が作成されたときにのみ `ActivityLog` を書き込む)
...
> **注**: UPDATE イベントの属性レベルフィルタリングには、`change` と組み合わせて使用します (例: `{type: "change", attributeName: "status"}`)。CREATE / DELETE では、`changedAttributes` が undefined であるため、`change` は常に false と評価されます。

### 7. CEL Expression Condition

[Common Expression Language (CEL)](https://github.com/google/cel-spec) を使用した柔軟な条件式です。複雑な計算、文字列操作、および複数の属性の組み合わせ評価をサポートします。
...
> **ヒント — `has()` でガードする**: 属性が以前の状態に存在しない可能性がある場合(例: `EntityCreated` の場合、または新しく追加された属性の場合)、アクセスを `has()` でラップします:
>
> ```text
> has(previous.attribute.temperature) && previous.attribute.temperature.value <= 30 && attribute.temperature.value > 30
> ```
>
> 欠損キーに対して `has()` なしで直接アクセスすると評価エラーが発生し、これはキャッチされて `false` として扱われます。
...
冪等更新(同じ値の再書き込み)は、`previous.attribute.temperature.value` が既に > 30 であるため発火しません。
...
Haversine 公式を使用した大圏距離の計算。入力は GeoJSON Point オブジェクト、出力はメートル (Number) です。
...
Ray casting アルゴリズムを使用したポイントインポリゴン判定。入力は GeoJSON Point と GeoJSON Polygon、出力は Boolean です。外側のリングのみがサポートされます (穴/内側のリングはサポートされていません)。外側のリングは閉じている必要があります (開始座標と終了座標が同じである必要があります)。
...
2. **閾値違反時のスナップショット記録**:特定の条件が満たされた場合にのみ時系列データを記録します (条件との組み合わせで使用)
...
ルールエンジンは、プロトコル境界を越えたエンティティの作成をサポートします。例えば、NGSIv2 センサーの変更が NGSI-LD エンティティの作成をトリガーしたり、その逆も可能です。

### 概要

GeonicDB はプロトコルの分離を強制します。NGSIv2 エンティティは NGSIv2 API 経由でのみアクセス可能で、NGSI-LD エンティティは NGSI-LD API 経由でアクセス可能です。ルールエンジンは、アクションがトリガーエンティティのプロトコルとは異なるターゲット `protocol` を指定できるようにすることで、このギャップを埋めます。
...
アクションで明示的に `servicePath` または `scope` を指定すると、自動マッピングが上書きされます。テンプレート変数(`${trigger.servicePath}`、`${trigger.scope}`)は、カスタムマッピングロジックに使用できます。
...
* **ルート servicePath**: `'/'` は scope にマッピングされません(NGSI-LD では意味的な意味を持ちません)
...
パス解決に加えて、アクションテンプレートは `${name(args)}` の形式で純粋関数の小さなホワイトリストを呼び出すことができます。サーバーのウォールクロック時刻をスタンプしたり、派生エンティティで一意の ID を生成したりするのに便利です(例:追記専用の `ActivityLog` レコード)。
...
* 関数は**ルール発火ごと**に評価されます — すべてのイベントが新しい値を作成するため、`${uuid()}` は派生エンティティごとに本当に一意であり、`${now()}` はルール登録時ではなく評価の瞬間を反映します。
  
* 引数パーサーは、単純なカンマ区切りのリテラル文字列(`'iso'`、`"unix"`)のみを処理します。ネストされた式、数値演算、および `${now(entity.id)}` などの参照はサポートされていません — それらは代わりに CEL `celExpression` 条件で計算し、結果をエンティティ属性として公開してください。
  
* 不明な関数名とサポートされていない引数値は、プレースホルダーテキストをそのまま残します(例:`${notAFunction()}` はリテラルのまま)。ルール作成者がタイプミスを修正できるように警告がログに記録されます。
...
温度が 30 度以上で、かつ湿度が 80% 以上、または時間が 12:00 から 15:00 の間である場合に警告を発行します。
...
**注意:** このルールは `priority: 5` を持ち、Example 6 の `priority: 10` より高い優先度です。したがって、DI > 80 の場合、まず `DANGER` が設定され、その後の `WARNING` 更新で上書きされないように注意する必要があります。同じエンティティに対して両方のルールが一致する場合、優先度の昇順で実行されるため、順序は `DANGER` → `WARNING` となります。これを回避するには、WARNING ルールの CEL 式に上限条件を追加します:
...
* ルールが作成したエンティティによってルール自身が再トリガーされることを防止
...
**制限**: 単一のエンティティに対して、単一のルールは**1分間に最大 10回**まで実行できます。
...
* ルール毎、エンティティ毎の実行回数を追跡
  
* 時間ウィンドウ内で最大値に達した場合、それ以降の実行をブロック
  
* カウンターは時間ウィンドウの期限が切れると自動的にリセットされます
...
* 実行チェーン内で同じルールが 2回出現する場合(循環)、実行がブロックされます
...
2\. **Change 条件を使用する**: 属性が実際に変更された場合にのみトリガーします
...
4\. **ルールの優先順位を適切に設定する**: 意図しない連鎖を防ぐために実行順序を制御します
...
あるルールのアクションが別のルールの条件にマッチすると、無限ループが発生する可能性があります。
...
2. `change` 条件を使用して、特定の属性の変更時のみトリガーする
...
本文書は、NGSI ベースのコンテキストブローカーにおけるエンティティ変更を処理するための GeonicDB Rule Engine フォーマットを規定します。本仕様は、Event-Condition-Action (ECA) パターンに従った JSON ベースのルールフォーマットを定義し、IoT およびスマートシティアプリケーション向けに最適化されています。
...
実装は、REQUIRED とマークされたすべての機能を実装している場合、**適合** とみなされます。

OPTIONAL とマークされた機能は、実装者の裁量で実装してもかまいません。

#### 2.2 必須機能

適合実装は、次のことを満たさなければなりません (MUST):
...
GeonicDB Rule Specification v1.0 の完全な JSON Schema は仕様書に記載されています。すべてのルールはこのスキーマに対して検証される必要があります (MUST)。

主要な検証ルール:

* `ruleId`、`name`、`tenantId`、`servicePath`、`conditions`、`actions`、`isActive`、および `priority` は必須フィールドです (REQUIRED)
  
* `conditions` 配列は少なくとも 1 つの条件を含む必要があります (MUST)
  
* `actions` 配列は少なくとも 1 つのアクションを含む必要があります (MUST)
  
* `cooldownSeconds` は指定する場合、正の整数である必要があります (MUST)
  
* `servicePath` は `/` で始まる必要があります (MUST)

完全な JSON Schema 定義については、正式仕様書のセクション 8 を参照してください。

### 4. Versioning

#### 4.1 バージョン形式

本仕様は Semantic Versioning 2.0.0 (<https://semver.org/>) に準拠します:
...
1. 機能はドキュメントで非推奨 (DEPRECATED) としてマークされます
   
2. 機能は少なくとも 1 つの MAJOR バージョンの間、動作し続けます
   
3. 非推奨警告はログに記録されるべきです (SHOULD)
   
4. 移行ガイドは提供される必要があります (MUST)
...
* [API Specification](../api-reference/endpoints.md) - すべてのエンドポイントの一覧
