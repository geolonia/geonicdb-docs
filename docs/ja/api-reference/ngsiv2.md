...
GeonicDB はエンティティ `type` に対して **POSIX Portable Filename Character Set** を、エンティティ `id` に対して URN 対応セットを、そして MongoDB / `q` パーサーの制約 (#1209) により属性名に対してより厳格なセットを強制します。これは NGSIv2 仕様のフィールド構文制限(ASCII マイナス制御文字 / 空白 / `& ? / #` を許可)の文書化された拡張です。
...
* `id` / `type` の先頭文字は `-` にできません(POSIX 慣例; CLI フラグとの衝突を防ぐため)。古い `id` 正規表現 `^[\w:.-]+$` は先頭の `-` を許可していました; これは一貫性のため #1209 で**わずかに厳格化**されました。
  
* `id` は NGSI-LD URN 形式 (`urn:ngsi-ld:Type:identifier`) のために `:` を受け入れます; `type` は受け入れません。URI / 完全修飾型名 (例: `https://uri.fiware.org/ns/data-models#Sensor`) は `type` 値として**まだサポートされていません** — #1211 を参照してください。
  
* 属性名は `[A-Za-z0-9_]` に制限されたままです (`.` `-` は不可)。これは、属性名が MongoDB フィールドキー (`attributes.${name}`) として直接埋め込まれ、既存の `q` クエリパーサーが `([\w.]+)` で属性名をキャプチャするため、`-` を含む名前にマッチしないためです。両方のレイヤーが更新されるまで、属性名で `.` `-` を許可すると、サイレントな落とし穴が生じます。
  
* 違反は `400 BadRequest` を返し、問題のある値と許可される文字セットを含むエラーメッセージを返します。例: `Entity type contains invalid characters (allowed: A-Z a-z 0-9 . _ - (must not start with -)); got "Sensor@Type"`。
...
| ---------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `id`             | string  | Filter by entity ID (multiple values can be specified as a comma-separated list). **Mutually exclusive with `idPattern`** — specifying both returns `400 Bad Request` (NGSIv2 spec: "Both cannot be used at the same time"). An empty string counts as an explicit value for this purpose (#2114) | -           |
...
| `type`           | string  | Filter by entity type. **Mutually exclusive with `typePattern`** — specifying both returns `400 Bad Request` (NGSIv2 spec: "Both cannot be used at the same time"). An empty string counts as an explicit value for this purpose (#2114)                                                          | -           |
| `typePattern`    | string  | Regular expression pattern for entity type, evaluated **verbatim** (no implicit `*`→`.*` conversion — write a regex like `Room.*` or `.*`, not a glob) (#2114). **Mutually exclusive with `type`**                                                                                                | -           |
| `idPattern`      | string  | Regular expression pattern for entity ID, evaluated **verbatim** (no implicit `*`→`.*` conversion) (#2114). **Mutually exclusive with `id`**                                                                                                                                                      | -           |
...
> **範囲クエリ (#1586)**: NGSIv2 Simple Query Language の範囲形式は `attr==min..max` です (例: `temperature==20..30`)。単独の `:` (例: `temperature:20..30`) は有効な NGSIv2 SQL 演算子では**ない**ため、範囲として解析されません — [Query Language](./endpoints.md#query-language) を参照してください。
...
> **パスパラメータのパーセントエンコーディング (#2695、ポリシー #1628 と同じ契約)。**
> `event.path` は API Gateway およびローカルの Express ラッパーからパーセントエンコードされた状態で到着します。
> エンティティ `entityId` / `attrName` (および登録 `registrationId`) は、ルックアップ前に**正確に 1 回**デコードされます。不正なエスケープ (`/v2/entities/%`、`a%b`) は、生のセグメントを ID として扱う代わりに `400 BadRequest` (`entityId contains invalid URL encoding`) を返します (以前の NGSIv2 本番環境の動作である `/v2/entities/%` に対する `404 NotFound` の代わり)。
> 文字通り `%` を含む ID は `%25` でアドレス指定する必要があります (例:`50%-off` → `/v2/entities/50%25-off`)。作成時の `Location` ヘッダーは既にパス ID と `type` クエリ値の両方をパーセントエンコードしているため、それをそのまま追跡すれば機能します。
>
> \*\*マッチングセマンティクスの変更:\*\*パーセントエスケープのように見えるパスセグメント (例:保存された `id` がリテラル文字列 `%2F` であるエンティティ) は、1 回のデコード後にルックアップされるようになりました (`%2F` の場合は `/`)。以前に生のエスケープをパスセグメントとして送信し、保存された id に対する完全一致文字列に依存していたクライアントは、サイレントな不一致 (#1800 クラス) が発生する可能性があります。HTTP スタックがパスセグメントを自動エンコードするクライアントは修正されました (以前は不完全な `%` に対して `404` を取得していました)。

> **GeonicDB 拡張 — エンティティ ID の一意性**:エンティティ ID は、テナントおよびServicePathのスコープ内で一意です。同じ ID で異なるタイプのエンティティを作成することは許可されておらず、`409 AlreadyExists` を返します。これは、異なるタイプで同じ ID のエンティティを許可する NGSIv2 仕様とは異なります。詳細については、[エンティティ ID の一意性](./endpoints.md#entity-id-uniqueness-geonicdb-extension) を参照してください。

> **注意 (軽微な破壊的変更、#1644)**:`geo:json` 属性値の `Polygon` リングは、最初と最後の位置がすべての要素で**等しく**閉じられている必要があります — 3 要素の `[longitude, latitude, altitude]` 位置が使用される場合は、高度も含まれます。NGSIv2 は以前、経度/緯度のみを比較し、最初と最後の位置が高度で異なるリングをサイレントに受け入れていました。そのようなリングは現在 `400 Bad Request` (「must be closed」) で拒否され、ルールが NGSI-LD と統一されます。2 要素 (2D) 座標を送信するクライアントは影響を受けません。[ポリゴンリングの閉鎖](./endpoints.md#polygon-ring-closure-1644) を参照してください。
...
> **注意**: `/v2/entities/{entityId}?attrs=...` とは異なり、このエンドポイントは `id` および `type` フィールドを含みません。属性のみが必要な場合に使用してください。
...
> **注意 (GeonicDB 拡張)**: エンティティのタイプに対してカスタムデータモデルが定義されており、属性が `required: true` とマークされている場合、代わりに `400 Bad Request` を返します — モデルがその属性を必要としている間は削除できません。`required: false` の属性、および `isActive: false` のモデル上の属性は、引き続き削除可能です。
...
**注意**: この操作では、既存の属性の型やメタデータは変更されません — それらは保持されます。

***

## バッチ操作

> **注意**: バッチ操作は、1 リクエストあたり最大 **`MAX_BATCH_SIZE`** のエンティティを処理できます (デフォルト: 100、`MaxBatchSize` SAM パラメータにより最大 10,000 まで設定可能)。この制限を超えるリクエストは `400 Bad Request` エラーになります。
...
> **相互排他性 (#2114)**: 各 `entities[]` セレクタ内では、`id`/`idPattern` は相互排他的であり、`type`/`typePattern` は相互排他的です — 同じセレクタ内でペアの両方のメンバーを指定すると `400 Bad Request` が返されます (NGSIv2 仕様:「両方を同時に使用することはできません」)。`idPattern`/`typePattern` は、文字通りの正規表現として評価されます (暗黙的な `*`→`.*` 変換はありません)。

> **`expression` は直接 `id` を含むすべてのセレクタに適用されます (#2135)**: `expression` (`q` / `mq` / `georel`+`geometry`+`coords`) は、**すべての** `entities[]` セレクタの結果をフィルタリングします。`{"id": ...}` セレクタで、そのエンティティが expression を満たさない場合、結果は得られません — これは、呼び出し元の認証モードに関係なく同じ結果です (以前は、無制限のプリンシパルは expression が暗黙的に無視されたエンティティを受け取っていました)。
...
> **注意 (#1586)**: `condition.expression.q` は、`q` クエリパラメータと同じ Simple Query Language を使用します ([Query Language](./endpoints.md#query-language) を参照)。範囲形式は `attr==min..max` です (例: `pressure==700..800`)。単なる `:` (例: `pressure:700..800`) は有効な演算子ではありません。
...
存在しない属性は文字列 `null` に置換されます。マクロは、attrs/exceptAttrs フィルタが適用される前の完全なエンティティに対して評価されます。

**型を保持する JSON テンプレート (`json`)**

`payload` は置換されたすべての値を文字列化します。属性の型を保持する必要がある場合(数値は数値のまま、ブール値はブール値のまま)、代わりに `json` を使用してください。**単一マクロ値** (`"${temperature}"`) は属性の元の JSON 型を保持します。**部分マクロ値** (`"prefix-${id}"`) は文字列化されます。**キーは決して置換されません** — キー内のマクロは `400` で拒否されます。テンプレートは作成時に境界が設定されます(シリアライズサイズ ≤ `MAX_PAYLOAD_LENGTH`、ネスト深度 ≤ `MAX_JSON_DEPTH`。違反 → `400`)。通知はデフォルトで `Content-Type: application/json` で送信されます(`receiverInfo` で上書き可能)。詳細は [SUBSCRIPTIONS.md](../features/ngsi-subscriptions.md#httpcustomjson-type-preserving-template) を参照してください。
...
`url` はブラウザの `PushSubscription.toJSON().endpoint` と同じ値です。
フィールドマッピングの全体については [SUBSCRIPTIONS.md](../features/ngsi-subscriptions.md#web-push-3014) を参照してください。

```json
{
  "description": "Room temperature Web Push notification",
  "subject": {
    "entities": [{ "type": "Room" }],
    "condition": { "attrs": ["temperature"] }
  },
  "notification": {
    "webpush": {
      ...
      "keys": {
        "p256dh": "<87-char base64url P-256 key>",
        "auth": "<22-char base64url auth secret>"
      },
      "ttl": 86400,
      "urgency": "normal",
      "topic": "room-temp"
...
| Field         | Type    | Required | Description                                              |
| ------------- | ------- | -------- | -------------------------------------------------------- |
| `url`         | string  | ✓        | Push Service endpoint URL (`https://`)                   |
| `keys.p256dh` | string  | ✓        | RFC8291 client public key (base64url, exactly 87 chars)  |
| `keys.auth`   | string  | ✓        | RFC8291 auth secret (base64url, exactly 22 chars)        |
| `ttl`         | integer | -        | RFC8030 TTL seconds (default applied at send if omitted) |
| `urgency`     | string  | -        | `very-low` / `low` / `normal` / `high`                   |
| `topic`       | string  | -        | RFC8030 topic (max 32 base64url chars)                   |
...
内部 `protocol` フィールドが存在する前に作成されたサブスクリプション (#1570) は、レガシーの正規化されたボディを保持します。それらの API プロトコルは判定できず、推測するとレシーバーがすでに解析しているボディを書き換えてしまいます。
...
> **行レベル読み取り認可 (#2140):** 行レベル読み取り述語(カスタム XACML ポリシー)によって制限されたサブジェクトの場合、`GET /v2/subscriptions` および `GET /v2/subscriptions/{subscriptionId}` は `subject.entities` から読み取り不可能な型セレクタを編集し、読み取り不可能な型のみを監視するサブスクリプションを非表示にし(リストの省略 / ID による読み取り時の `404`)、編集後に `Fiware-Total-Count` を計算します。サブスクリプションはプロトコル共有で保存されるため、これは NGSI-LD の動作と一致します([AUTH.md](../reference/auth.md) #2140 を参照)。制限のないサブジェクトには変更はありません。
...
> **自動一時停止 (#3080):** 配信失敗が `NOTIFICATION_AUTO_PAUSE_AFTER_MS`(デフォルト 1 時間)の間継続すると、Context Brokerは `status` を `inactive` に設定し、`autoDisabledAt`(GeonicDB 拡張、ISO 8601)を記録します。`GET` は両方のフィールドを公開します。`PATCH { "status": "active" }` で再開します(`autoDisabledAt` と失敗の連続をクリアします)。
...
認証が有効な場合(デフォルト)、サブスクリプションの更新(PATCH)および削除(DELETE)操作は `createdBy` フィールドに基づいて所有権検証を実行します。作成者以外のユーザーがこれらの操作を試みると、`404 Not Found` が返されます(サブスクリプションが全く存在しない場合と同じであるため、外部からは違いを観察できません)。`createdBy` が**ない**サブスクリプション(そのフィールドが存在する前に作成されたもの)も、非管理者に対しては `404 Not Found` を返します(#2161)。`super_admin` および `tenant_admin` ロールはこの検証をバイパスできます。詳細は [AUTH.md](../reference/auth.md) を参照してください。
...
> **行レベル読み取り認可 (#2084):** 行レベル読み取り述語 (カスタム XACML ポリシー) によって制限されたサブジェクトの場合、`GET /v2/registrations` および `GET /v2/registrations/{registrationId}` は `dataProvided.entities` から読み取り不可能な型セレクターを編集し、読み取り不可能な型のみを宣言する登録を非表示にし (一覧からの除外 / ID による読み取りでの `404`)、編集後に `Fiware-Total-Count` を計算します。登録はプロトコル間で可視であるため、これは NGSI-LD の `csourceRegistrations` の動作と一致します ([API\_NGSILD.md](./ngsild.md) / [AUTH.md](../reference/auth.md) を参照)。制限のないサブジェクトには変更はありません。
...
認証が有効な場合(デフォルト)、登録の更新(PATCH)および削除(DELETE)操作は `createdBy` フィールドに基づいて所有権の検証を実行します。作成者以外のユーザーがこれらの操作を試みた場合、`404 Not Found` が返されます(登録が全く存在しない場合と同様であるため、外部からはその違いを観察することはできません)。`createdBy` を**持たない**登録(そのフィールドが存在する前に作成されたもの)も、管理者以外には `404 Not Found` を返します(#2161)。`super_admin` および `tenant_admin` ロールはこの検証をバイパスできます。詳細は [AUTH.md](../reference/auth.md) を参照してください。
...
エンティティをクエリする際、一致する登録が存在する場合、クエリは並行してそのプロバイダーにも送信され、結果がマージされて返されます。
...
エンティティを更新または削除する際、一致する登録が存在する場合、更新は並行してそのプロバイダーにも転送されます。
...
* **Tenant isolation**: `Fiware-Service` ヘッダーによるテナント分離
  
* **Pagination**: `limit`/`offset` パラメータ。総数を取得するには `options=count` を使用
...
| `/v2/entities/{entityId}/attrs/{attrName}`       | DELETE | Delete attribute                        | 204     | 400, 401, 404      | -             |
...
> **注**: バッチ操作 (クエリを除く) は、リクエストあたり **`MAX_BATCH_SIZE`** エンティティまでに制限されています (デフォルト: 100)。この制限を超えると `400 Bad Request` が返されます。
