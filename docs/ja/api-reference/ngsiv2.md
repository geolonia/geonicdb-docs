...
> このドキュメントは [API.md](./endpoints.md) から分割されました。主な API 仕様については、[API.md](./endpoints.md) を参照してください。
...
| パラメータ | 型 | 説明 | デフォルト値 |
|-----------|------|-------------|---------|
| `id` | string | エンティティ ID でフィルタ (複数指定はカンマ区切りリストで可能) | - |
...
| `orderDirection` | string | ソート方向 (`asc`、`desc`)。**GeonicDB 拡張機能** (公式仕様では `!` プレフィックス方式のみサポート) | `asc` |
...
| パラメータ | 型 | 説明 |
|-----------|------|-------------|
| `options` | string | `upsert`: エンティティがすでに存在する場合は更新します。`keyValues`: リクエストボディを keyValues 形式として解釈します |
...
エンティティが存在しない場合は作成され (`201 Created`)、すでに存在する場合はその属性が更新されます (`204 No Content`)。

**レスポンス**
- ステータス: `201 Created` (新規作成)、`204 No Content` (upsert による更新)
- ステータス: 同じ ID を持つエンティティがすでに存在する場合 (型に関係なく) `409 AlreadyExists`- ヘッダー: `Location: /v2/entities/Room1?type=Room`> **GeonicDB 拡張 — エンティティ ID の一意性**: エンティティ ID は、テナントとServicePathのスコープ内で一意です。同じ ID だが異なる型でエンティティを作成することは許可されず、`409 AlreadyExists` を返します。これは、同じ ID で異なる型のエンティティを許可する NGSIv2 仕様とは異なります。詳細は [エンティティ ID の一意性](./endpoints.md#entity-id-uniqueness-geonicdb-extension) を参照してください。
...
| パラメータ | 型 | 説明 |
|-----------|------|-------------|
| `type` | string | エンティティ型 (オプションのフィルタ; エンティティ ID が一意であるため型の曖昧さ解消は不要 — [エンティティ ID の一意性](./endpoints.md#entity-id-uniqueness-geonicdb-extension) を参照) |
| `attrs` | string | 取得する属性名 (カンマ区切り) |
...
> **注意**: `/v2/entities/{entityId}?attrs=...` とは異なり、このエンドポイントには `id` と `type` フィールドは含まれません。属性のみが必要な場合はこれを使用してください。
...
値の型に応じて、異なる Content-Type で返されます:
...
認証が有効な場合 (`AUTH_ENABLED=true`)、サブスクリプションの更新 (PATCH) と削除 (DELETE) の操作は `createdBy` フィールドに基づいて所有権検証を実行します。作成者以外のユーザーがこれらの操作を試みた場合、`403 Forbidden` が返されます。`super_admin` および `tenant_admin` ロールはこの検証をバイパスできます。詳細については AUTH.md を参照してください。

---

## 登録

登録 (Registration) は外部コンテキストプロバイダーを登録し、エンティティ情報のソースを管理します。
...
認証が有効な場合 (`AUTH_ENABLED=true`)、登録の更新 (PATCH) および削除 (DELETE) 操作は `createdBy` フィールドに基づいて所有権の検証を行います。作成者以外のユーザーがこれらの操作を試みた場合、`403 Forbidden` が返されます。`super_admin` および `tenant_admin` ロールはこの検証をバイパスできます。詳細は AUTH.md を参照してください。

---

## フェデレーション (クエリ転送 / 更新転送)

登録に基づいて、GeonicDB はクエリを外部コンテキストプロバイダに転送し、結果を統合して返したり、更新を転送したりします。

### フェデレーションの仕組み

エンティティをクエリする際、一致する登録が存在する場合、そのプロバイダにも並行してクエリが送信され、結果がマージされて返されます。
...
エンティティを更新または削除する際、一致する登録が存在する場合、そのプロバイダにも並行して更新が転送されます。
...
| 排他モードで全プロバイダが失敗 | 502 エラーを返す (オプション) |
...
| ヘッダー | 値 | 目的 |
|--------|-------|---------|
| `ETag` | `W/"..."` | 弱いバリデータ。生成シードには `path + Accept + Fiware-Service + Fiware-ServicePath` が含まれるため、異なるエンドポイント / Accept / テナント / ServicePathは常に異なる ETag を生成します。リスト: `id + modifiedAt` と総数およびスコープを混合したストリーミングダイジェスト。単一: `modifiedAt` とスコープを混合したハッシュ。 |
...
| 409 | AlreadyExists | エンティティが既に存在 (POST 作成時) |
