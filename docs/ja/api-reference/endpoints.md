...
すべてのエンドポイントで `OPTIONS` メソッドがサポートされています。これは CORS プリフライトリクエストに応答して、許可されたメソッドとヘッダーに関する情報を返します。
...
> **注意**: `If-None-Match` / `If-Modified-Since` は `Access-Control-Allow-Headers` に明示的にリストされているため、ブラウザの HTTP キャッシュ自動再検証および SDK 条件付きリクエストがプリフライト拒否なしにクロスオリジンで発行できます (#1065)。
...
- バッチアップサート操作は `entityId` のみでエンティティを照合します (型は上書きされる可能性があります)
- 同じ ID のエンティティ間の型の曖昧性解消のための NGSIv2 `?type=` クエリパラメータは適用されなくなりました
...
各 API タイプごとに、合計件数を示すヘッダーが返されます:
...
- `offset` が合計件数を超える場合、空の配列が返されます (エラーではありません)
...
GET エンドポイントは、エンドポイントのクラスに基づいてキャッシュ関連のヘッダーを返します。クライアントはこれらを使用して、変更されていないレスポンスボディの転送をスキップできます。これは [RFC 7232](https://datatracker.ietf.org/doc/html/rfc7232) および [RFC 7234](https://datatracker.ietf.org/doc/html/rfc7234) に準拠しています。
...
| **時系列** | `/ngsi-ld/v1/temporal/entities` (リスト、単一、集約を含む) | ✗ (ETag なし — 時系列集約には安価な単調バリデーターが不足) | ✗ | `private, no-cache` |
...
| ヘッダー | 説明 |
|--------|-------------|
| `ETag` | 弱いエンティティタグ (`W/"..."`、RFC 7232 §2.3.2 弱いバリデーター)。生成時には常に **リソーススコープ** (`path + Accept + tenant + Fiware-ServicePath`) をシードに混ぜ込むため、異なるエンドポイント、Accept フォーマット、**テナント**、または **ServicePath** は、基礎となる状態が同一であっても異なる ETag を生成します。`tenant` スロットは最初に `NGSILD-Tenant` を読み取り、`Fiware-Service` にフォールバックし、`extractTenantContext` の優先順位と一致します。テナント / servicePath のシードは、中間キャッシュによって `Vary` が誤って処理された場合でも、テナント間の ETag 衝突を防ぎます。<br>• **リスト**: 各要素の `id + modifiedAt` のストリーミングダイジェストと、合計カウントおよびリソーススコープを組み合わせます。<br>• **単一リソース**: `modifiedAt` のハッシュとリソーススコープを組み合わせます。 |
...
メタエンドポイントは意図的に `ETag` / `Last-Modified` を省略します。これは、それらのコンテンツが安価な単調バリデーターを持たない集約クエリから派生しているためです。クライアントは条件付きリクエストの代わりに `max-age` に依存する必要があります。
...
両方のヘッダーが存在する場合、RFC 7232 §6 により `If-None-Match` が優先されます。
...
- ETag は弱い (`W/`) — バイト単位の同一性ではなく、セマンティックな等価性を伝えます。同じデータで属性の順序が異なる 2 つのレスポンスは、同じ ETag を共有します。
...
| `resourceScopes` | ResourceScope[] | いいえ | エンティティレベルのアクセス制御スコープ。省略時はフルアクセス。詳細は AUTH.md を参照してください |
...
**レスポンス**: `204 No Content`**注意**: パスワードを変更すると、既存のすべてのアクセストークンとリフレッシュトークンが無効化されます。新しいトークンを取得するには、再度ログインしてください。
...
すべてのセッションを無効化します。このユーザーに発行されたすべてのアクセストークンとリフレッシュトークンは即座に無効化されます。

**レスポンス**: `204 No Content`### API キートークン交換
...
> **注記**: テナント名には小文字の英数字とアンダースコア (`^[a-z0-9_]+$`) のみを含める必要があります。
...
**カスケード削除**: テナントが削除されると、関連するすべてのデータ (エンティティ、サブスクリプション、レジストレーション、ルール、ポリシー、ユーザ、メンバーシップ、およびすべての 16 個のコレクション) が自動的にカスケード削除されます。削除が開始される前に、テナントは自動的に無効化され、新しい API リクエストがブロックされます。
...
> **注記**: カスタムデータモデル API は `/custom-data-models` に移動しました。詳細については [カスタムデータモデル API](#カスタムデータモデル-api) セクションを参照してください。
...
テナントごとに個別の IP 制限を設定できます。テナントレベルの設定が存在する場合、グローバル設定 (`ADMIN_ALLOWED_IPS`) よりも優先されます。
...
スコープは `admin` (管理 API のみ) または `all` (すべての API) のいずれかを指定できます。詳細については AUTH.md を参照してください。
...
| `servicePath` | ServicePathでフィルタリング |
| `isActive` | アクティブ/非アクティブでフィルタリング (`true` / `false`) |
...
**有効化:** OAuth 2.0 は `AUTH_ENABLED=true` の場合、常に有効になります。`OAUTH_ENABLED` 環境変数は非推奨であり、無視されます。
...
**リソーススコープ:** `POST /oauth/token` で `resource_scopes` パラメータ (JSON 文字列) を指定すると、エンティティレベルのアクセス制御が適用されたトークンが発行されます。詳細は AUTH.md を参照してください。
...
**セキュリティ層:** Origin 検証 → HMAC Nonce (60秒 TTL) → Proof of Work → 短期間有効な JWT (1時間)
...
メタエンドポイントは認証不要で、システムステータスと API 情報を提供します。
...
Kubernetes / Route 53 の Liveness Probe 用。サービスが実行中かどうかをチェックします。
...
Kubernetes / Route 53 の Readiness Probe 用。MongoDB 接続性をチェックし、オプションで DynamoDB と EventBridge のディープヘルスチェックを実行します。
...
**レスポンス**
- 成功: `200 OK` と `status: "healthy"` を返す
- 失敗: `503 Service Unavailable` と `status: "unhealthy"` を返す
...
FIWARE Orion互換の統計エンドポイントとPrometheus形式のメトリクスエンドポイントを提供します。
...
FIWARE Orion互換形式でサーバーの運用統計を返します。認証が有効な場合 (`AUTH_ENABLED=true`)、認証されたユーザーのみがこのエンドポイントにアクセスできます。
...
Claude Tool Use / OpenAI Function Calling に互換性のある JSON 形式でツール定義を返します。これは AI エージェントが API をツールとして使用するためのスキーマです。
...
MCP Streamable HTTP エンドポイント。MCP 互換の AI クライアント(Claude Desktop など)から直接接続できます。ステートレスモード(JSON レスポンス)で動作し、全 5 つのツールが MCP tools/call 経由で利用可能です。
...
A2A エージェントカード。このエージェントの機能、スキル、認証方法を記述します。認証は不要です。
...
- `message/send` — メッセージを送信し、同期レスポンスを受け取る
- `tasks/get` — タスクの現在の状態を取得する
- `tasks/list` — フィルタリングとページネーションでタスクをリストする
- `tasks/cancel` — タスクのキャンセルをリクエストする
...
セミコロン (`;`) で AND 条件を結合します:

```text
q=temperature>20;pressure<800
```

パイプ (`|`) で OR 条件を結合します (`;` は `|` より優先されます):
...
`==` 演算子を `..` と組み合わせて範囲フィルタリングを行います (境界を含む):
...
| `coords` | 座標 (NGSIv2: 緯度,経度 形式; NGSI-LD: 経度,緯度 形式; 複数の点はセミコロンで区切る) |
...
> **重要**: NGSIv2 における緯度,経度の順序は、GeoJSON 標準 (経度,緯度) からの逸脱です。これは NGSI-LD で修正され、GeoJSON と同じ経度,緯度の順序を使用します。API を使用する際は、使用している API バージョンに対して正しい順序で座標を指定してください。
...
1. **距離ソート**: 結果は指定した座標からの距離の昇順でソートされます
...
`orderDirection=desc` を併用すると、距離の降順 (最も遠いものから) でソートできます:
...
| エラー条件 | エラーメッセージの例 |
|----------------|-----------------------|
| 無効な `georel` の値 | `Invalid georel: xxx. Supported values: near, coveredBy, within, contains, intersects, disjoint, equals` |
| 無効な `geometry` の値 | `Unsupported geometry type: xxx. Supported types: point, polygon, linestring, line, box` |
| 座標の不足 (Point) | `Point geometry requires at least 2 coordinates, but got 1` |
| 座標の不足 (Polygon) | `Polygon geometry requires at least 4 coordinate pairs (8 values), but got 6 values` |
| 座標の不足 (LineString) | `LineString geometry requires at least 2 coordinate pairs (4 values), but got 2 values` |
| 座標の不足 (Box) | `Box geometry requires 2 coordinate pairs (4 values), but got 2 values` |
...
日本のデジタル庁 / IPA が定める 3 次元空間識別標準 (ZFXY 形式) に基づく空間検索をサポートしています。
...
`spatialIdDepth` パラメータを指定すると、指定された空間 ID を中心とした周辺タイルに検索を拡張します。
...
エンティティは XYZ タイルスキームに基づいた GeoJSON ベクタータイルとして出力できます。大量のエンティティを地図上に効率的に表示するために最適化されています。
...
| CRS | EPSG | 説明 | 使用例 |
|-----|------|-------------|----------|
| WGS84 | EPSG:4326 | World Geodetic System 1984 (デフォルト) | GPS、国際標準 |
| JGD2011 | EPSG:6668 | Japanese Geodetic Datum 2011 | 日本における高精度測量 |
| Web Mercator | EPSG:3857 | Web Mercator 投影法 | Google Maps、OpenStreetMap など |
...
Geo-query の座標は指定された CRS で解釈されます:
...
| ヘッダー | 必須 | 説明 |
|--------|----------|-------------|
| `x-cadde-resource-url` | - | アクセス対象のリソースの URL |
...
テナント情報とその他の詳細は、`x-cadde-options` ヘッダーで指定できます。
...
CADDE リクエストへの応答には、以下のプロバナンスヘッダーが含まれます。

| ヘッダー | 説明 |
|--------|-------------|
| `x-cadde-provenance-id` | リクエストの一意識別子(Fiware-Correlator を使用) |
| `x-cadde-provenance-timestamp` | 応答生成時刻(ISO 8601 形式) |
...
| `Signing key not found` | 指定された kid を持つキーが JWKS に存在しません |
...
| 検索タイプ | ヘッダー値 | 説明 |
|-------------|--------------|-------------|
| 横断検索 | `x-cadde-search: meta` | CKAN 形式でデータセット一覧を返す (`q` パラメータでキーワードフィルタリング) |
| 詳細検索 | `x-cadde-search: detail` | 個別データセットの詳細を返す (`id` または `fq` パラメータで指定) |

CADDE 固有のフィールドがレスポンスに追加されます:
- `caddec_dataset_id_for_detail`: 詳細検索用データセット ID
...
| アクション | 説明 |
|--------|-------------|
| `subscribe` | エンティティタイプ / ID パターンによるフィルタの設定 |
...
| 機能 | ステータス | 備考 |
|---------|--------|-------|
| `near` ジオクエリ (近接検索) | サポート済み | Point ジオメトリのみ; `orderByDistance=true` で距離ソートと距離情報をサポート |
...
| 管理者 | `/admin` | 必須 (super_admin / tenant_admin) | `application/json` |
...
JavaScript SDK は npm パッケージとして利用可能: `npm install @geolonia/geonicdb-sdk`SDK は完全な公開 API を提供: `login()`、`setCredentials()`、エンティティ CRUD、`request()`、`connect()`、`reconnect()`、`disconnect()`、`isConnected()`、`subscribe()`、`on()`/`off()` イベントリスナー (`tokenRefresh` イベントを含む)。詳細は SDK ドキュメントを参照してください。

### ユーザーエンドポイント

認証済みユーザーが自分の情報を管理するためのエンドポイント。
...
テナントおよびユーザを管理するための API です。エンドポイントには `super_admin` または `tenant_admin` ロールが必要です（`tenant_admin` は自テナントスコープのみ）。
...
| `/admin/tenants/{tenantId}/users/{userId}` | PUT | テナントにユーザを追加（tenant_admin: 自テナントのみ） | 200 | 400, 401, 403, 404 | - |
| `/admin/tenants/{tenantId}/users/{userId}` | DELETE | テナントからユーザを削除（tenant_admin: 自テナントのみ） | 204 | 400, 401, 403, 404 | - |

#### ユーザ管理

| エンドポイント | メソッド | 説明 | 成功 | エラー | ページネーション |
|----------|--------|-------------|---------|-------|------------|
| `/admin/users` | GET | ユーザ一覧取得 | 200 | 400, 401, 403 | あり (最大: 100) |
| `/admin/users` | POST | ユーザ作成 | 201 | 400, 401, 403, 409 | - |
| `/admin/users/{userId}` | GET | ユーザ取得 | 200 | 401, 403, 404 | - |
| `/admin/users/{userId}` | PATCH | ユーザ更新 | 204 | 400, 401, 403, 404, 409 | - |
| `/admin/users/{userId}` | DELETE | ユーザ削除 | 204 | 401, 403, 404 | - |
| `/admin/users/{userId}/activate` | POST | ユーザ有効化 | 204 | 401, 403, 404 | - |
| `/admin/users/{userId}/deactivate` | POST | ユーザ無効化 | 204 | 401, 403, 404 | - |
| `/admin/users/{userId}/unlock` | POST | ログインロック解除 | 200 | 400, 401, 403, 404 | - |
| `/admin/users/{userId}/tenants` | GET | ユーザが所属するテナント一覧取得（自身または super_admin） | 200 | 401, 403 | あり (最大: 100) |
...
| `tenantService` | テナントサービス名（`Fiware-Service` ヘッダ） | リクエスト |
| `servicePath` | ServicePath（`Fiware-ServicePath` ヘッダ） | リクエスト |
...
| `entityOwner` | エンティティ作成者のユーザ ID（`createdBy` フィールド） | エンティティコンテキスト |

> `entityType` は HTTP リクエストからパスレベルで自動抽出されます（`?type=` クエリパラメータまたはリクエストボディの `type` / `@type` フィールドから）。これにより、エンティティレベルのチェックなしでエンティティタイプベースのアクセス制御が可能です。`entityId`、`entityOwner`、`scope` は、エンティティレベルの認可チェック（`requireEntityAuthz` 経由）でのみ利用可能です。`scope` は NGSI-LD エンティティのスコープ配列をカンマ区切り文字列として結合したもので、柔軟なマッチングには `string-regexp` または `glob` を使用してください。
...
ユーザは自身の OAuth クライアントを管理できます。1 ユーザあたり最大 5 クライアントまで。オプションの `policyId` は既存の XACML ポリシーにクライアントをバインドします。

| エンドポイント | メソッド | 説明 | 成功 | エラー | ページネーション |
|----------|--------|-------------|---------|-------|------------|
| `/me/oauth-clients` | GET | 自身の OAuth クライアント一覧取得 | 200 | 400, 401 | あり (最大: 100) |
| `/me/oauth-clients` | POST | 自身の OAuth クライアント作成 | 201 | 400, 401, 403 | - |
| `/me/oauth-clients/{clientId}` | PATCH | 自身の OAuth クライアント更新（部分更新） | 200 | 400, 401, 403, 404 | - |
...
`X-Api-Key` ヘッダによる認証用の API キーを管理します。新しいキーはプレーン UUID 形式（`randomUUID()`）を使用します。`gdb_` プレフィックス付きの既存キーは引き続き有効です。保存時は SHA-256 でハッシュ化されます。平文キーは作成時と更新時にのみ返されます。一覧取得/取得レスポンスは `"key": "******"` を返します。オプションの `policyId` フィールドは既存の XACML ポリシーにキーをバインドします（バインドされたポリシーのターゲットは評価中にバイパスされます）。`policyId` がない場合、キーはテナントポリシー + ロールデフォルト（api_key = 全て拒否）にフォールバックします。
...
| `/me/api-keys` | GET | 自分の API キーを一覧表示 | 200 | 400, 401, 403 | あり (最大: 100) |
...
API を介して CADDE (分野間データ連携基盤) 設定を管理します。設定は MongoDB に保存され、環境変数は不要です。
...
| `jwtIssuer` | string | - | JWT 発行者クレームの検証値 |
| `jwtAudience` | string | - | JWT オーディエンスクレームの検証値 |
...
| エンドポイント | メソッド | 説明 | 成功 | エラー | ページネーション |
|----------|--------|-------------|---------|-------|------------|
| `/rules` | GET | ルールを一覧表示 | 200 | 400, 401, 403 | あり (最大: 100) |
...
テナント固有のカスタムデータモデルを管理するための API です。JWT 認証が必要で、XACML ポリシーベースの認可により、`tenant_admin` および `user` ロールが自分のテナント内でカスタムデータモデルを管理できます。

**関連ドキュメント**: [SMART_DATA_MODELS.md](../features/smart-data-models.md)

| エンドポイント | メソッド | 説明 | 成功 | エラー | ページネーション |
|----------|--------|-------------|---------|-------|------------|
| `/custom-data-models` | GET | カスタムデータモデル一覧取得 | 200 | 400, 401, 403 | あり (最大: 100) |
...
カスタムデータモデルが定義されている場合、エンティティの作成または更新時に自動的に検証が実行されます。検証は `isActive: true` が設定されているモデルにのみ適用されます。
...
| 必須フィールド | `required: true` が設定されている属性が存在するか |
...
`propertyDetails` の各プロパティには、JSON-LD ボキャブラリマッピングのためのオプショナルな `@context` フィールドを HTTP(S) URL で含めることができます。これにより、自動生成される URI の代わりに既知のボキャブラリ (例: schema.org) を使用できます。
...
NGSI-LD レスポンスにおいて、カスタムデータモデルに `contextUrl` が設定されている場合、カスタムコンテキストがエンティティの `@context` に自動的に含まれます (コアコンテキストと一緒に配列として返されます)。
...
| `/catalog/datasets` | GET | データセット一覧取得 | 200 | 400, 401 | あり (最大: 1000) |
...
| エンドポイント | プロトコル | 説明 |
|----------|----------|-------------|
| `wss://{api-id}.execute-api.{region}.amazonaws.com/{stage}?tenant={name}` | WebSocket | エンティティ変更イベントをストリーム配信 (認証は `Authorization` ヘッダー経由で送信) |
