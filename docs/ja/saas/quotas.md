...
2. **ストレージクォータシステム** - MongoDB に基づくエンティティ/サブスクリプション/レジストレーション/時系列データの数量制限
...
* 1 分あたり: 600 リクエスト (10 req/sec)
  
* 1 時間あたり: 10,000 リクエスト
...
GeonicDB は、3 つのスライディングウィンドウ (分/時間/日) で動作するトークンバケットアルゴリズムを使用しています:


1. トークンは、エンドポイントの重みに基づいてリクエストごとに消費されます
   
2. リクエストは、3 つすべてのウィンドウに十分なトークンがある場合にのみ許可されます
   
3. トークンは、各時間ウィンドウの移行時に自動的に補充されます
...
各プランには、短期間の突発的なトラフィックスパイクに対処するためのバースト許容量があります。これにより、一時的に制限を超えることが可能になります。

### レスポンスヘッダー

レート制限が有効になっている場合、NGSIv2、NGSI-LD、および Catalog API エンドポイントからのレスポンスには、現在のレート制限ステータスを示すヘッダーが含まれます:
...
### 公開 (認証なし) エンドポイントのレート制限 (#1075)

認証なしでアクセス可能な公開エンドポイントは、テナントごとの `QUOTAS.PLANS` とは独立した IP ベースのトークンバケットによって保護されています。これにより、OAuth の `client_id+secret` ブルートフォースや、大規模な JSON 生成 (`/openapi.json` など) による DoS がブロックされます。
...
* `/auth/login` はこの制限の**対象外**です。これは `LoginProtectionService` (メール + IP ベースの段階的ロックアウト) によって保護されています。
  
* `/health`、`/health/live`、`/health/ready`、`/version` はこの制限の**対象外**です (ヘルスチェックポーリング用)。
  
* バケットストア (DynamoDB / MongoDB) が利用できない場合、リクエストは通過が許可されます。公開サーフェスをオフラインにしないよう、インフラストラクチャエラー時にはフェイルクローズしません。
...
* バッチ操作の場合、すべてのエンティティがクォータ内に収まる場合のみ実行が進行します
  
* 1 つでもクォータを超える場合、操作全体が拒否されます(全か無か)
...
* **tenant\_admin**: 自分のテナントのクォータを閲覧および変更可能
...
* リアルタイム通知を受信するための Webhook URL を設定
...
1. `Retry-After` ヘッダーで指定された秒数待ちます
   
2. リクエスト頻度を削減します
   
3. バッチ操作を活用してリクエスト数を削減します
   
4. プランのアップグレードを検討してください
...
1. 不要なエンティティ / サブスクリプション / レジストレーションを削除します
   
2. 時系列データの保持期間を短縮します
   
3. プランのアップグレードを検討してください
...
1. `RATE_LIMIT_ENABLED` 環境変数を確認します
   
2. SAM テンプレートのパラメータを確認します
   
3. DynamoDB テーブルが正しくデプロイされていることを確認します

## 入力検証の制限

GeonicDB は、不正使用を防止しシステムの安定性を確保するため、入力長とカウント制限を適用しています。
...
* アカウント単位の最大ログイン失敗回数:**15 分**以内に**5 回**
...
* ログイン成功時は IP カウンターをリセットしません(タイミングベースの列挙を防止)
...
Admin API における作成-削除サイクル攻撃を防止します:
...
* `createUser` および `deleteUser` に対してテナントごとに適用
...
#### Catalog / CADDE / Vocabulary API 検証
...
### Storage Quota 修正: `/v2/op/update` (#902)
...
* 開発・デプロイメントガイド - インフラストラクチャセットアップ
  
* [認証・認可](../reference/auth.md) - テナント/ユーザー管理、アクセス制御
