...
1. **レート制限システム** - DynamoDB または MongoDB によってバックアップされた固定ウィンドウカウンターを使用した API リクエスト制限
   
2. **ストレージクォータシステム** - MongoDB に基づくエンティティ/サブスクリプション/レジストレーション/時系列データのカウント制限
   
3. **監視・管理システム** - 使用状況の追跡、アラート配信、および管理 API

## クォータプラン

GeonicDB は 4 つの標準プランと 1 つのカスタムプランを提供しています:

> **プラン名。** `FREE` / `STANDARD` / `PREMIUM` / `ENTERPRISE` は、顧客向けの `T0` / `T5` / `T30` / `T40` に対応します(マッピングとクラスタ階層については QUOTA\_OPERATIONS.md §1 を参照してください)。
>
> **値は #2082 で引き上げられました**。顧客向けプランシートと以前の実装のうち、高い方に合わせられたため、**どのテナントの実効制限も引き下げられていません**。シートの方が低かった残りの 3 つのセル(T0 users / T0 batch size / T5 users)は、実装に合わせてシートを引き上げることで解決されたため、**以下の値は現在 [customer/PLAN\_QUOTAS.md](./customer/PLAN_QUOTAS.md) §2/§4 とセル単位で一致しています**。
>
> ⚠️ **同じ数値が 4 か所に存在します。1 つの PR で 4 か所すべてを変更してください** — 決して 1 か所だけを変更しないでください。そうしないと #2082 が再発します(どちら側が権威あるものかを誰も判断できなくなります):
>
> 1. `QUOTAS.PLANS` (`src/config/defaults.ts`) — 実装であり、権威
> 2. [customer/PLAN\_QUOTAS.md](./customer/PLAN_QUOTAS.md) §2/§4 — 顧客向け
> 3. **このセクション** — 開発者リファレンス
> 4. QUOTA\_OPERATIONS.md §1.1 — オペレーション実行手順書
>
> **`Maximum batch operation size` はプランごとに適用されるようになりました**(#2082)。以前は適用されていませんでした:`QUOTAS.PLANS[*].limits.maxBatchSize` は定義されており、Admin API は `customQuotas.limits.maxBatchSize` を受け入れていましたが、それを読み取るコードはありませんでした — 実効上限はすべてのプランで一律 `SECURITY.MAX_BATCH_SIZE` でした。`SECURITY.MAX_BATCH_SIZE` (**1000**) は現在、プランに依存しない絶対上限として Zod スキーマで使用されるだけです;プランごとの値は `handlers/api/index.ts` で `getMaxBatchSize()` を介して適用され、`cannot exceed the plan limit` で拒否されます。
>
> **`Maximum users` はプランにリンクされるようになりました**(#2082)。明示的な `settings.maxUsers` は引き続きプランのデフォルトよりも優先されるため、テナントごとのアドオンはプラン変更によって上書きされません。

### 技術的制限: 最大エンティティドキュメントサイズ (#2517)

プランのストレージカウントとは独立: **`QUOTAS.MAX_ENTITY_DOCUMENT_BYTES` = 1 MiB**(1 つのエンティティドキュメントの BSON サイズ)。`EntityRepository` で作成/置換時(合成候補の `BSON.calculateObjectSize`)および追加/マージ時(アトミックな `$expr` + `$bsonSize` 事前イメージガード)に適用されます。**413 `RequestEntityTooLarge`** で拒否されます。スナップショット復元は `EntityRepository` をバイパスし、**意図的に免除されます**(祖父条項による復元)。

顧客向け概要: [`docs/customer/PLAN_QUOTAS.md`](./customer/PLAN_QUOTAS.md)(以下の詳細セクションと文言を同期させてください)。

注意: PREMIUM/ENTERPRISE のリクエストボディ制限は 5MB/10MB であるため、リクエストがボディチェックを通過しても、結果のドキュメントが 1 MiB を超える場合、永続化時に 413 を受け取る可能性があります。

**暗号化されたテナント (#2517 SECREVIEW4):** 置換 (`append:false`) は、M2 に必要な場合のみ暗号化します。**現在の**ドキュメントがすでに 1 MiB 制限内であれば、過大な属性ペイロードは `encryptAttributes` の**前に**拒否されます(破棄されたバイトが DEK ローテーションにカウントされるのを回避)。現在のドキュメントがすでに制限を超えている場合、早期ペイロードチェックはスキップされるため、縮小する置換はアトミックな縮小 `$expr` ガードを介して成功できます。

**1 つのバッチ内の同一エンティティ ID:** `SECURITY.MAX_ENTITY_BATCH_OCCURRENCE_ROUNDS` (8)。1 つのリクエスト内で同じ ID がこれ以上出現すると、書き込み前に **400** で拒否されます(リクエストレベルの無効な入力 — 207 を返す書き込み後のエンティティごとのエラーとは異なります)。重複 ID をラウンド分割する**すべての**バッチアップサートエントリポイントに適用されます:NGSIv2 `/v2/op/update`、NGSI-LD `entityOperations/upsert` (マージおよび `options=replace`)、MCP バッチアップサート、および A2A バッチアップサート。

### T0 / FREE プラン(評価および開発用)

**レート制限:**

* 毎分: 3,000 weight units(weight-1 GET のみの場合 50 req/sec に相当 — [Endpoint Weights](#endpoint-weights) を参照)
  
* 毎時: 50,000 weight units
  
* 毎日: 500,000 weight units
  
* バースト許容量: 500 weight units
  
* 最大同時実行数: 6(`ceil(50 × 0.084 × 1.2)`、#3118 / #3113 `read-one` p50)
  
* 1 日あたりの最大通知数: 50,000(#1544; #3118 以降、意図的に `perDay` と連動しない)
...
### T5 / STANDARD プラン(小規模本番環境)

**レート制限:**

* 毎分: 48,000 weight units(weight-1 GET のみの場合 800 req/sec に相当)
  
* 毎時: 800,000 weight units
  
* 毎日: 8,000,000 weight units
  
* バースト許容量: 8,000 weight units
  
* 最大同時実行数: 81(`ceil(800 × 0.084 × 1.2)`)
  
* 1 日あたりの最大通知数: 800,000(#1544; #3118 以降、意図的に `perDay` と連動しない)
...
* 1 分あたり: 60,000 ウェイトユニット (weight-1 の GET のみの場合、**1,000** req/sec 相当 —
  公開されたソフトシーリング #3137; ステージング GW は #3132 の後 2000 だが、1,500 は公開不可;
  生の ×10 では 1,500)
  
* 1 時間あたり: 1,000,000 ウェイトユニット
  
* 1 日あたり: 10,000,000 ウェイトユニット
...
* 毎分: 60,000 重み単位 (重み 1 の GET のみの場合 **1,000** req/sec 相当 —
  PREMIUM と同じ公開ソフト上限 #3137; 生の ×10 は 3,000 — 共有エントリでは到達不可能、#3132 コールド)
...
* エンティティ: 250,000,000 (Mongo 分離には専用クラスターが必要 — DEDICATED\_CLUSTER\_ONBOARDING.md / #1492 / #3141 を参照; エントリは #3138 まで共有のまま)。エンティティ + 時系列上限の同時充填は **M50 推奨** (M40 \~1 TB はステージング avgObjSize \~933 B からの BSON+インデックスサイジング下で厳しい)
...
> **#3137 公開 rps ルール (裁可 2026-09-15; #3118 の "GW=1000 が上限" という文言に取って代わる):**
> 共有環境が両方の条件下で提供できるものだけを公開する:
> (1) 持続負荷下での **単テナント成功率 100%**、および (2) **他テナント無影響**
> (同居テナントへの測定可能な 429/5xx ブラスト半径なし)。#3132 以降、ステージングエントリは
> `ApiThrottlingRateLimit=2000` / `ApiReservedConcurrency=256` だが、PREMIUM/ENTERPRISE の **公開ソフト上限** は **1,000 req/s** (`perMinute` 60,000) のまま。**`read-one open` (ウォームアップ 2)** (#3132) で測定: 1,000 rps = 100%; 1,500 rps = 95.6% で
> ConcurrentExecutions は 256 に固定され、同居テナントへの \~73k API GW 5xx あり — したがって 1,500 は持続**または**バーストとして公開不可。他の重み 1 GET (list / `types` / `attrs`)
> はクォータ重みの等価物のみ; 到達可能な rps は測定されるまで未検証として扱う。
> ENTERPRISE 生 ×10 (3,000) は共有エントリでは到達不可能のまま。専用クラスタープランは **Mongo (Atlas) 分離のみ** を意味する; エントリ (GW + 予約済み同時実行数) は依然として共有 — 未公開のエントリパフォーマンスを "専用クラスター" に帰属させないこと
...
>
> テナントの到達可能 rps ≈ `maxConcurrency / latency` (かつ公開ソフト上限 / GW シェアを超えることはない)。重み単位予算は依然として適用される; 顧客シートの req/s 数値は重み 1 GET 相当。
...
GeonicDB は 3 つのウィンドウ (分/時/日) にわたる **固定ウィンドウカウンター** を使用します。ウィンドウ境界は絶対的です: 現在時刻は分/時/日の境界に切り捨てられ
(`src/core/quotas/rate-limit/rate-limit.service.ts` の `getWindowTimestamp()`)、その切り捨てられたタイムスタンプがウィンドウを識別します。


1. 各リクエストはエンドポイントの重みに等しい許容量を消費する
   
2. リクエストは 3 つすべてのウィンドウに十分な残り許容量がある場合にのみ許可される
   
3. ウィンドウがロールオーバーすると、残り許容量は **完全な制限にリセット** される —
   徐々に補充されるわけではない

> ⚠️ これは **トークンバケットではなく**、ウィンドウは **スライディングではありません**。徐々に補充されることはありません: ウィンドウ境界を越えると一度に全許容量が復元されます
> (`src/infrastructure/mongodb-kv/rate-limit.ts` の `trySlowPathConsumeMongo()` はローテーション時に
> `remainingMinute = limits.minute - weight` を割り当てる)。補充モデルを仮定すると誤った結論に至ります — #1806 を参照。そこでは不安定なテストがその根拠に基づいて誤診されました。

### Counter Key — どのテナントが課金されるか (#2221)

counter key は `<tenant>#<servicePath>` で、**`<tenant>` は呼び出し元が宣言したテナントではなく、リクエストが認可されたテナント**です。
...
`/custom-data-models` では、ヘッダーは単に counter で無視されるだけではなく、同じ解決されたテナントがプラン検索、リクエスト/レスポンスサイズ制限、`X-RateLimit-*` レスポンスヘッダー、および API コールメトリクスに使用されるため、すべての会計ディメンションが認可決定で使用されたのと同じテナントを指定します。監査ログもこれを使用しますが、このルートでは無効です。`resolveAuditAction` は `/custom-data-models` パスをマッピングしないため、そもそもここでは監査エントリが出力されません。

代わりにヘッダーでそのルートをキーイングすると、呼び出し元は (a) 別のテナントを指定することでその**他の**テナントの counter を枯渇させたり、(b) すべてのリクエストで異なるテナントを指定してキーを分散させることで**自身の**制限を回避したり、(c) 存在しないテナントを指定することでチェックを完全にスキップしたりできるようになります(プラン検索が `null` を返し、ブロック全体がバイパスされます)。

> テナントごとの**同時実行数**制限は、依然として `/custom-data-models` を除外しています (#1510)。この除外の元々の理由である未検証のキーは、もはや適用されませんが、これを有効にすると、このルートに新しい `429` が導入され、これは正確性の修正ではなく、容量ポリシーの決定です。

### Endpoint Weights

異なるエンドポイントには、処理コストに基づいて異なる重みが割り当てられます:
...
受信リクエストパスは、`resolveEndpointWeight()` (`src/core/quotas/rate-limit/rate-limit.constants.ts`) によってこれらのエンドポイントテンプレートと照合され、各 `{placeholder}` を単一のパスセグメントとして扱います。これは、`/v2/entities/urn:ngsi-ld:Store:001` のような実際の本番パスが `DEFAULT_WEIGHT` にフォールバックするのではなく、`GET /v2/entities/{id}` の重みに正しく解決されることを意味します (#1521)。

### Burst Allowance

各プランには、短期間の突然のトラフィックスパイクを処理するためのバースト許容量があります。これにより、制限を一時的に超えることができます。

このバースト許容量は、常にレート制限の事実上のソフトリミットでした。*拒否*境界は `perMinute/perHour/perDay + burstAllowance` であり、名目上の制限だけではありません。#1571 で変更されたのは**可観測性**のみです。バースト許容量内にいる間に名目上の制限を超えることは、`X-Quota-Soft-Exceeded` レスポンスヘッダーによって表面化されるようになりました。下記の [Soft Limits (Grace Band)](#soft-limits-grace-band-1571) を参照してください。
...
### Public (未認証) Endpoint のレート制限 (#1075)

認証なしで到達可能な公開エンドポイントは、テナント単位の `QUOTAS.PLANS` とは独立した IP ベースの固定ウィンドウカウンターによって保護されます。これにより OAuth `client_id+secret` のブルートフォースと、大量の JSON 生成 (`/openapi.json` など) による DoS をブロックします。
...
`publicDereference` は `metadata` とは別のバケットであるため、フェデレーションピアや RDF クローラーによる語彙や `@context` の一括参照が 30/分 の metadata 上限に到達することはありません。1 日あたりのクォータは意図的にサブリニア (`perDay < perHour × 24`、`auth` / `apiKeyExchange` と同じ比率系列) です。目的は DB 負荷の軽減のみであり、存在オラクル制御はレート制限の範囲外です (同じ情報は既に未認証の `jsonldContexts` 経由で到達可能)。
...
* `/auth/login` はこの制限の対象**外**。`LoginProtectionService` (メール + IP ベースの段階的ロックアウト) によって保護される。
  
* `/health`、`/health/live`、`/health/ready`、`/version` はこの制限の対象**外**(ヘルスチェックポーリング用)。
  
* バケットストア (DynamoDB / MongoDB) が利用不可の場合、リクエストは通過を許可される。パブリックサーフェスをオフラインにしないため、インフラエラーでフェイルクローズしない。
...
上記のストレージクォータは**ドキュメント**数をカウントし、バイト数ではありません。それとは独立して、各 temporal データポイント (1 つの MongoDB ドキュメント) は `TEMPORAL.MAX_DATA_POINT_BSON_BYTES` (**64 KiB**、全プラン) で上限が設定されており、挿入直前 (暗号化および `location` / `expiresAt` などの派生フィールド後) に構築されたドキュメントに対して `BSON.calculateObjectSize` で測定されます。これはアドミッションガードであり、テナント全体のバイトバジェットではありません (バイトバジェットは #1559 待ち)。

**エントリーポイント別のリジェクション形態:**
...
**Metrics:** この上限を超えると `recordQuotaViolation('requestSize')` 経由で `requestSizeExceeded` がインクリメントされます。このカウンターは**粒度が混在**しています — HTTP ボディサイズのリジェクトは**リクエストごと**ですが、temporal のデータポイントごとのリジェクトは**データポイントごと**です。`addAttributeInstances` / マルチポイント構築では、バッチが失敗する前にすべての過大なドキュメントが測定されるため、1 回の呼び出しで N 個の過大なポイントがあるとカウンターは **N** 回インクリメントされます (その後、最初のサイズエラーが再スローされ、何も挿入されません)。

**暗号化テナント:** 測定されるドキュメントは**保存された**形式です。エンベロープ暗号化により `value` が base64 暗号文フィールドに置き換えられます (約 1.33 倍の膨張)。そのため、実効的な平文上限は低くなり、目安として約 **48 KiB** です。

**レガシーの過大なポイント:** PATCH は完全な保存ドキュメントを再構築し、その**再構築された** BSON サイズが 64 KiB 以下の場合にのみ許可されます。`value` を縮小するのが通常の修正方法ですが、その他の削減 (例: `attrMetadata` のクリア) もカウントされます — 過大な再構築を残す `observedAt` のみの PATCH でも 413 を返します。エラーメッセージは、リクエストボディサイズではなく、**保存されたデータポイントサイズ**を参照します。

**カウントとサイズの両方がリジェクトする場合の優先順位**: ハンドラーのカウントベースのストレージクォータチェックが最初に実行され → **507** `InsufficientStorage` になります。サイズは挿入時にのみチェックされるため、すでにポイント数を超えているリクエストは BSON ガードに到達しません。
...
#### エンティティカウントのアドミッションは `countDocuments` ではなく近似カウンターを使用します (#3155)

ENTERPRISE スケールの `maxEntities` (250M、#3141 後) では、共有 `entities` コレクションに対するライブ `countDocuments` は 3 秒のインタラクティブ `maxTimeMS` 内に完了できません — `docs/PERFORMANCE_ASSESSMENT.md` のカウントラダーでは、1000 万マッチドキュメントから `MaxTimeMSExpired` が測定され始め、関係はほぼ線形です (スキャンを回避できるインデックス形状はなく、2 億 5000 万に外挿すると約 75–95 秒)。そのスケールでのリクエストごとの `countDocuments` は、テナントが実際に制限に近いかどうかに関係なく、大規模テナントのすべての書き込みを 503 でフェイルクローズさせることになります。

エンティティアドミッション (`StorageQuotaRepository.getEntityCount`) は、代わりにテナントごとの近似カウンター (`entityCounters` コレクション、`_id` = テナント名) をインクリメンタルに維持して読み取ります: 物理ドキュメント数を変更するすべての書き込みパス (`create`、`batchCreate`、`batchUpsert`、`batchReplace`、`delete` (ハード)、`deleteMany`、`deleteDuplicates`) は、リクエストされたカウントではなく、**実際の** MongoDB 書き込み結果 (`insertedCount` / `deletedCount` / `upsertedIds` サイズ) によってカウンターを調整します — したがって、部分的な `ordered:false` バッチ失敗も正しくカウントされます。TTL ベースの物理的な有効期限切れ (MongoDB のネイティブ TTL モニター、`idx_entity_ttl` 上) にはアプリケーションフックがなく、リアルタイムでデクリメントされ**ません**。これはカウンターを上方向にのみドリフトさせ (過剰カウント)、アドミッションに対してはフェイルクローズ方向です (テナントがクォータを超えることを許しません)。

コールドテナント (カウンタードキュメントがまだない — 例: この機能のデプロイ直後の既存テナント) は、元の `countDocuments` パスに一度だけフォールバックし、結果から遅延的にカウンターをシードするため、テナントごとの最初のアドミッションチェックのみがフルスキャンコストを支払います。ドリフト (TTL 有効期限切れや見逃されたエッジケースによる) は `QuotaMonitoringSweeper` (`rate(1 hour)`) によって修正されます。これはクライアントに見えるリクエストパスの外で実行されるため、はるかに大きな `maxTimeMS` (`QUOTAS.ENTITY_COUNTER_RECONCILE_MAX_TIME_MS`、4 分) で再カウントし、カウンターを権威ある値に `$set` します。

使用量表示 (`getStorageUsage`、管理クォータ情報エンドポイントと同じスイーパーで使用) は、同じカウンター対応パスを通じてエンティティを読み取ります — サブスクリプション/レジストレーション (小さな上限、≤2000/≤1000) と temporal (既にバケット最適化済み、以下参照) のみが、直接 `countDocuments` を使用します。

### レスポンスヘッダー

`X-Storage-Quota-*` レスポンスヘッダーは**実装されていません**。ヘッダービルダー (`buildStorageQuotaHeaders`) は未使用のコードとしてのみ存在し、削除されました (#2604)。現在の使用状況と制限は、[管理 API](#management-api) (`GET /admin/tenants/{tenantId}/quotas`) から利用可能であり、NGSIv2 / NGSI-LD / Catalog のレスポンスヘッダーからは利用できません。

レスポンスヘッダーが後で実装される場合、それは公開 API 契約の変更であり、明示的な設計決定が必要です(使用カウントがタイムアウトした場合のフェイルクローズとフェイルオープンを含む)。

### ストレージクォータを超過した場合の動作

ストレージクォータは**ソフトリミット**です (#1571): プランの公称制限を超える使用も、拒否される前のグレース上限まで許可されます。2 層モデルについては、以下の[ソフトリミット(グレースバンド)](#soft-limits-grace-band-1571)を参照してください。グレース上限を超える使用のみが拒否されます:


* **HTTP ステータスコード**: `507 Insufficient Storage`
  
* **エラーメッセージ**: リソースタイプ、現在の使用状況、プランの公称制限、実際に適用されたグレース上限を含みます
  
* **例**: `{"error": "InsufficientStorage", "description": "Storage quota exceeded for entities. Current: 12000, Limit: 10000 (soft limit; grace ceiling 12000), Requested: 1", "details": {"resourceType": "entities", "current": 12000, "limit": 10000}}` — `details.limit` は常にプランの**公称**制限であり、グレース上限ではないことに注意してください。上限はメッセージテキストにのみ表示されます。

### 時系列データポイントクォータと Entity デュアルライト (#2508)

`maxTemporalDataPoints` は Temporal Evolution インスタンスに上限を設定します。`TEMPORAL_ENTITY_DUAL_WRITE=true` の場合、Entity API の書き込みもこのクォータを消費します。オーバーフロー時でも **Entity の書き込みは成功します** (2xx)。時系列への追記のみがドロップされ、warn ログと `recordQuotaViolation('storageQuotaDropped')` が記録されます。これは `storageQuotaExceeded` (507 拒否) とは別にカウントされます。上限に達した後は履歴にギャップが発生します。Entity API のステータスコードではなく、クォータメトリクス / アラートを監視してください。長期実行デプロイメントでは `TEMPORAL_DATA_RETENTION_DAYS` (TTL) との併用を推奨します。

**バッチデュアルライトアドミッション (#2549):** デュアルライトを遅延する Entity バッチ書き込み (`entityOperations/update`・`merge`、MCP `replace`、および既存の create/upsert/bulk-update バッチパス) では、時系列クォータはバッチ内の属性ポイントの合計で **一度だけ** チェックされます (`maybeRecordTemporalEvolutionBatch`)。この単一のアドミッションが失敗した場合 (クォータ超過またはカウント `maxTimeMS` 超過)、**バッチ全体のデュアルライトがドロップされます** — 同一リクエスト内の前の方の Entity に対する部分的な時系列追記は行われません。(Entity 単位の Entity 書き込みは引き続き成功します。) #2549 以前は、`entityOperations/update`・`merge` と MCP `replace` は Entity ごとにアドミッションしていたため、同一リクエスト内の前の方の Entity は残クォータがなくなるまで履歴を追記できました。create/upsert バッチパスは既に全成功または全失敗のアドミッション契約を使用していました。#2549 は遅延パスをこれに整合させます。

時系列ポイントのアドミッションは **`maxTimeMS` のみ**を指定した `countDocuments` を使用します (`getTemporalDataPointCountForAdmission`)。**`$limit` は追加しないでください** — MongoDB 8.0 ではバケットレベルのカウント最適化が無効になり、クォータ未満のテナントが遅くなります (#2508 SECREVIEW4)。カウントがタイムアウトした場合、HTTP アドミッションは **503** で拒否し、`recordQuotaViolation('storageQuotaCheckTimedOut')` を記録します。Entity→Temporal デュアルライトがカウントタイムアウトでドロップされた場合も同じカウンターがインクリメントされます (Entity 書き込みは引き続き成功)。507 拒否と `storageQuotaDropped` の両方とは別です。使用量表示 (`getStorageUsage`) は時系列カウントに同じ `maxTimeMS` を適用します (`$limit` なし) (#2562)。タイムアウトはフェイルクローズで伝播します (管理クォータ / 監視 → 503)。部分的な使用量オブジェクトは返されません。

レート制限 (上記) は **インバウンド** API リクエストを制限します。#1544 まで、**アウトバウンド**通知のファンアウトを制限する次元はありませんでした — 少数の書き込みリクエストが、多数の一致 Entity を持つサブスクリプション、または多数のサブスクリプションをトリガーし、プランに紐付いた上限なしに無制限の数の HTTP/MQTT 通知配信にファンアウトする可能性がありました。

> **これらの値はエンジニアリングデフォルトであり、価格調整の対象です** (#1544 の裁可待ち、2026-08-15)。意図的に、顧客向けの `docs/customer/PLAN_QUOTAS.md` にはまだ公開して**いません** — #1544 は、この数値を ENTERPRISE サブスクリプション制限の再設計と合わせて決定する必要があると述べています。その批准前に定数を変更しても、顧客に見えるコミットメント変更にはなりません。


* **新しいクォータディメンション**: `rateLimit.maxNotificationsPerDay` (`QUOTAS.PLANS[*]`、`src/config/defaults.ts`) — FREE=50,000 / STANDARD=800,000 / PREMIUM=1,500,000 / ENTERPRISE=3,000,000 (各プランの `rateLimit.perDay` と同じ値。1 回の通知配信のコスト — EventBridge + Lambda ×3 + SQS + Webhook 宛先への egress + CloudWatch Logs — は 1 回のインバウンド書き込みリクエストと同じオーダーであるため。この制限の意図は、通知駆動コストがインバウンドリクエスト駆動コストを上回らないようにすることです)。`customQuotas.rateLimit.maxNotificationsPerDay` で上書き可能 (非負、最大 100,000,000 — [Configure Custom Quotas](#configure-custom-quotas) 参照)。`0` はそのテナントのすべての通知配信を明示的に停止します。
  
* **ウィンドウ**: UTC カレンダー日 (`YYYY-MM-DD`)、7 日間の TTL を持つ MongoDB 日次バケット (`notificationCounters`、コントロールプレーン側) で追跡されます。
  
* **消費ポイント**: 送信直前、通知ファンアウトパス上 (トリガーとなる書き込みではありません)。**インプロセスリトライ** (`retryWithBackoff`) は、ゲートがリトライループの外側にあるため、予算を一度だけ消費します。**Lambda パス上の SQS 再配信は再度消費します** — `deliverNotification` はすべてのインプロセスリトライが失敗した後に throw し、それが SQS に再配信させ、再配信されたメッセージはゲートに再入します。exactly-once のアカウンティングは at-least-once キューでは達成できません。カウンターは配信の上限であり、異なる通知の下限です。カウンターは **ゲートを通過して実際にディスパッチされた通知に対してのみ** 進みます — クォータが拒否した通知はカウントされないため、保存される値は制限を超えてインフレートせず、コスト見積もりに直接使用できます。その後失敗したディスパッチもカウントされます。ファンアウトコスト (Lambda 実行時間、アウトバウンド試行) は既に発生しているためです。
  
* **超過時の動作**: 通知は**ドロップ**され、警告がログに記録され (`Notification dropped: daily notification quota exceeded`)、メトリクス (`notificationQuotaExceeded`) が発行されます。**サブスクリプション自体は無効化されません** — クォータ枯渇で無効化すると、コストガードレールを意図したものに対して大きな可用性への影響範囲になります。通知配信はトリガーとなる API リクエストの外側で発生するため、この拒否に対する HTTP レスポンスはありません。
  
* **fail-open**: カウンターストアが利用できない場合、通知は通過を許可されます — インフラストラクチャの一時的な障害で通知配信をオフラインにしないために、レート制限 (#1492) で既に行われている選択と同じです。
  
* **可観測性**: `GET /admin/tenants/{tenantId}/quotas` の `currentUsage` には、`rateLimit` と `storage` に並ぶ 3 番目の独立したディメンションが含まれます: `notifications.day = {used, limit, remaining, usagePercent, date}`。
  
* **スコープ**: 通常のサブスクリプション通知 (HTTP/MQTT、Lambda およびスタンドアロンパス) **とコンテキストソースサブスクリプション通知**の両方 (`csource-notification.service.ts`、独自の `fetch` で外部エンドポイントに登録変更をファンアウトします)。後者を除外すると、`csourceSubscription` が同じ制限の無制限バイパスになってしまいます。
  
* **既知の制限**: この上限は HTTP/MQTT 経由のサブスクリプション通知配信にのみ適用されます。WebSocket/SSE イベント配信は別のメカニズムであり、この制限の対象ではありません。

### テナントごとの通知同時実行数 (#2269 / #1544 AC3)

遅い webhook 宛先が**他のテナント**の通知配信を遅延させてはなりません。`NOTIFICATION.MAX_CONCURRENT` はプロセス / Lambda 呼び出しの制限であり、**テナント次元がありません**。第二の制御プレーンがない場合、1 つのテナントの遅い HTTPS ラウンドトリップがすべての送信中スロットを占有する可能性があります。
...
* **名前空間の分離**: 通知スロットは `#notification-concurrency` を使用し、API パスの `#concurrency` (#1510) とは区別されます。一方を埋めてももう一方を枯渇させてはなりません。
  
* **順序**: 同時実行スロットを**先に**取得してから日次通知クォータを消費することで、同時実行数の低下が日次カウンターを膨張させないようにします。
  
* **パス**: Lambda `notifier.ts`、スタンドアロン `notification-delivery.ts`、csource `csource-notification.service.ts` はすべて `acquireNotificationSendSlot` / `withNotificationSendSlot` を共有します。
  
* **リース**: `NOTIFICATION.CONCURRENCY.LEASE_MS` (150s) は `TIMEOUT_MS` × リトライ + バックオフをカバーし、送信中のものが奪われないようにします。
  
* ストア障害時には **fail-open** (#1510 / 日次通知クォータと同じ)。
...
> **GeonicDB 拡張機能。** これは NGSIv2/NGSI-LD で義務付けられているものではありません — これは GeonicDB が `docs/customer/PLAN_QUOTAS.md` §5(「上限は安定運用の目安であり、超過した瞬間にご利用が停止するものではありません」)でなされた約束を果たす方法です。
...
**グレースバンドは無制限ではありません。** ハード上限は依然として適用されるため、これは #1485(テナントごとの同時実行数 / DB 過負荷ブレーカー)の DB 保護保証や #1454 の結果サイズ強化を損なうものではありません — グレースバンドは短期間のバーストを吸収しますが、持続的なクォータ超過使用を許可するものではありません。

### 次元ごとに上限がどのように計算されるか


* **ストレージ(entities / subscriptions / registrations / temporalDataPoints)**: 上限は `Math.floor(nominal limit × QUOTAS.SOFT_LIMIT.STORAGE_GRACE_MULTIPLIER)`(`src/config/defaults.ts`、multiplier = `1.2`、つまり 20% のグレース)です。結果が切り捨てられるため、公称制限が小さいほど、グレースバンドの絶対値は縮小します — 最小の実プラン値は T0 の `maxSubscriptions=10` で、それでも非ゼロの上限(`12`)が得られます。人為的に小さい制限(テストなど)のみが、グレースバンドを完全に切り捨てることができます。`StorageQuotaService.checkQuota()`(`src/core/quotas/storage/storage-quota.service.ts`)で適用されます。
  
* **レート制限(minute/hour/day)**: **拒否境界は移動していません**。[Burst Allowance](#burst-allowance) で述べたように、`perMinute/perHour/perDay + burstAllowance` が常に実効上限でした — グレースバンド(公称制限を超えたが、まだバースト許容量内)は以前から存在していましたが、観測可能ではありませんでした。#1571 は、その既存のバンドへの可視性を追加するだけです。3 つのウィンドウすべてが評価され、グレースバンド内にある最も細かい粒度のものが報告されます(`src/core/quotas/rate-limit/rate-limit.service.ts` の `resolveSoftExceededWindow()`)。
  
* **テナントごとの同時実行数は対象外であり、変更されていません。** `docs/customer/PLAN_QUOTAS.md` §4 では、同時実行数について「超過分は一時的に待機・再試行の対象になります」とすでに説明されており、既存の `429 + Retry-After` 動作(`QUOTAS.CONCURRENCY.RETRY_AFTER_SECONDS`、[Per-Tenant Concurrency Limit](#per-tenant-concurrency-limit-1510--epic-1485) を参照)がすでにこれを満たしています。これをグレースバンドに拡大すると、#1485 のために導入された DoS 強化が損なわれます。

### `X-Quota-Soft-Exceeded` レスポンスヘッダー

グレースバンド内で受け入れられたリクエストのレスポンスに付加されます。値は、公称制限を超えた次元のカンマ区切りリストです:
...
* レート制限次元にはプレフィックスが付けられます: `rateLimit.minute`、`rateLimit.hour`、`rateLimit.day`。
  
* **拒否されたリクエストにはこのヘッダーは付きません** — 拒否は、ヘッダーが組み立てられる前に `507`/`429` エラーパスへショートサーキットするため、このヘッダーは公称制限を超えた `2xx` レスポンスにのみ表示されます。
  
* ヘッダー名自体は定数 `QUOTAS.SOFT_LIMIT.SOFT_EXCEEDED_HEADER`(`src/config/defaults.ts`)です。

### 持続的なクォータ超過使用の運用シグナル

単一のグレースバンド受け入れは予想され、無害です。このヘッダーを*継続的に*持つテナントは、プランアップグレードまたはカスタムクォータ調整の候補です。これを一時的なバーストと区別できるようにするため:


* すべてのグレースバンド受け入れは `warn` レベルのログ行を出力します(ストレージ: `storage-quota.service.ts`; レート制限: `handlers/api/index.ts`)。
  
* すべてのグレースバンド受け入れは、`quotas.rateLimitSoftExceeded` / `quotas.storageQuotaSoftExceeded`(`src/core/metrics/metrics.service.ts`)でもカウントされます。これらは既存の拒否カウンター(`rateLimitExceeded` / `storageQuotaExceeded`)とは**意図的に分離されています** — それらをマージすると、「実際に拒否されたリクエストの数」と「単にグレースバンド内にあったリクエストの数」を区別できなくなります。

持続的なパターンが*いつ*プランアップグレードの会話を必要とするかを決定することは、運用/営業プロセスであり、このセクションが規定するものではありません。

## モニタリングとアラート

### レート制限バケットシャーディング (#3115)

高 rps テナントは以前、すべての `UpdateItem` を単一の DynamoDB アイテム(`pk={tenant}#unified`、`sk=dayTs`)に集中させていました。パーティションスループット(\~1,000 WCU/s)に加えて `MAX_SLOW_PATH_RETRY=1` が `ThrottlingException` / 競合枯渇を引き起こし → `RateLimitCheckTransientFailure` による **fail-open** が PREMIUM/ENTERPRISE パブリッシュターゲットを大きく下回る結果となりました(ステージング環境で closed-bearer \~800 rps クラスで測定)。

**現在の設計**(`QUOTAS.RATE_LIMIT_SHARD_COUNT`、デフォルト **8**):


* 有効なシャード数:`min(設定された N, floor(min(minute,hour,day) / REPRESENTATIVE_WEIGHT))` により、小さなカスタム / E2E クォータは単一パーティション上に留まり(重み増加 vs 合計不一致を回避)、`N × floor(global/N) ≤ global` となります
  
* 書き込みキー:`pk={tenant}#unified#{shardId}`(`shardId` ∈ `0..N-1`)、ランダムスタート + シャード単位の拒否時に最大 `RATE_LIMIT_SHARD_PROBES` の **追加** プローブ(総試行回数 = `1 + PROBES`、N にクランプ)
  
* シャード単位の公称上限:`floor(global/N)` により `N × perShard ≤ global`; 有効な書き込み上限は `min(global, max(floor(global/N), weight))` となるため、大きな重みでも 1 つのシャードに収まります
  
* `getRateLimitInfo` はすべてのシャード **に加えて** レガシー `pk={tenant}#unified`(読み取り専用; 新しい書き込みはレガシーに触れない)を合計します。**未使用(不在)のシャードはシャード単位の完全な残量としてカウントされます** — それらをスキップすると、まだすべてのシャードに到達していないテナントの残量が過小報告されます
  
* `N > 1` での消費成功は `getInfo` と同じ合計残量を返します(ソフトリミット / ヘッダー精度); `N = 1` は単一バケットの残量を直接返します
  
* **デプロイ / 移行**: レガシー `#unified` の残量は新しいシャードキーに **コピーされません**。デプロイ後、新しい消費は `#unified#{shardId}` のみに触れるため、既存のテナントは実質的に新鮮なシャードプールから開始します(その日のウィンドウの残りに対する一時的な **緩和** — 無制限の拡大ではなく、依然としてプラン制限にクランプされます)。レガシーアイテムは TTL が期限切れになるまで `getInfo` で読み取り可能であり、クランプまで一時的に合計残量を膨らませる可能性があります; オペレーターはデプロイ日にレート制限アカウンティングの一回限りのソフトリセットを予期すべきです

関連しているが別: テナント単位の同時実行スロットは **同じテーブル** 上で `pk={tenant}#concurrency#{slot % N}` を使用します(`N = min(CONCURRENCY.SHARD_COUNT, max)`、#3125)。#3125 以前の単一 pk 形式 `pk={tenant}#concurrency` はレガシーのみです(デプロイ後のリース TTL \~35s)。

### レート制限インフラストラクチャ障害メトリクス (#1419)

レート制限チェックは **fail-open** です: バケットストア(DynamoDB / MongoDB)がエラーを起こした場合、リクエストはブロックされるのではなく許可されます。この障害モードを観察可能に保つため、飲み込まれたすべてのエラーは CloudWatch Metric Filters に適した構造化された `metric` マーカーでログに記録されます:


* `metric: "RateLimitInfrastructureFailure"` — 自己回復しない **永続的** エラー(`ValidationException`、`SerializationException`、`ResourceNotFoundException`、`AccessDeniedException`; `QUOTAS.RATE_LIMIT_PERMANENT_ERROR_NAMES`、`src/config/defaults.ts` のリスト)。これらはコードまたは設定のバグを示します — レート制限は修正されるまでフリート全体でサイレントに無効化されます。**このマーカーは高重要度の CloudWatch アラームに接続すべきです。**
  
* `metric: "RateLimitCheckTransientFailure"` — スロットリング、タイムアウト、およびその他の一時的なストアエラー。

背景: 3 つの連続した DynamoDB 式のバグ(#1385)は、未分化の fail-open キャッチによって数日間隠されていました。分類は `src/core/quotas/rate-limit/rate-limit-failure.ts` に存在し、テナントおよびパブリックレート制限パスで共有されています。

例外(#1685): Mongo(スタンドアロン)パス上では、レート制限バケットのアトミシティは `insertOne` + ユニークインデックス `idx_rate_limit_bucket_unique` に対する重複キー再試行に依存しています — これがないと、重複バケットがレート制限をサイレントにバイパスします。したがって、このインデックスは `MONGODB.CRITICAL_INDEXES` にリストされており、作成に失敗すると **fail-closed** となります(fail-open ランタイムパスに流れるのではなく、インデックス初期化時に接続が拒否されます)。
...
### 容量アドミッションチェック（#1509 / Epic #1485）

手動のクォータの設定ミスによるクラスタの飽和を防ぐため（2026-07-21 インシデントの直接的なトリガーは、単一テナントの `perMinute` が検証なしで引き上げられたことでした）、`PUT /admin/tenants/{tenantId}/quotas` は変更を永続化する前に**単一テナント容量アドミッションチェック**を実行します。

モデル（テナントごとのガードレール — すべてのテナントの集計は**評価されません**）:
...
`perMinute` はすでに**1 分あたりの重みユニット**で表されています（レートリミッターはそれに対して `calculateWeight()` ユニットを消費します）。#3114 は以前の `× REPRESENTATIVE_WEIGHT` の二重計上を削除しました。

このチェックは**有効な**クォータ（リクエストボディをテナントの既存設定にマージしたもの）に対して評価されるため、部分的な更新は結果の値を使用して検証されます。

**オーバーライド（要承認）:** `super_admin` は `acknowledgeOvercommit: true` を設定することで、意図的にガードレールを超えることができます。オーバーライドは高重要度の監査ログに記録されます。`tenant_admin` はオーバーライドできません（実際には、クォータの更新はまったくできません — 変更には `super_admin` が必要です）。
...
> **キャリブレーション注記（#3114）:** `CLUSTER_CAPACITY_WEIGHTED_PER_MIN` は**プレースホルダーのガードレール予算**であり、測定された Atlas スループットではありません。現在の値 `600000` と `MAX_SINGLE_TENANT_SHARE=0.5` により `maxPerMinute=300000` が得られ、#3118 ENTERPRISE/PREMIUM `perMinute` に対するレガシーの約 5 倍のマージン（`300000/60000`）が維持されます（共有 API GW は公開 rps を 1000 に制限、生の ×10 ENTERPRISE は 180000 でした）。病的な値（例: `perMinute=1_000_000`）は依然として拒否されます。`ENDPOINT_WEIGHTS` は経験的キャリブレーション（#1509）を待つ間はヒューリスティックのままです。定数は**コンパイル時のもの — 環境で調整できません**（[Environment Variables](#environment-variables) を参照）。手順: QUOTA\_OPERATIONS.md。
>
> 専用クラスタのバインディング / plan×tier の導出は #1492 / #1559 のままです。
...
**容量許可チェック(#1509 / #1983): 環境変数での調整は不可。**

> ⚠️ **これらはコンパイル時定数であり、環境変数ではありません(#1983)。** このドキュメントの以前のリビジョンでは、`QUOTA_ADMISSION_ENABLED`、`CLUSTER_CAPACITY_WEIGHTED_PER_MIN`、`QUOTA_MAX_SINGLE_TENANT_SHARE`、`QUOTA_ADMISSION_REPRESENTATIVE_WEIGHT` を調整可能な環境変数としてリストしていました。これらは **`infrastructure/template.yaml` に配線されたことはなく**、そのため設定してもデプロイされた環境では一切効果がありませんでした。また、#1983 で `process.env` の読み取りは完全に削除されました。**これらの名前を設定しても何も起こりません。** これは特にインシデント時に重要です。その場合の失敗モードは「変数を設定し、再デプロイし、なぜ何も変わらないのか疑問に思う」というものです。

現在の値は `QUOTAS.ADMISSION`(`src/config/defaults.ts`)にあり、コード変更 + デプロイによってのみ変更されます:
...
これらは **プレースホルダーのガードレールであり、測定された Atlas 容量ではありません**。意図された方向性は、環境変数を再導入するのではなく、**プラン × クラスター層**から導出することです(#1559)。キャリブレーション手順については QUOTA\_OPERATIONS.md を、専用クラスターバインディングについては #1492 を参照してください。
...
**原因**: ストレージクォータ超過 — 使用量がプランの公称制限だけでなく、[グレース上限](#soft-limits-grace-band-1571)を超えた。(公称制限と上限の間の使用量は `X-Quota-Soft-Exceeded` ヘッダー付きで許可され、拒否されない。)
...
3. DynamoDB テーブルが正しくデプロイされていることを確認

## 入力検証の制限

GeonicDB は、悪用を防ぎシステムの安定性を確保するために、入力の長さとカウントの制限を適用します。
...
* 段階的遅延:**2 秒**から開始する指数バックオフ (2^(n-2))

#### IP 毎のログイン保護 (#900)

単一の IP から複数のアカウントへのパスワードスプレー攻撃を防ぎます:
...
* `createUser` と `deleteUser` にテナント毎に適用されます
...
GeonicDB はすべての API エンドポイントで包括的な入力検証を実施します。いずれかの制限を超えた場合、`400 Bad Request` が返されます。
...
> エンティティドキュメントの BSON サイズは**バイト**制限です(要素数ではありません)。
...
保存される各エンティティ(`entities` コレクション内の MongoDB ドキュメント)は **≤ 1 MiB BSON** (`QUOTAS.MAX_ENTITY_DOCUMENT_BYTES`) でなければなりません。この制限は**プランごとの HTTP リクエストボディクォータとは独立しています** — PREMIUM (5 MB) または ENTERPRISE (10 MB) リクエストが入力許可を通過しても、結果のドキュメントが 1 MiB を超える場合は **413 `RequestEntityTooLarge`** を受け取る可能性があります。既に制限に達しているドキュメントの増加は拒否されます(既存の読み取り/削除/縮小は引き続き許可されます)。
暗号化されたテナントでは、現在のドキュメントがまだ制限内にある場合、置換は暗号化**前に**過大な属性ペイロードを拒否します(KMS / DEK ローテーションによる節約)。現在のドキュメントが既に制限を超えている場合、縮小置換はアトミック縮小ガードを介して続行されます。
同じエンティティ id は、1 つのバッチアップサート内に最大 `SECURITY.MAX_ENTITY_BATCH_OCCURRENCE_ROUNDS` (8) 回まで出現できます — それを超えると書き込み前に **400** となります。これは NGSIv2 `/v2/op/update`、NGSI-LD `entityOperations/upsert`(マージと置換)、MCP、および A2A バッチアップサートすべてに適用されます。スナップショット復元は意図的にこのガードをバイパスします(`snapshot.repository.ts` を参照)。
...
* 制限を超えた場合、プリミティブ型 (string、number、boolean、null) のみが受け入れられます
...
操作がエンティティストレージクォータを消費するのは、エンティティを**作成**できる場合です。バッチ更新の場合、これは、リクエストが upsert パスに到達するかどうかによって決まります:
`EntityService.batchUpdateEntities()` は `append` / `appendStrict` / `update` を `batchUpsert()` (upsert: true を指定した `bulkWrite` なので、存在しない id は挿入されます) に送り、`replace` / `delete` のみをエンティティごとのループ (`replaceEntityAttributes` / `deleteEntity`) に送ります。
...
* 未知の `actionType` は作成として扱われます (fail-closed); リクエストはその後バリデーションによって拒否されます

同じルールがすべてのエントリポイントに適用されます。MCP / A2A の `batch` ツールは `create` / `upsert` / `merge` および `update` を同じ upsert パスにマッピングするため、これらは HTTP の `/v2/op/update` と同じようにクォータを消費します。`update` が除外されるのは、その `actionType` が非 upsert 値のいずれかである場合のみです — 分類器は HTTP パスと `batchActionTypeCreatesEntities()` を共有しているため、それは `"replace"` または `"delete"` です。実際には `"replace"` のみが到達可能です: MCP の `batch` ツールスキーマは `actionType: "update" | "append" | "replace"` を受け入れるため、`"delete"` はディスパッチされる前に入力バリデーションによって拒否されます (A2A の `batch` スキルには `update` アクションがまったくありません)。

履歴: `/v2/op/update` は元々、`actionType` に関係なく**すべての**リクエストをカウントしていました; #902 はそれを `append` / `appendStrict` に絞りました。これは狭すぎました — `update` も upsert するため、#2185 (HTTP) と #2183 (MCP `batch` の `update` / `merge`) が「作成できるか?」ルールを復元し、決定をすべてのエントリポイントで共有する単一のヘルパーに移動するまでカウントされないままでした。

### JSON-RPC バッチボディ (#2182)

`/mcp` は JSON-RPC バッチとして**トップレベル JSON 配列**を受け入れ、すべての要素を実行します。したがって、クォータ分類は各要素を検査し、制限をチェックする前にリソースディメンションごとおよびテナントごとに消費量を**合計**します。合計がなければ、各書き込みを独自の配列要素でラップすることで、合計が制限を超えていても各要素が独自のチェックを通過できます (使用量はまだ書き込まれていません)。プランレベルの `maxBatchSize` も同じ方法で要素全体で合計されます。

## 関連ドキュメント


* Quota Operations Guide - Ops ランブック: クォータの引き上げ/診断、plan↔tier↔constant マッピング、キャリブレーション手順


* Development & Deployment Guide - インフラストラクチャのセットアップ
