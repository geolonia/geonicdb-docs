...
クォータシステムは 3 つの主要なコンポーネントで構成されています:


1. **レート制限システム** - DynamoDB ベースのトークンバケットアルゴリズムを使用した API リクエスト制限
   
2. **ストレージクォータシステム** - MongoDB に基づくエンティティ/サブスクリプション/レジストレーション/時系列データの件数制限
   
3. **モニタリング & 管理システム** - 使用状況の追跡、アラート配信、管理 API
...
2. リクエストは、3 つのウィンドウすべてに十分なトークンがある場合にのみ許可されます
   
3. トークンは各時間ウィンドウの切り替わり時に自動的に補充されます

### エンドポイントの重み

処理コストに基づいて、異なるエンドポイントに異なる重みが割り当てられます:
...
各プランには、短期間の突発的なトラフィックスパイクに対処するためのバースト許容量があります。これにより、一時的に制限を超えることができます。
...
認証なしでアクセス可能なパブリックエンドポイントは、テナント別の `QUOTAS.PLANS` とは独立した IP ベースのトークンバケットによって保護されます。これにより、OAuth の `client_id+secret` のブルートフォース攻撃や、大量の JSON 生成 (`/openapi.json` など) による DoS を防ぎます。
...
* バケットストア (DynamoDB / MongoDB) が利用できない場合、リクエストは許可されます。パブリックサーフェスをオフラインにしないよう、インフラストラクチャエラー時にフェイルクローズしません。
...
ストレージクォータは作成操作の**前**にチェックされます:


* バッチ操作の場合、すべてのエンティティがクォータ内に収まる場合にのみ実行が続行されます
  
* 1 つでもクォータを超える場合、操作全体が拒否されます(全か無か)

### レスポンスヘッダー

NGSIv2、NGSI-LD、Catalog API エンドポイントには、現在のストレージ使用状況を示すヘッダーが含まれます:
...
**対象エンドポイント**: これらのヘッダーは、NGSIv2、NGSI-LD、Catalog API のすべてのエンドポイントによって返されます。管理 API (`/admin/tenants/{tenantId}/quotas`) を使用すると、より詳細なクォータ情報を取得できます。詳細については、[認証と認可](../reference/auth.md#quota-management) を参照してください。
...
* **tenant\_admin**: 自分のテナントのクォータを表示および変更可能
...
* リアルタイム通知を受け取るために Webhook URL を設定
...
1. 不要なエンティティ/サブスクリプション/レジストレーションを削除する
...
* アカウント毎の最大ログイン失敗試行回数:**15 分**以内に **5 回**
  
* アカウントロック期間:しきい値到達後 **15 分間**
...
* ログイン成功時は IP カウンターをリセットしない(タイミングベースの列挙を防止)
...
* `tenant.settings.maxUsers` によるテナント毎のオーバーライド
...
以前は、すべての `/v2/op/update` リクエストが `actionType` に関係なく、誤ってエンティティ作成クォータに対してカウントされていました。
