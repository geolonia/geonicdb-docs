...
- [1Password CLI](https://developer.1password.com/docs/cli) (`op`) — シークレット注入用 (推奨)## セットアップ
...
機密情報 (MongoDB 認証情報、JWT シークレット、管理者パスワード) は **`geonic-ops`** 1Password ボルトに保存され、1Password CLI を介して実行時に注入されます。`.env.op` ファイル (リポジトリにコミット済み) には `op://` URI 参照のみが含まれ、実際の値は含まれません。
...
確認: `op --version`#### 1Password へのシークレット追加

**`geonic-ops`** ボルトに **`geonicdb-dev`** という名前のアイテムを以下のフィールドで作成します:

| フィールド | 説明 |
|-------|-------------|
| `MONGODB_URI` | 認証情報を含む完全な MongoDB 接続文字列 |
| `JWT_SECRET` | ランダムな文字列、32 文字以上 |
| `SUPER_ADMIN_EMAIL` | ローカル開発用スーパー管理者メールアドレス |
| `SUPER_ADMIN_PASSWORD` | ローカル開発用スーパー管理者パスワード |
...
非機密変数 (`ENVIRONMENT`、`AWS_REGION`、`LOG_LEVEL` など) は、通常通りローカルの `.env` ファイルから読み込まれます。

> **注意:** `.env.op` はコミットしても安全です。`.env` と `.env.local` は gitignore 対象であり、決してコミットしてはいけません。`npm run dev:op` を使用する場合、`.env` に `MONGODB_URI`、`JWT_SECRET`、`SUPER_ADMIN_EMAIL`、または `SUPER_ADMIN_PASSWORD` を定義しないでください。これらは 1Password によって注入されるため、両方に実際の値があると混乱を招く可能性があります。
...
| コマンド | 説明 |
|---------|------|
| `npm start` | ローカル開発サーバーの起動 (インメモリ MongoDB を使用) |
| `npm run dev:op` | 1Password (`geonic-ops` ボルト) からシークレットを注入して開発サーバーを起動 |
...
このプロジェクトでは 2 つのタイプのテストフレームワークを使用しています:
...
HTML レポートは `coverage/lcov-report/index.html` で確認できます。## 統合アプリケーションの開発 (npm パッケージとして使用)
...
`npx geonicdb` コマンドで GeonicDB をスタンドアロンで起動できます。`--proxy` オプションを使用すると、GeonicDB のルートにマッチしないリクエストをアプリケーションの開発サーバーに転送します。
...
`concurrently` を使用して GeonicDB とアプリケーションの開発サーバーを同時に起動できます。`--kill-others` を使用すると、1 つのプロセスが終了した場合、もう 1 つのプロセスも自動的に停止します:
...
Vite 側にプロキシ設定を追加することで、どのポートにアクセスしても API を使用できるようになります:
...
JavaScript/TypeScript から直接サーバーを起動および制御することもできます:
...
CI/CD 環境では、GitHub Personal Access Token または Deploy Key を設定する必要があります。チーム全体での運用を考える場合は、GitHub Packages への公開も検討してください。## ローカル開発サーバー

### シンプルサーバー (推奨)

`npm start` を使用して、インメモリ MongoDB でローカルサーバーを起動できます。外部の MongoDB インスタンスは不要です。
...
指定されたポートが使用中の場合、次に利用可能なポートが自動的に選択されます (最大 10 ポートまで検索)。

> **ヒント**: git worktrees と組み合わせて、異なるブランチのサーバーを同時に起動できます:
> ```bash
> # メインの worktree で起動
...
AWS SAM CLI を使用して API をローカルでテスト:
...
## MCP (Model Context Protocol) を介した Claude Desktop からの接続

ローカルサーバーが起動している状態で、Claude Desktop からコンテキストブローカーに直接接続できます。
...
**認証が有効 (`AUTH_ENABLED=true`) の場合**、Bearer トークンヘッダーを指定する必要があります:
...
以下のように Claude Desktop とチャットすると、コンテキストブローカーツールが自動的に呼び出されます。
...
| 409 | Conflict | エンティティ ID が既に存在する |
...
新しい環境 (ステージングまたはプロダクション) にデプロイした後、データベースにはスーパー管理者ユーザーが存在しません。MongoDB 経由で直接作成、更新、または削除するには、`scripts/create-super-admin.ts` を使用してください。

### 前提条件

- Node.js + npm の依存関係がインストールされていること (`npm ci`)
- 1Password CLI がインストールされていること (ステージング/プロダクションの場合)、または `MONGODB_URI` が明示的に設定されていること
...
各環境アイテムには以下のフィールドが必要です:

| フィールド                  | 説明                      |
|------------------------|----------------------------------|
| `MONGODB_URI`          | Atlas 接続文字列          |
| `SUPER_ADMIN_EMAIL`    | Bootstrap スーパー管理者のメール      |
| `SUPER_ADMIN_PASSWORD` | Bootstrap スーパー管理者のパスワード   |
...
CLI 引数 (`--email`、`--password`) は常にボルトの値より優先されます。

### 注意事項

- 既存の `super_admin` ユーザーに対して再実行しても安全です — パスワードがその場で更新されます。メールが異なるロールで存在する場合、意図しない権限昇格を防ぐためスクリプトは中止されます。
- パスワードはアプリケーションのパスワードポリシー (大文字、小文字、数字、特殊文字) を満たす必要があります。
- ユーザーを作成した後、`POST /auth/login` 経由でログインが機能することを確認してから使用してください。
...
GeonicDB は AWS SAM (Serverless Application Model) アプリケーションとしてデプロイされます。すべてのリソースは `infrastructure/template.yaml` で定義され、`sam deploy` コマンドを使用して CloudFormation スタックとしてデプロイされます。
...
- MongoDB Atlas でクラスターを作成 (M0 Free Tier で十分)
- ネットワークアクセス: 開発環境では `0.0.0.0/0` を許可。プロダクションでは、固定の送信 IP のために VPC + NAT Gateway (Elastic IP) を使用するか、Atlas へのプライベート接続のために AWS PrivateLink / VPC Peering を使用
...
`samconfig.toml` は `.gitignore` に含まれており、コミットすべきではありません (シークレットが含まれる可能性があります)。環境ごとに作成してください。
...
ステージング環境は、`main` へのマージごとに `.github/workflows/deploy.yml` を介して自動的にデプロイされます。ワークフローは最初に完全な CI スイートを実行し、その後 `staging` GitHub Environment にデプロイします (承認ゲートは不要です)。

#### 必要な GitHub Actions のシークレットと変数

すべてのシークレットと変数は、リポジトリではなく、それぞれの GitHub Environment (`staging`、`production`) にスコープされています。つまり、同じシークレット名 (例: `DEPLOY_ROLE_ARN`) は環境ごとに異なる値を持ち、`environment: <name>` を宣言したジョブからのみアクセス可能です。
...
デプロイターゲットごとに `Settings → Environments` で 1 つの環境を作成します:
- `staging` — 必須レビュアーなし; `main` へのプッシュごとに自動デプロイ
- `production` — 必須レビュアーを追加; `v*.*.*` タグでデプロイ ([本番デプロイ](#production-deploy) を参照)

#### 機密性の低いステージングパラメータ

機密性の低い値 (リージョン、環境名、機能フラグ) は `infrastructure/parameters/staging.json` から読み取られ、デプロイ時に `sam deploy --parameter-overrides` に渡されます。

#### 本番デプロイ

本番デプロイ (マルチリージョン、手動承認ゲート) はタグトリガー (`v*.*.*`) で実行され、本番パイプラインの実装後に別途ドキュメント化されます。### 環境変数とパラメータ一覧

すべてのパラメータは `infrastructure/template.yaml` の Parameters セクションで定義されています。値は `samconfig.toml` の `parameter_overrides` を介して設定されます。
...
| パラメータ | 環境変数 | デフォルト | 説明 |
|-----------|---------|-----------|------|
| `AuthEnabled` | `AUTH_ENABLED` | `true` | 組み込み認証を有効化 |
...
| `AdminAllowedIps` | `ADMIN_ALLOWED_IPS` | `''` | 管理 API 許可 IP (カンマ区切り CIDR) |
| `AuthzDefaultDecision` | `AUTHZ_DEFAULT_DECISION` | `Deny` | XACML デフォルト認可判定(`Permit` / `Deny`) |
...
| パラメータ | 環境変数 | デフォルト | 説明 |
|-----------|---------|-----------|------|
| `OidcEnabled` | `OIDC_ENABLED` | `false` | OIDC 外部 IdP 認証を有効化 |
| `OidcIssuerUrl` | `OIDC_ISSUER_URL` | `''` | OIDC Issuer URL |
| `OidcAudience` | `OIDC_AUDIENCE` | `''` | 期待される audience (aud) クレーム |
...
| パラメータ | 環境変数 | デフォルト | 説明 |
|-----------|---------|-----------|------|
| `OAuthEnabled` | `OAUTH_ENABLED` | ~~`true`~~ | **非推奨** (#672): `AUTH_ENABLED=true` の場合、OAuth 2.0 は常に有効です。この変数は無視されます。 |
...
| パラメータ | 環境変数 | デフォルト | 説明 |
|-----------|---------|-----------|------|
| `RateLimitEnabled` | `RATE_LIMIT_ENABLED` | `true` | API レート制限を有効化 |
...
| `MaxEntitiesPerRequest` | `MAX_ENTITIES_PER_REQUEST` | `1000` | リクエストあたりの最大エンティティ数(タイルクラスタリング閾値) |
...
詳細は [docs/EVENT_STREAMING.md](../features/subscriptions.md) を参照してください。

| パラメータ | 環境変数 | デフォルト | 説明 |
|-----------|---------|-----------|------|
| `EventStreamingEnabled` | `EVENT_STREAMING_ENABLED` | `true` | WebSocket イベントストリーミングを有効化 |

#### OpenTelemetry

詳細は docs/INTEGRATIONS.md を参照してください。

| パラメータ | 環境変数 | デフォルト | 説明 |
|-----------|---------|-----------|------|
| `OtelEnabled` | `OTEL_ENABLED` | `false` | OpenTelemetry 分散トレーシングを有効化 |
...
以下の環境変数はスタック内のリソース参照から自動的に設定されるため、手動で指定する必要はありません:
...
GeonicDB は Active-Passive アーキテクチャによるマルチリージョン HA をサポートしています。

- **プライマリ**: ap-northeast-1 (東京)
- **セカンダリ**: ap-northeast-3 (大阪)
- **アーキテクチャ**: Active-Passive (SQS FIFO のクロスリージョン制約による)
- **目標値**: RPO < 1 分、RTO < 5 分、可用性 99.99%
...
- Change Stream プロセッサ
- サブスクリプションマッチャー / 通知送信機能
- SQS 通知キュー### 必要なサービスプラン

マルチリージョン HA は、シングルリージョンデプロイメントと比較して、より高いティアのサービスプランが必要です。

#### MongoDB インデックス (テナントスコープのユニーク制約)

GeonicDB は、テナントスコープのユニークインデックスを使用して、データベースレベルでデータ分離を実施します。すべてのリソース ID は **テナントごと** に一意であり、異なるテナントが同じ ID を独立して使用できます。

| コレクション | インデックス名 | フィールド | 自動生成 ID |
|-----------|-----------|--------|-------------------|
| `entities` | `idx_entity_unique_v3` | `(tenant, servicePath, entityId)` | なし (ユーザー指定) |
| `subscriptions` | `idx_subscription_unique` | `(tenant, servicePath, subscriptionId)` | あり (UUID) |
| `registrations` | `idx_registration_unique` | `(tenant, servicePath, registrationId)` | あり (URN) |
| `policies` | `idx_policy_unique_v2` | `(tenantId, policyId)` | あり (UUID) |
| `policySets` | `idx_policyset_unique_v2` | `(tenantId, policySetId)` | あり (UUID) |
| `apiKeys` | `idx_api_key_id_v2` | `(tenantId, keyId)` | あり (UUID) |
| `oauthClients` | `idx_oauth_client_id_v2` | `(tenantId, oauthClientId)` | あり (UUID) |
| `oauthClients` | `idx_client_id_v2` | `(tenantId, clientId)` | あり (UUID) |
| `rules` | `idx_rule_unique_v2` | `(tenantId, ruleId)` | あり (UUID) |
...
| 要件 | 最小プラン | 備考 |
|-------------|-------------|-------|
| マルチリージョンクラスタ | **M10+** (専用) | M0/M2/M5 (共有) はマルチリージョンレプリケーションをサポートしていません |
| クロスリージョンリードレプリカ | **M10+** | セカンダリリージョンの読み取りアクセスに必要 |
| Change Streams | **M10+** (専用) | サブスクリプションに必要。Change Streams は WiredTiger ストレージエンジンを持つレプリカセットで動作します。サーバーレスインスタンスはサポートしていません。Atlas App Services の制限:M10/M20 は最大 100、M30/M40 は最大 1000 の同時ストリーム |

> **注意**:本番ワークロードには M30+ を推奨します。エンティティ数によるサイジングガイドラインについては、[FAQ](../faq.md) も参照してください。レイテンシベースのルーティングのためにゾーンベースのシャーディング (Global Cluster) が必要な場合は、現在のティア要件について [Atlas Global Clusters ドキュメント](https://www.mongodb.com/docs/atlas/global-clusters/) を参照してください。

MongoDB Atlas 接続文字列は両方のリージョンで同じです — Atlas が内部的にレプリカルーティングを処理します。セカンダリリージョンはデフォルトで `readPreference: primaryPreferred` を使用しますが、`MONGODB_READ_PREFERENCE` 環境変数で変更できます。
...
| Point-in-Time Recovery | 含まれる | すべての GlobalTable レプリカで有効 |

**GlobalTables として設定される 3 つのテーブル** (プライマリとセカンダリ間でレプリケート):
...
> **注意**: DynamoDB Global Tables は `PAY_PER_REQUEST` (オンデマンド) と `PROVISIONED` の両方の課金モードをサポートしています (デフォルトは `PROVISIONED`)。GeonicDB のテンプレートは、運用を簡素化しキャパシティプランニングを回避するための設計上の選択として `PAY_PER_REQUEST` を使用しています。クロスリージョンレプリケーションコストが適用されます — 書き込みはレプリカごとにレプリケートされた書き込みリクエストユニット (rWRUs) として課金されます。

### HA 関連の環境変数

| 変数 | 説明 | デフォルト |
|----------|-------------|---------|
| `REGION_ROLE` | リージョンロール (`primary` / `secondary`) | `primary` |
| `PRIMARY_REGION` | プライマリ AWS リージョン | `ap-northeast-1` |
| `SECONDARY_REGION` | セカンダリ AWS リージョン | `ap-northeast-3` |
| `JWT_SECRET_ARN` | JWT シークレット用の Secrets Manager ARN | (空) |
| `MONGODB_URI_ARN` | MongoDB URI 用の Secrets Manager ARN | (空) |
| `HEALTH_CHECK_DYNAMODB` | DynamoDB ヘルスチェックを有効化 | `false` |
| `HEALTH_CHECK_EVENTBRIDGE` | EventBridge ヘルスチェックを有効化 | `false` |
| `MONGODB_READ_PREFERENCE` | MongoDB リードプリファレンス | `primaryPreferred` |
| `MONGODB_WRITE_CONCERN` | MongoDB ライトコンサーンレベル | `majority` |
| `MONGODB_READ_CONCERN` | MongoDB リードコンサーンレベル | `majority` |
| `MONGODB_RETRY_WRITES` | MongoDB リトライライト | `true` |### Read/Write 一貫性ポリシー
...
マルチリージョンデプロイメントでは 2 つのテンプレートを使用します:

| テンプレート | スコープ | 説明 |
|----------|-------|-------------|
| `infrastructure/template.yaml` | リージョン | メインスタック (Primary と Secondary の両方にデプロイ) |
...
Secrets Manager でシークレットを作成します。Secrets Manager はリージョナルサービスのため、各リージョンでシークレットを作成する必要があります。
...
セカンダリデプロイメントでは以下が**自動的に無効化**されます(`IsPrimaryCondition` で制御):
- サブスクリプションマッチャー / 通知送信 Lambda
- SQS 通知キュー / DLQ
- Change Stream プロセッサのスケジュールトリガー
- DynamoDB テーブル(GlobalTable 経由でプライマリから既にレプリケート済み)

#### ステップ 4: Route 53 フェイルオーバーの設定

Route 53 はグローバルリソースのため、別テンプレートを使用して一度だけデプロイします。

> **注意**: Route 53 フェイルオーバーを設定する前に、各リージョンでベースパスマッピングを持つ API Gateway カスタムドメインが必要です。生の `execute-api` ホスト名にはステージパスセグメントが含まれており、DNS CNAME レコードで直接使用できません。まず各リージョンでベースパスマッピングを持つカスタムドメイン(例: `primary-api.example.com`、`secondary-api.example.com`)を設定してください。
...
このテンプレートで作成されるもの:
- Route 53 ヘルスチェック(プライマリの `/health/ready` を 10 秒ごとに監視、3 回連続失敗で異常とマーク)
...
Route 53 ヘルスチェックがプライマリの `/health/ready` で 3 回連続失敗を検出した場合:

1. **自動フェイルオーバー**: Route 53 が DNS をセカンダリエンドポイントに切り替え(RTO < 5 分)
2. **通知**: CloudWatch アラーム → SNS → 管理者へのメール通知
3. **フェイルオーバー記録**: フェイルオーバー Lambda が DynamoDB に状態を記録

プライマリが復旧した場合:
1. Route 53 ヘルスチェックが正常ステータスを検出
2. DNS が自動的にプライマリに切り戻し
3. SNS 経由で復旧通知
...
以下のテーブルはリージョンローカル(GlobalTable では**ない**):

| テーブル | 理由 |
|-------|--------|
| `RateLimitBucketsTable` | レート制限はリージョンごとに計算される |
| `WsConnectionsTable` | WebSocket 接続はリージョン固有 |### WAF 設定

各リージョンの API Gateway に WAF WebACL が適用されています:

| ルール | 説明 |
|------|-------------|
| AWSManagedRulesCommonRuleSet | 一般的な攻撃に対する保護 (除外項目: SizeRestrictions_BODY、CrossSiteScripting_BODY、EC2MetaDataSSRF_BODY) |
| AWSManagedRulesSQLiRuleSet | SQL インジェクションに対する保護 |
| AWSManagedRulesKnownBadInputsRuleSet | 既知の不正な入力に対する保護 (Log4Shell など) |
| RateLimitPerIP (2000 req / 5min per IP) | IP あたりのレート制限 |
| SizeRestrictionBody10MB | カスタムボディサイズ制限 (10 MB) |
...
セカンダリモードでは、Change Stream プロセッサは起動時にスキップされます (ログ出力で確認できます)。## デプロイメント管理 (ホスト名ベースのマルチデータベース)
...
| フィールド | 型 | 必須 | 説明 |
|-------|------|:--------:|-------------|
| `hostname` | String | ✅ | パーティションキー。リクエストの Host ヘッダと照合されます |
...
| `enabled` | Boolean | ✅ | `false` の場合、デフォルト DB にフォールバックします |
...
現在 (Phase 0) では、デプロイメント設定は AWS CLI または DynamoDB コンソールを使用して直接管理します。
...
| デプロイメントが `enabled: false` を持つ | 未登録と同様に扱われます |
| DynamoDB 接続失敗 | エラーをログに記録 + `null` フォールバック (リクエストはブロックされません) |
| キャッシュ期限切れ | 次のリクエスト時に DynamoDB を再クエリします |
| 不明なホスト名での継続的なリクエスト | ネガティブキャッシュ (`null` がキャッシュされ、DynamoDB 負荷を軽減します) |

### 設定変更の伝播

デプロイメント設定の変更は、キャッシュ TTL (デフォルト 5 分) が期限切れになった後に有効になります。即座に反映するには、Lambda 関数を再デプロイしてください。
...
`geonicdb-deployments` テーブルは DynamoDB GlobalTable として設定されており、Primary と Secondary の両方のリージョンに自動的にレプリケートされます。デプロイメント設定は、フェイルオーバー時に即座に利用可能です。
