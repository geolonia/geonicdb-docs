...
- `@context`: JSON-LD vocabulary URI (optional, HTTP(S) URL only). For custom data models, specify well-known vocabulary (e.g., `https://schema.org/email`) to improve Linked Data interoperability. Properties without `@context` get auto-generated URLs on this broker's own base URL (`{brokerBaseUrl}/vocab/{tenantId}/{propertyName}`, #1984) — dereferenceable via `GET /vocab/{tenantId}/{term}` as `rdf:Property` (#3191). `rdfs:domain` is added only when exactly one model declares the attribute; `rdfs:range` only when `ngsiType`/`valueType` resolves to a datatype (`Relationship` and unresolved value types omit it). Multiple declaring models omit both. See [API.md → Term kind](../api-reference/endpoints.md#term-kind-type).
