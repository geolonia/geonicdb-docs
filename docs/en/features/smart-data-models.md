...
| Domain | Example Included Models |
|---------|----------------|
...
- `example`: Sample value demonstrating actual usage
- `required`: Whether it's a required field (optional)

## @context Auto-completion

When retrieving entities via the NGSI-LD API, GeonicDB automatically provides the appropriate @context for known Smart Data Model types.

### How It Works

Priority order for @context resolution when retrieving entities:

1. **Explicit @context** (specified in Link header or parameter) - Always takes priority
2. **Smart Data Models @context** (when entity type is a known SDM) - Auto-completed
...
**Response** (@context automatically added):
```json
{
  "@context": [
...
- **@context is not stored in storage**: @context is metadata dynamically generated during API responses
- **Explicit @context takes priority**: If @context is specified in the Link header, it takes priority over SDM auto-completion
...
- **Ecosystem integration**: Collaboration with FIWARE Marketplace and other FIWARE components

### Enhanced AI Assistant Capabilities

MCP tools enable AI assistants (such as Claude) to:
...
- **Get property information**: Retrieve detailed information for each property from `propertyDetails`
  ...
  - Understand value types (numeric, string, GeoJSON structure, etc.)
  - Use sample values demonstrating actual usage
  - Identify required fields
- **Create accurate entities**: Generate correctly structured NGSI-LD entities based on retrieved information
- **Apply domain-specific best practices**: Implementation following Smart Data Models standards

**Recommended Workflow**:
1. Search models with `list_models`
2. Get `propertyDetails` of selected model with `get_model`
