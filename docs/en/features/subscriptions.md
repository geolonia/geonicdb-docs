...
GeonicDB supports real-time event streaming using WebSocket. You can subscribe to entity changes in real-time and reflect them instantly in web applications or dashboards.
...
EventBridge ─┬─> SubscriptionMatcher -> SQS -> HTTP/MQTT  [existing]
             └─> WsBroadcastFunction -> API GW WebSocket -> clients  [new]
```

- **Connection state**: DynamoDB (PAY_PER_REQUEST, TTL automatic cleanup)
...
|--------------|-------------|
| `EVENT_STREAMING_ENABLED` | Enable with `true` |
| `WS_CONNECTIONS_TABLE` | DynamoDB connection table name (automatically configured) |
| `WS_API_ENDPOINT` | WebSocket API endpoint (automatically configured) |
...
When `AUTH_ENABLED=true`, an authentication token is required for WebSocket connections. Tokens are extracted in the following priority order:

1. **`Authorization` header (recommended)**: `Authorization: Bearer <token>` — Most secure method
2. **`Sec-WebSocket-Protocol` header (for browsers)**: `Sec-WebSocket-Protocol: access_token, <token>` — Use when `Authorization` header cannot be set in browser clients
3. **`token` query parameter (deprecated)**: `?token=<token>` — Remains for backward compatibility. Has security risks as the token is exposed in the URL, scheduled for future removal
...
- The `tenant_admin` / `user` roles can only connect to tenants they belong to

| Condition | Result |
|-----------|--------|
| `AUTH_ENABLED=false`, no token | ✅ Connection allowed |
| `AUTH_ENABLED=true`, no token | ❌ Connection denied (1008) |
| `AUTH_ENABLED=true`, invalid token | ❌ Connection denied (1008) |
| `AUTH_ENABLED=true`, valid token, own tenant | ✅ Connection allowed |
| `AUTH_ENABLED=true`, valid token, other tenant | ❌ Connection denied (1008) |
| `AUTH_ENABLED=true`, super_admin, any tenant | ✅ Connection allowed |

### Connection Flow

1. Client connects to WebSocket URL (`tenant` query parameter required, token also required when authentication is enabled)
2. Server validates token and checks tenant access permissions (when authentication is enabled)
3. Server records connection in DynamoDB (TTL: 2 hours)
4. Optional: Set filter conditions with `subscribe` message
5. Server pushes events on entity changes
...
- **Entity Type Filter (optional)**: Receive only types specified in `entityTypes` of `subscribe` message. If not specified, receive all types
- **Entity ID Pattern Filter (optional)**: Filter by regex pattern in `idPattern` of `subscribe` message
  ...
  connect() {
    ...
    this.ws.onclose = () => {
      if (this.shouldReconnect) {
        console.log(`🔄 Reconnecting in ${this.