...
| `batch` | - | create, upsert, update, merge, delete, query, purge | Batch operations for entities (up to 1,000 items) |
...
GeonicDB supports the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/). You can directly connect to the Context Broker from MCP-compatible AI clients (such as Claude Desktop).
...
- **Authentication disabled**: If omitted, the `default` tenant is used.
- **Authentication enabled**: If omitted, the logged-in user's tenant is used by default. `super_admin` can access any tenant, but `tenant_admin`/`user` can only access their own tenant.
...
- **Format**: Paths starting with `/` (e.g., `/hello`, `/city/sensors`)
...
- **Authentication**: Bearer token is required when `AUTH_ENABLED=true`. When `AUTH_ENABLED=false`, it operates without authentication.
- **OAuth Scopes**: When using OAuth tokens, the appropriate OAuth scope is required for each MCP tool operation (e.g., `read:entities` for entity reads, `write:entities` for writes). JWT RBAC tokens are not subject to scope restrictions.
...
JSON Schema (Draft 2020-12) is automatically generated when custom data models are created. AI tools can leverage this JSON Schema for the following purposes.

### Use Cases for AI Tools

**Schema Reference During Entity Creation**: AI agents can retrieve custom data models using the `config` tool's `data_models` resource and reference the `jsonSchema` field to generate entities that comply with correct types and validation rules.

```yaml
# 1. Get JSON Schema for custom data model
config tool:
  action: "get"
...
**Automatic Validation Error Correction**: When validation errors are returned during entity creation, AI agents can reference the JSON Schema to identify the cause of the error and correct values accordingly.
...
The `/openapi.json` endpoint dynamically adds JSON Schemas for custom data models associated with the authenticated user's tenant to `components/schemas`. This allows AI tools and code generation tools that reference the OpenAPI specification to automatically recognize tenant-specific data models.
...
When retrieving entities via the NGSI-LD API, if `contextUrl` is set in the custom data model, custom context is automatically included in the response's `@context`. Similar to Smart Data Models context, AI agents can use this `@context` to interpret semantic information of entities.
