...
Custom data models created or updated through the MCP `config` tool get the **same auto-generated `@context`** as ones created over HTTP (#1986): when `contextUrl` is omitted, a JSON-LD context is generated from `propertyDetails`, registered under `/ngsi-ld/v1/jsonldContexts/`, and its URL stored as `contextUrl`. The broker's own public URL is resolved with the **same precedence as the HTTP controller** (#2075): `API_BASE_URL` → the MCP request's own `Host` header → the local server address. The MCP tool handler itself receives no HTTP event, but the server is rebuilt per request (`POST /mcp` is itself an HTTP call), so that request's event is threaded through to the `config` tool instead of being resolved with no event at all. Deployments that leave `API_BASE_URL` unset (e.g. an environment missing the SSM parameter) now get a `contextUrl` built from the request's own `Host` instead of always falling back to `http://localhost:3000`.
