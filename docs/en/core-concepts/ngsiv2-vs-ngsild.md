...
Invalid charset / terms longer than 500 characters / scopeQ strings longer than 2,000 characters / more than 50 OR alternatives or total terms → **400 BadRequestData** (#2583; same charset as write-side `SCOPE_PATH_PATTERN`).
...
- `pick` and `omit` cannot be used simultaneously (nor with `attrs`)
- When using `pick`: only `@context` and the listed Entity members remain. Listing `id` / `type` / `scope` is valid; omitting them yields a payload that is no longer a valid NGSI-LD Entity (intentional per ETSI v1.8.1 New Features).
- When using `omit`: listed Entity members are removed, including `id` and `type` if named (#2275).
- Retrieve Entity returns `404 ResourceNotFound` when no members remain after projection. Query Entities still returns `200`.
