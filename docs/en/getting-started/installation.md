...
## Bootstrap: Managing the Initial Super Admin

After deploying to a fresh environment (staging or production), no super admin user exists in the database. Use `scripts/create-super-admin.ts` to create, update, or delete one directly via MongoDB.

### Prerequisites

- Node.js + npm dependencies installed (`npm ci`)
- 1Password CLI installed (for staging/production), or `MONGODB_URI` set explicitly
...
Each environment item must contain these fields:
...
CLI args (`--email`, `--password`) always take precedence over vault values.

### Notes

- Safe to re-run for existing `super_admin` users — updates the password in place. If the email exists with a different role, the script aborts to prevent unintended privilege escalation.
- The password must meet the application's password policy (uppercase, lowercase, number, special character).
- After creating the user, verify login works via `POST /auth/login` before relying on it.
- Delete is permanent — the user is removed from MongoDB immediately.
...
- Network access: Allow `0.0.0.0/0` for development. In production, use VPC + NAT Gateway (Elastic IP) for fixed egress IPs, or use AWS PrivateLink / VPC Peering for private connectivity to Atlas
...
| Parameter | Environment Variable | Default | Description |
|-----------|---------|-----------|------|
| `OAuthEnabled` | `OAUTH_ENABLED` | ~~`true`~~ | **Deprecated** (#672): OAuth 2.0 is always enabled when `AUTH_ENABLED=true`. This variable is ignored. |
...
| Parameter | Environment Variable | Default | Description |
|-----------|---------|-----------|------|
| `EventStreamingEnabled` | `EVENT_STREAMING_ENABLED` | `true` | Enable WebSocket event streaming |
...
#### MongoDB Indexes (Tenant-Scoped Unique Constraints)

GeonicDB enforces data isolation at the database level using tenant-scoped unique indexes. All resource IDs are unique **per tenant**, allowing different tenants to use the same ID independently.

| Collection | Index Name | Fields | Auto-generated ID |
|-----------|-----------|--------|-------------------|
| `entities` | `idx_entity_unique_v3` | `(tenant, servicePath, entityId)` | No (user-specified) |
| `subscriptions` | `idx_subscription_unique` | `(tenant, servicePath, subscriptionId)` | Yes (UUID) |
| `registrations` | `idx_registration_unique` | `(tenant, servicePath, registrationId)` | Yes (URN) |
| `policies` | `idx_policy_unique_v2` | `(tenantId, policyId)` | Yes (UUID) |
| `policySets` | `idx_policyset_unique_v2` | `(tenantId, policySetId)` | Yes (UUID) |
| `apiKeys` | `idx_api_key_id_v2` | `(tenantId, keyId)` | Yes (UUID) |
| `oauthClients` | `idx_oauth_client_id_v2` | `(tenantId, oauthClientId)` | Yes (UUID) |
| `oauthClients` | `idx_client_id_v2` | `(tenantId, clientId)` | Yes (UUID) |
| `rules` | `idx_rule_unique_v2` | `(tenantId, ruleId)` | Yes (UUID) |

Legacy v1 indexes (without `tenantId` prefix) are automatically dropped during Lambda cold start (`dropLegacyIndexes()` in `src/infrastructure/mongodb/client.ts`).
...
> **Note**: Each region requires an API Gateway custom domain with base path mapping before configuring Route 53 failover. Raw `execute-api` hostnames include a stage path segment and cannot be used directly in DNS CNAME records. Set up custom domains (e.g., `primary-api.example.com`, `secondary-api.example.com`) with base path mapping in each region first.
...
| Rule | Description |
|------|-------------|
| AWSManagedRulesCommonRuleSet | Common attack protection (excludes: SizeRestrictions_BODY, CrossSiteScripting_BODY, EC2MetaDataSSRF_BODY) |
| AWSManagedRulesSQLiRuleSet | SQL injection protection |
| AWSManagedRulesKnownBadInputsRuleSet | Known bad inputs protection (Log4Shell 等) |
| RateLimitPerIP (2000 req / 5min per IP) | Per-IP rate limiting |
| SizeRestrictionBody10MB | Custom body size limit (10 MB) |
