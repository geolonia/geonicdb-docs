...
description: "Development Environment Setup & Installation"
outline: deep
---
# Developer Guide
...
# IP addresses allowed to access Admin API (comma-separated)
...
To enable CADDE (Cross-domain Architecture for Data Distribution and Exchange) integration:
...
# JWT validation settings (when CADDE_AUTH_ENABLED=true)
...
| Command | Description |
|---------|-------------|
| `npm start` | Start local development server (uses in-memory MongoDB) |
...
This project uses two types of test frameworks:
...
│   └── orion-tutorial.feature      # Tutorial based on Orion user manual
├── common/
...
└── ngsi-ld/
    ...
    ├── geojson-output.feature      # GeoJSON output
    ├── crs-transform.feature       # Coordinate Reference System (CRS) transformation
    └── attributes.feature          # Attribute list & details
```
...
## Integration with Applications (Using as npm Package)
...
Use the `npx geonicdb` command to start standalone. With the `--proxy` option, requests not matching GeonicDB routes are forwarded to your app's development server.
...
# Start with proxy (integration with Vite etc.)
...
Browser → localhost:3000 (GeonicDB)
  ...
  └── Others (HTML, JS, CSS etc.)       → Proxied to app dev server
```

> **Note**: If URLs overlap (e.g., if app also has `/llms.txt`), GeonicDB takes precedence.

### Example package.json Configuration

Use `concurrently` to start both GeonicDB and your app's development server simultaneously. With `--kill-others`, when one process exits, the other automatically stops:
...
Add proxy configuration to Vite to access the API from either port:
...
| `mongoUri` | `string` | MongoDB connection URI (available for testing) |
| `close()` | `() => Promise<void>` | Stop server and MongoDB |

### Installing from Private Repository

For private repositories, it works as-is if SSH keys are registered with GitHub:

```bash
npm install -D github:geolonia/geonicdb
```

In CI/CD environments, you need to configure GitHub Personal Access Token or Deploy Key. For production team use, consider publishing to GitHub Packages.

## Local Development Server

### Simple Server (Recommended)

Run `npm start` to launch a local server with in-memory MongoDB. No external MongoDB instance required.
...
Default port is `3000`. You can change the port via CLI argument or environment variable:
...
> **Tip**: Combined with git worktree, you can run servers from different branches simultaneously:
> ```bash
...
To stop the server, press `Ctrl+C`. MongoDB stops automatically.
