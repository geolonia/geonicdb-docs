...
> **Note:** The NGSI-LD API ignores the `Fiware-ServicePath` header per ETSI GS CIM 009 spec. Hierarchy is managed via the `scope` entity property and `scopeQ` query parameter. `servicePath` and `scope` are independent concepts — they are not automatically synchronized (see [INTEROPERABILITY.md](../core-concepts/ngsiv2-vs-ngsild.md#3-scope-scope-hierarchy)).
>
