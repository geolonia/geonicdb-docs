...
**The auto-generated `contextUrl` itself is also stable (#2094).** When `propertyDetails` changes trigger a context regeneration, the context document body is updated in place at its existing URL — the URL does not change. Previously distributed `contextUrl` values therefore keep resolving instead of 404/504ing after a model update. This does not apply to manually specified `contextUrl` values, since GeonicDB does not manage that document.
