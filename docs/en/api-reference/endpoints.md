...
| `write:entities` | Write entities (implies `read:entities`) | ✅ | ✅ | ✅ |
| `read:subscriptions` | Read subscriptions | ✅ | ✅ | ✅ |
| `write:subscriptions` | Write subscriptions (implies `read:subscriptions`) | ✅ | ✅ | ✅ |
| `read:registrations` | Read registrations | ✅ | ✅ | ✅ |
| `write:registrations` | Write registrations (implies `read:registrations`) | ✅ | ✅ | ✅ |
| `read:rules` | Read rules | ✅ | ✅ | ✅ |
| `write:rules` | Write rules (implies `read:rules`) | ✅ | ✅ | ✅ |
| `read:custom-data-models` | Read custom data models | ✅ | ✅ | ✅ |
| `write:custom-data-models` | Write custom data models (implies `read:custom-data-models`) | ✅ | ✅ | ✅ |
...
> `entityType` is automatically extracted from the HTTP request at the path level — from the `?type=` query parameter or the request body's `type` / `@type` field — enabling entity-type-based access control without entity-level checks. `entityId` and `entityOwner` are only available for entity-level authorization checks (via `requireEntityAuthz`).
...
Users can manage their own OAuth clients. Max 5 clients per user. Scope restrictions apply by role.
...
Manage API keys for authentication via `X-Api-Key` header. Keys use `gdb_` prefix + 64 hex characters. SHA-256 hashed for storage. Gated by `features.apiKeysEnabled` tenant flag (enabled by default; when disabled, creation and authentication are rejected with 403).
