...
- Batch upsert (NGSI-LD default update mode) matches by `entityId` only and **appends** novel Entity Types to the existing list (does not replace or delete; ETSI GS CIM 009 5.6.8.4 → 5.6.2.4 / #2455). NGSIv2 batch append still preserves the stored type
...
NGSI-LD pagination links also include the `type` Link Target Attribute. Its value is **exactly** the media type of the original request (the negotiated `Content-Type`), as required by [ETSI GS CIM 009](https://cim.etsi.org/NGSI-LD/official/clause-6.html) clause 6.3.10. JSON-LD `@context` links keep `type="application/ld+json"` regardless of the response media type.
...
| `servicePath` | Filter by service path. If omitted, defaults to the servicePath authorization was evaluated against (resolved `Fiware-ServicePath`, default `/`) rather than matching every servicePath (#2259) — see [REACTIVCORE_RULES.md](../features/reactivcore-rules.md) |
...
For Route 53 health checks and other liveness probes. Checks whether the service is running.
...
For Route 53 health checks and other readiness probes. Checks MongoDB connectivity, and optionally performs deep health checks for DynamoDB, EventBridge, and the WebSocket delivery path.
...
The optional `reset=true` query parameter clears the counters after they are read. Those counters (`counters`, `timing`, `notifQueue`) are **process-global and carry no tenant dimension**, so a reset wipes every tenant's numbers at once. It is therefore restricted to `super_admin`; any other role receives `403`. Reads without `reset` are unaffected.
...
Returns metrics in Prometheus exposition format. While authentication is enabled (the default), only authenticated users can access this endpoint. These metrics can be used for Prometheus-based monitoring and integration with Grafana dashboards.
...
##### Web Push VAPID Public Key (#3014)

```http
GET /.well-known/webpush-vapid-key
```

Returns `{ "publicKey": "<VAPID public key>" }` for `PushManager.subscribe({ applicationServerKey })`.
Returns `503` when `WEBPUSH_VAPID_*` is unset. See [SUBSCRIPTIONS.md](../features/ngsi-subscriptions.md#web-push-3014) and ENV.md.
...
Returns metrics by tenant and service path.

**Scope (#2217)** — `super_admin` sees every tenant. Any other authenticated role sees only
its own tenant, and that tenant is resolved from the access token: the `Fiware-Service` /
`NGSILD-Tenant` request headers do **not** select which tenant's metrics are returned
(`/admin/*` is dispatched before the tenant-boundary check, so those headers are unverified
here). A caller whose tenant cannot be resolved (no tenant assigned, or the tenant no longer
exists) receives `{"services": {}}` — fail-closed; it does **not** fall back to returning
every tenant.

**Reset (#2236)** — `DELETE /admin/metrics` and `GET /admin/metrics?reset=true` clear the
counters. Those counters are **process-global and carry no tenant dimension**, so a reset
wipes every tenant's numbers at once and cannot be sliced per tenant. Both are therefore
restricted to `super_admin`; any other role (including `api_key` and `oauth_client`)
receives `403`. Reads without `reset` are unaffected and keep the per-tenant scoping
described above. The same restriction applies to `GET /statistics?reset=true`, which resets
the same counters.
...
Returns the hourly API call history (call count, error count, average response time).
`tenant` is honoured for `super_admin` only; any other role is scoped to its own tenant.

**Persistence (#3207):** Counts are aggregated in-memory per Lambda container, then
written to the shared store (`USAGE_STATS_TABLE_NAME` / Mongo `apiCallMetrics`) by a
threshold flush at the end of each API invocation (`FLUSH_MIN_BUCKETS` /
`FLUSH_MAX_AGE_MS` in `@config/defaults`). `GET` here also flushes the **control-plane**
process's in-memory buckets before querying. After `#1507`, data-plane and control-plane
are separate Lambdas, so admin `GET` alone cannot flush data-plane memory — invocation-end
flush is required. Residual loss remains if a container is reclaimed before a flush
completes (best-effort metrics, not a complete audit trail). Standalone E2E shares one
process, so it cannot detect the cross-Lambda gap.
...
> **Authorization (#2465 / #2472).** Catalog applies the same row-level read filter as `GET /v2/entities`. Credentials that Permit only `/catalog/**` (without a matching `GET /v2/entities` Permit) receive **403** — intentional fail-closed. Open-data portal recipe: [AUTH.md — Catalog read credentials](../reference/auth.md#catalog-read-credentials-2465--2468--2472).
...
> **XACML exclusion (#2469).** CADDE v4 paths (`/cadde/api/v4/catalog`, `/cadde/api/v4/entities`) do not evaluate GeonicDB XACML policies — including row-level `Deny(entityType)` rules. Enabling CADDE is a deployment-wide, all-tenant data-exchange gate. See SECURITY.md — CADDE and XACML.
...
When `authEnabled: true` in the CADDE settings document, CADDE requests require Bearer authentication:
...
Set `jwksUrl` through `PUT /admin/cadde` to enable JWT signature verification for Bearer tokens. Configuration is stored in MongoDB (`settings`, `_id: 'cadde'`) — **not** via `CADDE_JWKS_URL` / `CADDE_JWT_*` environment variables.
...
| `/statistics` | GET | FIWARE Orion-compatible statistics (authentication required; `reset=true` is super_admin only) | 200 | 401, 403 |
...
#### Update Dry-Run (GeonicDB extension, #2098)

Updating a data model does not automatically re-validate existing entities (see [Entity Validation](#entity-validation) below) — a model tightened via `PATCH` can silently leave existing entities non-conformant until they are next written, at which point the write fails with `400`. `PATCH /custom-data-models/{type}?dryRun=true` lets you check this **before** applying the update.

With `dryRun=true`, the update is **not applied** — no database write, no `@context` update, no unique-constraint index sync. Instead, the response is a `200` conformance report describing how many existing entities of that type would fail validation against the updated model (`required` / `validation` / `additionalProperties` rules, evaluated with the same validation logic used on the write path):
...
The scan is scoped by tenant and entity type only (not by service path or protocol). Authorization and ownership checks (`assertOwnership`) are the same as for a real `PATCH`.

`dryRun=false` or omitted behaves as a normal update. Any other value (e.g. `?dryRun=yes`) returns `400 Bad Request` rather than silently applying the update, so a typo cannot turn into an accidental write.

Dry-run is available on the HTTP `PATCH` endpoint only — the MCP `custom_data_models` action and A2A have no equivalent option.

The scan is scoped to the tenant resolved from the **authenticated actor**, not from the `Fiware-Service` / `NGSILD-Tenant` request header (this endpoint deliberately skips `checkTenantAccess`, so the header is not validated against the caller's tenant).
...
**Required attribute deletion (GeonicDB extension):** deleting an attribute that has `required: true` returns `400 Bad Request` — via NGSIv2 `DELETE /v2/entities/{id}/attrs/{attr}`, NGSI-LD `DELETE /ngsi-ld/v1/entities/{id}/attrs/{attr}`, and NGSI-LD merge patch deletion (`urn:ngsi-ld:null`). Only deletions that would remove the attribute entirely are rejected: deleting one multi-attribute instance by `datasetId` while another instance remains returns `204` as usual. Attributes with `required: false`, and attributes on models with `isActive: false`, can still be deleted.

**`defaultValue` auto-fill (GeonicDB extension):** if `propertyDetails.<name>.defaultValue` is set, entities that omit that attribute have it auto-filled on writes that run full validation — creation (`POST /v2/entities`, `POST /ngsi-ld/v1/entities`, `POST /ngsi-ld/v1/entityOperations/create`), upsert-as-create (`POST /v2/op/update` with `append`, `POST /ngsi-ld/v1/entityOperations/upsert|update`), and full attribute replacement (`PUT /v2/entities/{id}/attrs`, batch `replace`). Partial updates (`PATCH .../attrs`, attribute append `POST .../attrs`) do not auto-fill. Explicitly supplied values are never overwritten; combined with `required: true`, the attribute becomes effectively optional.

**`indexed` (reserved, not implemented):** `propertyDetails.<name>.indexed` is accepted but currently ignored — no MongoDB index is created.
...
Entity types and attributes share the same namespace (`{brokerBaseUrl}/vocab/{tenantId}/{term}`), so the response distinguishes them through `@type` (#3191). Without this, RDF-level vocabulary alignment (federation with an external Context Broker, CADDE) cannot tell a class from a property.
...
`rdfs:range` comes from the declaration: `GeoProperty` → `https://purl.org/geojson/vocab#Geometry`, `LanguageProperty` → `rdf:langString`, `Relationship` → omitted (the target class is not declared in the model), otherwise the XSD datatype for the `valueType` (`string`/`number`/`integer`/`boolean`/`datetime`/`uri`/`geojson`). `array` and `object` have no XSD counterpart, so `rdfs:range` is omitted for them.

Two properties of this endpoint are deliberate:

- **An unknown term still returns 200**, not 404. A 404 would turn this unauthenticated endpoint into an oracle for enumerating which types and attributes exist in a tenant.
- **`description` is never copied into `rdfs:comment`.** In practice those descriptions carry internal notes (PII markers, storage layout), and this endpoint is public.
...
**Priority 1 exists because one deployment serves many hostnames, each with its own database** (#1291). A single deploy-wide `ApiBaseUrl` can only name the shared/default host, so before #3190 every custom data model created on a dedicated instance stored a `contextUrl` pointing at the shared host — where that context does not exist, giving a 404. The resolver therefore uses the **registered** hostname of the deployment row (not the raw `Host` header), which keeps the identifier stable regardless of how the request reached the broker. When `API_BASE_URL` names a different host than the one a request must advertise, the broker logs a warning once per host pair (`BaseUrl`, `#3190`) so the mismatch is visible in CloudWatch.

**Set `ApiBaseUrl` on any deployment that mints vocabulary IRIs from the default database.** Vocabulary IRIs are persisted identifiers; with priority 3 the value depends on which hostname the request arrived on, so a broker reachable under several hostnames (apex + API Gateway default URL) would mint different IRIs for the same term. A wildcard-only deployment leaves `ApiBaseUrl` unset by default (`.github/workflows/deploy-env.yml`); its per-hostname deployments are covered by priority 1.

**Existing models are not rewritten.** `contextUrl` is a stored value, so models created before #3190 on a dedicated instance keep their shared-host URL. `PATCH /custom-data-models/{type}` with `propertyDetails` alone does **not** migrate that host:
...
Supplying a new `contextUrl` in the same PATCH clears any managed `contextId` and deletes the auto-generated context document, so it is **not** a safe migration path for #3190 either. Repairing pre-#3190 dedicated-instance models is a separate operational migration (out of scope of the #3190 code change).

Vocabulary IRIs are **identifiers**, so they are never rewritten in place: when a model's `propertyDetails` change and the `@context` body is updated in place, the namespace already in use by that model is carried over. Models created before #1984 therefore keep their original (`https://example.com/vocab/...`) IRIs, which remain self-consistent with the entities written under them.

**The auto-generated `contextUrl` itself is also stable (#2094).** When `propertyDetails` changes trigger a context body update, the document is rewritten at its existing URL — the URL does not change. Previously distributed `contextUrl` values therefore keep resolving instead of 404/504ing after a model update. This does not apply to manually specified `contextUrl` values, since GeonicDB does not manage that document.
