...
> This document is separated from [API.md](./endpoints.md). For the main API specification, please refer to [API.md](./endpoints.md).
...
| `limit` | integer | Number of results (max: 1000) | 20 |
...
| `orderDirection` | string | Sort direction (`asc`, `desc`). **GeonicDB extension** (official spec uses only `!` prefix method) | `asc` |
| `type` | string | Filter by entity type | - |
| `typePattern` | string | Entity type regex pattern | - |
...
| `metadata` | string | Metadata output control (`on`, `off`). **GeonicDB extension** (official spec uses comma-separated name list with `*` wildcard, etc.) | `on` |
| `georel` | string | Geo-query operator (see [Geo-query](./endpoints.md#geo-query)) | - |
...
Returns the response in GeoJSON FeatureCollection format.
...
The response header will include `Content-Type: application/geo+json`.
...
**Response**
- Status: `201 Created` (new creation), `204 No Content` (update via upsert)
...
| Parameter | Type | Description |
|-----------|-----|------|
| `type` | string | Entity type (required when same ID exists across multiple types) |
| `attrs` | string | Attribute names to retrieve (comma-separated) |
...
Updates only the specified attributes. Non-existent attributes are added.
...
Replaces all attributes (unspecified attributes are deleted).
...
Adds new attributes (existing attributes are overwritten).

With `options=append`, only new attributes are added without overwriting existing ones (strict append mode). Returns `422 Unprocessable Entity` error if an already existing attribute name is included.
...
> **Note**: This endpoint differs from `/v2/entities/{entityId}?attrs=...` in that it does not include `id` and `type` fields. Use this when only attributes are needed.
...
Returned with different Content-Type depending on the value type:
...
| `text/plain` | Primitive value (`null`, `true`, `false`, numbers) or string |
...
**Note**: This operation preserves the existing attribute's type and metadata without changing them.
...
Receives notifications from external Context Brokers and processes entities with append (create if not exists, update if exists).
